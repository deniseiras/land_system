...
subset_loop: do while (b2o_next_subset(handle))
  ...
  layer_loop: do k = 1, n_layers
    ...
    do ak = 1, n_ak_levels
       resat_ak(jobs,resat_averaging_kernel_pak(ak)) = b2o_get_real(handle, "nonCoordinatePressure", rank=ak)
       resat_ak(jobs,resat_averaging_kernel_wak(ak)) = b2o_get_real(handle, "averagingKernelValue", rank=ak)
       resat_ak(jobs,resat_averaging_kernel_apak(ak)) = 0._B2O_DOUBLE
    end do
