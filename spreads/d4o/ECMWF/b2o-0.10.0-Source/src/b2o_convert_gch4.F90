subroutine b2o_convert_gch4(handle, status)
...
if (lhook) call dr_hook('b2o_convert_gch4',0,zhook_handle)
...
subset_loop: do while (b2o_next_subset(handle))
  ...
  layer_loop: do k = 1, n_layers
    ...
    body(jobs,body_obsvalue) = b2o_get_real(handle, "significandOfVolumetricMixingRatio", rank=m) &
    ...
    value  = b2o_get_real(handle, "significandOfVolumetricMixingRatio->firstOrderStatisticalValue", rank=m)
    ...
    value  = b2o_get_real(handle, "significandOfVolumetricMixingRatio->firstOrderStatisticalValue", rank=m, nesting=1)
    ...
    do ak = 1, n_ak_levels
      ak_value = b2o_get_real(handle, "averagingKernelValue", rank=ak)
      ak_pressure = b2o_get_real(handle, "nonCoordinatePressure", rank=n_ap_ak_levels+n_ak_levels*(k-1)+ak)
...
if (lhook) call dr_hook('b2o_convert_gch4',1,zhook_handle)

end subroutine b2o_convert_gch4
