subroutine b2o_convert_hiras(handle, status)

use b2o_internal
use b2o_option, only : B2O_HIRAS_CHANNELS
...
integer(kind=b2o_int), parameter :: MaxChans = 2275
...
if (lhook) call dr_hook('b2o_convert_hiras',0,zhook_handle)

! Read in namelist of channels to load

if (.not.allocated(channels)) then
  channels = load(B2O_HIRAS_CHANNELS)
end if
...
subset_loop: do while (b2o_next_subset(handle))
  ...
  rad(iobs,radiance_scanpos) = 4 * (b2o_get_int(handle, "fieldOfRegardNumber") - 1) + b2o_get_int(handle, "fieldOfViewNumber")
  ...
  ! HIRAS doesn't have scaled radiances
...
if (lhook) call dr_hook('b2o_convert_hiras',1,zhook_handle)
...
pure elemental function wavenumber(channel)
    ...
    select case (channel)
    case (1:777)     ; wavenumber =  650._b2o_double + 0.625_b2o_double * (channel-1)
    case (778:1642)  ; wavenumber = 1210._b2o_double + 0.625_b2o_double * (channel-778)
    case (1643:2275) ; wavenumber = 2155._b2o_double + 0.625_b2o_double * (channel-1643)
...
function load(file) result(channels)
    ...
    if (iostat /= 0) then
        call b2o_log(handle%context, B2O_ERROR, "Cannot read 'hiraschannels' namelist file")
        call b2o_exit(2)
    end if
    close(88)
...
end subroutine b2o_convert_hiras
