...
subroutine b2o_convert_ssmis_1d(handle, status)
...
integer(kind=b2o_int), parameter :: n_channels = 24
...
if (lhook) call dr_hook('b2o_convert_ssmis_1d',0,zhook_handle)
...
subset_loop: do while(b2o_next_subset(handle))
  ...
  sat(iobs,sat_satellite_instrument) = 908
  sat(iobs,sat_azimuth) = b2o_get_real(handle, "bearingOrAzimuth", rank=2)
  sat(iobs,sat_lsm_fov) = 0.01 * b2o_get_real(handle, "multiplexerHousekeeping", rank=1) ! stored in % in temporary location
  ...
  rad(iobs,radiance_typesurf) = b2o_get_int(handle, "surfaceFlag")

end do subset_loop

if (lhook) call dr_hook('b2o_convert_ssmis_1d',1,zhook_handle)

end subroutine b2o_convert_ssmis_1d
