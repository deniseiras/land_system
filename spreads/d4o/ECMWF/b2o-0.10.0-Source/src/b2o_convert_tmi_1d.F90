...
subroutine b2o_convert_tmi_1d(handle, status)
...
integer(kind=b2o_int), parameter :: n_channels = 9
...
if (lhook) call dr_hook('b2o_convert_tmi_1d',0,zhook_handle)
...
subset_loop: do while (b2o_next_subset(handle))
  ...
  sat(iobs,sat_satellite_instrument) = 365
  sat(iobs,sat_zenith) = b2o_get_real(handle, "satelliteZenithAngle")
  sat(iobs,sat_azimuth) = b2o_get_real(handle, "orbitNumber") / 100.d0

  if (b2o_is_missing(handle, "satelliteZenithAngle")) then
    call b2o_log(handle, B2O_WARNING, "b2o_convert_tmi_1d: Zenith Angle not found")
...
if (lhook) call dr_hook('b2o_convert_tmi_1d',1,zhook_handle)

end subroutine b2o_convert_tmi_1d
