...
subroutine print(this)
        implicit none
   class(AllMappings), intent(in)  :: this
   integer(kind=4)                 :: i

   do i=1,size(this%Entries)
      write(0,*) 'Entry: ',this%Entries(i)%reportype,this%Entries(i)%groupid, &
                         & this%Entries(i)%satid,this%Entries(i)%bufrtype,    &
                         & this%Entries(i)%subtype,this%Entries(i)%obstype,   &
                         & this%Entries(i)%codetype,this%Entries(i)%sensor,   &
                         & this%Entries(i)%satinst,this%Entries(i)%accumulation_length
   enddo
...
subroutine load_config_if_required() 
    ...
    if (.not.associated(CodeMappings%Entries)) then
        call get_environment_variable("ODB_CODE_MAPPINGS", length=len)
        allocate(character(len=len) :: config_filename)
...
function count_lines_in_file(file_handle) result(n_lines)
    ...
    n_lines = 0
    do
        read(file_handle,'(A)', iostat = RetCode) line
        if (RetCode < 0) exit
        if (trim(line) == "") cycle
        n_lines = n_lines + 1
    end do
...
subroutine load_report_type_mappings_from_config(config_filename)
    ...
    integer(kind=4)    :: inext,inext_a,inext_b
    integer(kind=4)    :: jcol
    integer(kind=4)    :: ncols
    character(len=256) :: line
    character(len=1)   :: delimiter_old,delimiter_new
    ...
    do i = 0,n_mappings
        ! Safety check
        if(i > size(CodeMappings%Entries))then
           write(0,*) 'ERROR : config file : ',trim(config_filename)
           call abor1("Error: odbmap_reportype - something has gone wrong parsing the config file")
        endif
        ...
        if (i==0) then
            ...
            if ( RetCode == -1) exit ! Detect end of file
            if (trim(line) == "") cycle ! Ignore empty lines
            ...
            ncols = 10
            do jcol=1,ncols
                inext_a = index(line,delimiter_old)
                inext_b = index(line,delimiter_new)
                inext = max(inext_a,inext_b)

                select case(jcol)
                    case(1)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%reportype
                    case(2)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%groupid
                    case(3)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%bufrtype
                    case(4)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%subtype
                    case(5)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%obstype
                    case(6)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%codetype
                    case(7)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%sensor
                    case(8)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%satinst
                    case(9)
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%satid
                    case(10)
                        if(inext == 0) inext = len_trim(line)+1
                        read(line(1:inext-1),'(i10)') CodeMappings%Entries(i)%accumulation_length
                end select
                line = trim(line(inext+1:))
            enddo
        end if
    end do
