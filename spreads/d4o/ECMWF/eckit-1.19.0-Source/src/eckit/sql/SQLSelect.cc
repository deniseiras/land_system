...
const SQLTable& SQLSelect::findTable(const std::string& name) const {
  ...
  if (names.size() == 0) {
    char *env = ::getenv("ODC_CANT_FIND_TABLE_SUBSTITUTE");
    if (env) {
      std::string tablename = env;
      std::cerr << "SQLSelect::findTable(" << name << ")@" << __FILE__ << ":" << __LINE__
		<< " : Can't find a table for column " << name
		<< " : Trying substitute table from env ODC_CANT_FIND_TABLE_SUBSTITUTE=" << tablename
		<< std::endl;
      for (const SQLTable* table : tables_) {
	if (table->hasColumn(tablename)) {
	  names.insert(table);
	}
      }
      if (names.size() == 0) {
	throw eckit::UserError("Can't find a table for ODC_CANT_FIND_TABLE_SUBSTITUTE", tablename);
      }
    }
    else {
