...
{
  char *env = ::getenv("ODC_COL_INDEX_ERROR_BAILOUT");
  size_t idx;
  if (env) { // Do not trigger exception
#if 0
    // Output pollution 
    std::cerr << "columnIndex(" << columnName << ",&md)@" << __FILE__ << ":" << __LINE__
	      << " : not triggering exception due to use of a substitute index value from env ODC_COL_INDEX_ERROR_BAILOUT=" << env
	      << std::endl;
#endif
