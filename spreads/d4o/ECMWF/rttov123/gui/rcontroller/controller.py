...
class MainController (GenericController):
    ...
    def __init__(self, app, debug):
        ...
        self.chooseCoeffsController = ChooseCoeffsController(self.mv, self)
    ...
    def CoeffsLoadedListener(self, msg):
        ...
        if self.project.myCoeffs.loadCoeffs:
            ...
            if self.rv:
                ...
                self.rv.WarmError("radiance window will close")
                self.rv.Close()
                self.rv = None
        else:
    ...
    def _RunDirectPC(self, e):
        ...
        if self.project.myCoeffs.loadCoeffs and self.project.isPC():
            ...
            if err == 0:
                if self.pcview:
                    if self.pcview.IsIconized():
                        self.pcview.Restore()
                    self.pcview.ReRead(self.project.pcFileName)
                    self.pcview.Show()
                else:
                    self.pcview = rview.pcView.pcView(
                        self.mv, self.project.pcFileName)
                if self.project.myOption["ADDRADREC"].value:
                    if self.rv:
                        if self.rv.IsIconized():
                            self.rv.Restore()
                        self.rv.read_rad_h5(self.project.pcFileName, addsolar)
                        self.rv.Refresh()
                    else:
                        self.rv = rview.radianceframe.RadianceFrame(
                            self.mv, "Radiance Viewer",
                            self.project.pcFileName, addsolar)
                        self.rv.Show()
                    # a tester il y a peut-etre pas besoin si listener surface
                    # changed utilise
                    if self.surfaceController.surfaceView:
                        self.write("plot emissivity reflectance")
                        self.surfaceController.surfaceView.\
                            PlotReflectanceEmissivity(
    ...
    def _RunK(self, e):
        ...
        if self.project.myCoeffs.loadCoeffs:
            ...
            if err == 0:
                ...
                kv = rview.kmatrixframe.KMatrixFrame(
                    self.mv,
                    "K Matrix Viewer run " + str(
                        self.RunkNumber), self.project.KMatrixFileName,
    ...
    def _RunKPC(self, e):
        ...
        if self.project.myCoeffs.loadCoeffs and self.project.isPC():
            ...
            if err == 0:
                ...
                if self.project.myOption["ADDRADREC"].value:
                    kv = rview.kmatrixframe.KMatrixFrame(
                        self.mv, "K Matrix Viewer run " + str(self.RunkNumber),
                        fileName, self.RunkNumber, self.project)
                    self.kv.append(kv)
                    kv.Show()
                else:
                    kvpc = rview.kpcmatrixframe.KPCMatrixFrame(
                        self.mv,
                        "K PC Matrix Viewer run " + str(self.RunkNumber),
                        fileName, self.project.npcscores, self.RunkNumber)
                    self.kvpc.append(kvpc)
                    kvpc.Show()
                    # kvpc.CenterOnParent()
                    kpcprofile = rview.kpcView.kpcView(
                        self.mv, None, None, 0, 10, fileName,
                        run=self.RunkNumber)
                    kpcprofile.Show()
            else:
...
class ChooseCoeffsController (GenericController):
    ...
    def __init__(self, parentFrame, parentController):
        """ Constructor of the MainController
            define a Project object
        ...
        self.theParentController = parentController
        super(GenericController, self).__init__()
...
def fxn():
    ...
    warnings.warn("MatplotlibDeprecationWarning", DeprecationWarning, stacklevel=1)
