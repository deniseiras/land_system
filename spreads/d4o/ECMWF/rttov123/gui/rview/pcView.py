...
class pcView(util.GenericView):
    ...
    def __init__(self, parent, pcFileName):

        util.GenericView.__init__(self, parent,  "PC SCORES")
        self.nbRun = 1
        ...
        self.nbpc = self.pc['PCSCORES'].shape[0]
        self.PCPageGraphic = PlotPage(nb, self.pc['PCSCORES'], "PCSCORES")

        # add the page to the notebook
        nb.AddPage(self.PCPageGraphic, "PCSCORES")
    ...
    def ReRead(self, pcfileName):
        self.OpenPC(pcfileName)
        nbpc = self.pc['PCSCORES'].shape[0]
        if nbpc != self.nbpc:
            txt = 'WARNING : nb pc scores : %s --> %s' % (self.nbpc, nbpc)
            self.writeSB(txt, "red", 10, 1)
            self.nbRun = 1
            return
        if str(self.misc['INSTRUMENT']) != self.instrument:
            ...
            return
        self.nbRun += 1
        self.PCPageGraphic.RePlot(self.pc['PCSCORES'], self.nbRun)
...
if __name__ == "__main__":
    ...
    fileName = p.config.ENV['RTTOV_GUI_PREFIX'] + '/rttov_tests/pc_1.h5'
    ...
    sv = pcView(None, fileName)
    fileName = p.config.ENV['RTTOV_GUI_PREFIX'] + '/rttov_tests/pc_4.h5'
    sv.ReRead(fileName)
    fileName = p.config.ENV['RTTOV_GUI_PREFIX'] + '/rttov_tests/pc_1.h5'
    sv.ReRead(fileName)
    ex.MainLoop()
