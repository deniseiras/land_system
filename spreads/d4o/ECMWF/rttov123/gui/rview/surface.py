...
class SurfaceView(util.GenericView):
    ...
    def __init__(self, parent, project):
        util.GenericView.__init__(self, parent,  "Surface")
        ...
        self.SetSizer(sizer)
        self.CreateMenuBar()
        self.SetSize((1020, 780))
        self.SetMinSize((1020, 780))
    ...
    def ShowSurfaceParameters(self):
        ...
        if wx.VERSION[0] <= 3:
            self.ct["DATE"] = wx.DatePickerCtrl(self.panel1)
        else:
        ...
        for label in self.myProfile.sskin_list[:-1]:
            line += 1
            self.gbs.Add(wx.StaticText(self.panel1, -1, label),
                         pos=(line, 2),
                         flag=wx.LEFT | wx.ALIGN_RIGHT, border=4)
    ...
    def GetProfileValues(self, aProfile=None):
        ...
        try:
            ...
            self.myProfile['TIME'][0] = int(self.ct['TIME'].GetValue()[0:2])
            self.myProfile['TIME'][1] = int(self.ct['TIME'].GetValue()[3:5])
            self.myProfile['TIME'][2] = int(self.ct['TIME'].GetValue()[6:8])
            self.myProfile['ID'] = str(self.ct['ID'].GetValue())
            for param in self.myProfile.profileParamList[3:]:
                if (param == 'IDG' or param == 'ICE_SCHEME'):
    ...
    def SetProfileValues(self, aProfile=None):
        ...
        self.ct['TIME'].SetValue(
            str(self.myProfile['TIME'][0]) + ':' +
            str(self.myProfile['TIME'][1]) +
            ':' + str(self.myProfile['TIME'][2]))
        for param in self.myProfile.profileParamList[2:]:
            if (param == 'ID'):
...
if __name__ == "__main__":
    ...
    p.openSurface(prefix + "/rttov_tests/surface.h5")
    ex = wx.App()
    sv = SurfaceView(None, p)
