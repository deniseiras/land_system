...
!INTF_END
  ...
  INTEGER(jpim)              :: i, ichan, igas, gas_pos, gas_id, ncoef
...
TRY
  ...
  IF (ASSOCIATED(coef1%ref_prfl_p)) THEN
    ...
    ALLOCATE(coef2%bkg_prfl_mr(coef2%fmv_lvl(gas_id_mixed), coef2%fmv_gas), stat=err)
  ...
  IF (ASSOCIATED(coef1%lim_prfl_p)) THEN
    ...
    ALLOCATE(coef2%env_prfl_tmax(coef2%fmv_lvl(gas_id_mixed)), stat=err)
    THROWM(err.NE.0, 'allocation of env_prfl_tmax')
    coef2%env_prfl_tmax = coef1%env_prfl_tmax

    ALLOCATE(coef2%env_prfl_tmin(coef2%fmv_lvl(gas_id_mixed)), stat=err)
    THROWM(err.NE.0, 'allocation of env_prfl_tmin')
    coef2%env_prfl_tmin = coef1%env_prfl_tmin

    ALLOCATE(coef2%env_prfl_gmax(coef2%fmv_lvl(gas_id_mixed), coef2%fmv_gas), stat=err)
    THROWM(err.NE.0, 'allocation of env_prfl_gmax')
    coef2%env_prfl_gmax = coef1%env_prfl_gmax

    ALLOCATE(coef2%env_prfl_gmin(coef2%fmv_lvl(gas_id_mixed), coef2%fmv_gas), stat=err)
  ...
  IF (ASSOCIATED(coef1%thermal)) THEN
    ALLOCATE(coef2%thermal(coef2%fmv_chn), stat=err)
    THROWM(err.NE.0, 'allocation of thermal')
    DO ichan = 1, coef2%fmv_chn
      ALLOCATE (coef2%thermal(ichan)%gasarray(coef2%fmv_gas), stat=err)
      THROWM(err.NE.0, 'allocation of thermal(:)%gasarray')
      NULLIFY (coef2%thermal(ichan)%mixedgas)
      NULLIFY (coef2%thermal(ichan)%watervapour)
      NULLIFY (coef2%thermal(ichan)%ozone)
      NULLIFY (coef2%thermal(ichan)%wvcont)
      NULLIFY (coef2%thermal(ichan)%co2)
      NULLIFY (coef2%thermal(ichan)%n2o)
      NULLIFY (coef2%thermal(ichan)%co)
      NULLIFY (coef2%thermal(ichan)%ch4)
      NULLIFY (coef2%thermal(ichan)%so2)
      ...
      DO igas = 1, coef1%fmv_gas
        ...
        IF (ASSOCIATED(coef1%thermal(channels(ichan))%gasarray(gas_pos)%coef)) THEN
          ALLOCATE(coef2%thermal(ichan)%gasarray(gas_pos)%coef(ncoef, coef2%nlayers), stat=err)
          THROWM(err.NE.0, 'allocation of thermal(:)%gasarray(:)%coef')
          coef2%thermal(ichan)%gasarray(gas_pos)%coef = &
            coef1%thermal(channels(ichan))%gasarray(gas_pos)%coef

          CALL set_pointers(coef2%thermal(ichan), gas_pos, gas_id)
        ENDIF
      ENDDO
  ...
  IF (ASSOCIATED(coef1%solar)) THEN
    ALLOCATE (coef2%solar(coef2%fmv_chn), stat=err)
    THROWM(err.NE.0, 'allocation of solar')
    DO ichan = 1, coef2%fmv_chn
      ALLOCATE (coef2%solar(ichan)%gasarray(coef2%fmv_gas), stat=err)
      THROWM(err.NE.0, 'allocation of solar(:)%gasarray')
      NULLIFY (coef2%solar(ichan)%mixedgas)
      NULLIFY (coef2%solar(ichan)%watervapour)
      NULLIFY (coef2%solar(ichan)%ozone)
      NULLIFY (coef2%solar(ichan)%wvcont)
      NULLIFY (coef2%solar(ichan)%co2)
      NULLIFY (coef2%solar(ichan)%n2o)
      NULLIFY (coef2%solar(ichan)%co)
      NULLIFY (coef2%solar(ichan)%ch4)
      NULLIFY (coef2%solar(ichan)%so2)
      ...
      DO igas = 1, coef1%fmv_gas
        ...
        IF (ASSOCIATED(coef1%solar(channels(ichan))%gasarray(gas_pos)%coef)) THEN
          ALLOCATE(coef2%solar(ichan)%gasarray(gas_pos)%coef(ncoef, coef2%nlayers), stat=err)
          THROWM(err.NE.0, 'allocation of solar(:)%gasarray(:)%coef')
          coef2%solar(ichan)%gasarray(gas_pos)%coef = &
            coef1%solar(channels(ichan))%gasarray(gas_pos)%coef

          CALL set_pointers(coef2%solar(ichan), gas_pos, gas_id)
        ENDIF
      ENDDO
    ENDDO
  ENDIF
