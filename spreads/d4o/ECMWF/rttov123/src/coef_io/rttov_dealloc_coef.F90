...
!- End of header --------------------------------------------------------
  ...
  IF (ASSOCIATED(coef%thermal)) THEN
    DO ichan = 1, SIZE(coef%thermal)
      IF (ASSOCIATED(coef%thermal(ichan)%gasarray)) THEN
        DO igas = 1, coef%fmv_gas
          IF (ASSOCIATED(coef%thermal(ichan)%gasarray(igas)%coef)) &
            DEALLOCATE(coef%thermal(ichan)%gasarray(igas)%coef, STAT = err)
            THROW(err.NE.0)
        ENDDO
        DEALLOCATE(coef%thermal(ichan)%gasarray, STAT = err)
        THROW(err.NE.0)
      ENDIF
    ENDDO
    DEALLOCATE(coef%thermal, STAT = err)
    THROW(err.NE.0)
  ENDIF

  IF (coef%solarcoef) THEN
    IF (ASSOCIATED(coef%solar)) THEN
      DO ichan = 1, SIZE(coef%solar)
        IF (ASSOCIATED(coef%solar(ichan)%gasarray)) THEN
          DO igas = 1, coef%fmv_gas
            IF (ASSOCIATED(coef%solar(ichan)%gasarray(igas)%coef)) &
              DEALLOCATE(coef%solar(ichan)%gasarray(igas)%coef, STAT = err)
              THROW(err.NE.0)
          ENDDO
          DEALLOCATE(coef%solar(ichan)%gasarray, STAT = err)
          THROW(err.NE.0)
        ENDIF
      ENDDO
      DEALLOCATE (coef%solar, STAT = err)
      THROW(err.NE.0)
    ENDIF
  ...
  IF (ASSOCIATED(coef%so2star)) DEALLOCATE (coef%so2star, STAT = err)
  THROW(err.NE.0)
