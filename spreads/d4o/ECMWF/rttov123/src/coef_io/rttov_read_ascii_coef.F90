...
! Open the file and skip the header
        ...
        IF (coef%fmv_coe(n) > 0_jpim) THEN
          CALL read_ascii_fast_coef(err, coef%thermal, n, coef%fmv_gas_id(n), coef%fmv_coe(n), lbl_mode = lbl1)
...
! read dummy string of gas name
        ...
        ! test existence of gas
        IF (coef%fmv_coe(n) > 0_jpim) THEN
          CALL read_ascii_fast_coef(err, coef%solar, n, coef%fmv_gas_id(n), coef%fmv_coe(n), lbl_mode = lbl1)
          THROWM(err.NE.0,"error occurred inside read_ascii_fast_coef while reading gas "//gas_name(n))
        ENDIF
      ENDDO
...
CONTAINS
  SUBROUTINE read_ascii_fast_coef(err, fast_coef, gas_pos, gas_id, ncoef, lbl_mode)
    ...
    LOGICAL(jplm),         INTENT(IN), OPTIONAL :: lbl_mode

    REAL(jprb), ALLOCATABLE :: coef_temp(:,:,:)
    INTEGER(jpim) :: i, j, k, chn
    LOGICAL(jplm) :: lbl_mode1 = .FALSE.

    IF (PRESENT(lbl_mode)) lbl_mode1 = lbl_mode
...
!but remain same on disk (for pre-RTTOV 11.3 coefs)
    ...
    DO i = 1, coef%fmv_chn
      ...
      IF (lbl_mode1 .OR. ANY(coef_temp(:, :, chn) /= 0._jprb)) THEN
