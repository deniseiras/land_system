...
#else
    CALL rttov_hdf_save(err, file_coef1, "/COEF", CREATE=.TRUE., COEF=coefs%coef, COMPRESS=compress, FORCE_DOUBLE=force_double)
    THROW(err.NE.0)
#endif
  ELSE
    err = errorstatus_fatal
    ...
    IF (PRESENT(instrument) .OR. PRESENT(file_scaer) .OR. PRESENT(file_id_scaer)) THEN
      ...
      ELSE IF (rttov_cmpuc(form_scaer1,"hdf5")) THEN
        ...
        CALL rttov_hdf_save(err, file_scaer, "/SCAER", CREATE=create, &
                    ...
                    OPTP=coefs%optp, COMPRESS=compress, FORCE_DOUBLE=force_double)
        THROW(err.NE.0)
#endif
      ELSE
        err = errorstatus_fatal
