...
!     INTEGER(KIND=jpis), POINTER :: latitude(:,:), lat_valr(:), lon_valr(:)
!     REAL(KIND=jprm)             :: lat_sfac, lon_sfac
...
#include "rttov_hdf_load.interface"
    ...
    IF (SIZE(emis_cov,1) .NE. numwave-1) THEN
      DEALLOCATE(emis_cov)
      err = errorstatus_fatal
    ...
    atlas%cov_emis(:,1:numwave-1) = TRANSPOSE(emis_cov)
    atlas%cov_emis(:,numwave) = atlas%cov_emis(:,numwave-1)
    DEALLOCATE(emis_cov)
    NULLIFY(emis_cov)
...
!     CALL rttov_hdf_load(err, fn, "/EMIS", SNAME="LATITUDE", PIS2=latitude)
!     THROWM(ERR.NE.0,"Cannot get latitude values from "//TRIM(fn))
!
!     CALL rttov_hdf_load(err, fn, "/EMIS", SNAME="LATITUDE_SFAC", RM0=lat_sfac)
!     THROWM(ERR.NE.0,"Cannot get latitude scale factor from "//TRIM(fn))
!     CALL rttov_hdf_load(err, fn, "/EMIS", SNAME="LATITUDE_VALR", PIS1=lat_valr)
!     THROWM(ERR.NE.0,"Cannot get latitude value range from "//TRIM(fn))
!
!     CALL rttov_hdf_load(err, fn, "/EMIS", SNAME="LONGITUDE_SFAC", RM0=lon_sfac)
!     THROWM(ERR.NE.0,"Cannot get longitude scale factor from "//TRIM(fn))
!     CALL rttov_hdf_load(err, fn, "/EMIS", SNAME="LONGITUDE_VALR", PIS1=lon_valr)
!     THROWM(ERR.NE.0,"Cannot get longitude value range from "//TRIM(fn))
!
!     atlas%cov_emis_gridres = NINT(ABS(latitude(1,1) - latitude(2,1)) * lat_sfac * 1000._jprb)
!     atlas%cov_emis_ygrid1 = NINT(MAXVAL(lat_valr) * lat_sfac * 1000._jprb)
!     atlas%cov_emis_xgrid1 = NINT(MINVAL(lon_valr) * lon_sfac * 1000._jprb)
!
!     DEALLOCATE(latitude, lat_valr, lon_valr)
...
!     INTEGER(KIND=jpis), ALLOCATABLE :: latitude(:,:)
! 
!     REAL(KIND=jprb)     :: lat_sfac, lon_sfac
!     INTEGER(KIND=jpim)  :: lat_valr, lon_valr
    ...
    ALLOCATE(emis_cov(numwave-1,atlas%cv_pack), atlas%cov_emis(atlas%cv_pack,numwave))
...
!     ALLOCATE(latitude(atlas%cv_lats,atlas%cv_lons))
! 
!     errstat = nf_inq_varid (ncid, 'latitude', varid)
!     errstat = nf_inq_varid (ncid,'latitude',varid)
!     errstat = nf_get_var_real(ncid,varid,latitude)
!     errstat = nf_get_att_real(ncid,varid,'scale_factor',lat_sfac)
!     errstat = nf_get_att_real(ncid,varid,'valid_range',lat_valr)
! 
!     errstat = nf_inq_varid (ncid,'longitude',varid)
!     errstat = nf_get_att_real(ncid,varid,'scale_factor',lon_sfac)
!     errstat = nf_get_att_real(ncid,varid,'valid_range',lon_valr)
! 
!     atlas%cov_emis_gridres = NINT(ABS(latitude(1,1) - latitude(2,1)) * lat_sfac * 1000._jprb)
!     atlas%cov_emis_ygrid1 = NINT(MAXVAL(lat_valr) * lat_sfac * 1000._jprb)
!     atlas%cov_emis_xgrid1 = NINT(MINVAL(lon_valr) * lon_sfac * 1000._jprb)
