...
!------------------------------------------
  ...
  SUBROUTINE rttov_camel_clim_init(  &
    ...
    !----------------------------------------------------------------------------
    fn = TRIM(path)//'CAMEL_coef_climatology_'//cmonth//'Month_'//TRIM(prd_version)
#ifdef _RTTOV_HDF
    fn = TRIM(fn)//'.H5'
#else
    fn = TRIM(fn)//'.nc'
    ...
    IF (atlas%std_init) THEN
      fn = TRIM(path)//'CAMEL_hsremis_covmat_deg0.25_2000-2016_month'//cmonth//'_'//TRIM(prd_version)//'_mask'
#ifdef _RTTOV_HDF
      fn = TRIM(fn)//'.H5'
#else
      fn = TRIM(fn)//'.nc'
      ...
      ELSE
        err = errorstatus_fatal
        THROWM(err .NE. errorstatus_success, 'CAMEL covariances file not found: '//TRIM(fn)//TRIM(errmsg))
      ENDIF
