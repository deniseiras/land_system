...
!INTF_OFF
  ...
  USE mod_uwiremis_atlas, ONLY : rttov_uwiremis
  USE mod_camel_atlas, ONLY : rttov_camel
...
!----------------------------------------------------------------------------
  ...
  htfrtc = ASSOCIATED(coefs%coef_htfrtc%sensor_freq)

  IF (htfrtc) THEN
    id_platform = 0  ! HTFRTC coefs don't contain these IDs
    id_sat      = 0
    id_inst     = 0
    nchan_coef  = coefs%coef_htfrtc%n_ch
    nchanprof   = nchan_coef * SIZE(profiles)
    chan_wvn    => coefs%coef_htfrtc%sensor_freq
    sensor_mw   = .FALSE.
  ELSE
