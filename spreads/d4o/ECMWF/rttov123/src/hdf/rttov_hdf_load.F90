...
!! @param         optp          RTTOV cloud/aerosol optical property structure, optional
...
TRY
      ...
      IF( PRESENT(SCAERCOEF) .AND. PRESENT(OPTP) .AND. PRESENT(COEF)) THEN
      
        CALL RTTOV_HDF_SCAERCOEF_RH(SCAERCOEF, OPTP, COEF, G_ID,ERR)
        THROWM(ERR.NE.0,"CANNOT READ AEROSOLS COEF IN FILE "//TRIM(F))
           
      ELSEIF( PRESENT(SCCLDCOEF) .AND. PRESENT(OPTP) .AND. PRESENT(COEF)) THEN
      
        CALL RTTOV_HDF_SCCLDCOEF_RH(SCCLDCOEF, OPTP, COEF, G_ID,ERR)
