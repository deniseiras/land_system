...
!! @param[in]     optp          RTTOV cloud/aerosol optical property structure, optional
...
TRY
      ...
      IF( PRESENT(SCCLDCOEF) .AND. PRESENT(OPTP)) THEN
        ...
        CALL RTTOV_HDF_SCCLDCOEF_WH(SCCLDCOEF, OPTP, G_ID,ERR, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
      ...
      IF( PRESENT(SCAERCOEF) .AND. PRESENT(OPTP)) THEN
        ...
        CALL RTTOV_HDF_SCAERCOEF_WH(SCAERCOEF, OPTP, G_ID,ERR, COMPRESS=COMPRESS, FORCE_DOUBLE=FORCE_DOUBLE)
