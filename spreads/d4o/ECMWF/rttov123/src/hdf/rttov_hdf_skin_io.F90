...
call h5lexists_f( lun, sname, lext, err )
if( lext ) then
call read_array_hdf(lun,sname,err,r0=x%SPECULARITY)
THROWM(err.ne.0,"CANNOT READ "//trim(sname))
endif
...
sname='SPECULARITY'
call write_array_hdf(lun,sname,&
  & 'Mix of specular/Lambertian for do_lambertian option (0-1, 0=Lambertian/1=specular)',&
  & err,r0=x%SPECULARITY , units = '0:1'  )
THROWM(err.ne.0,"CANNOT WRITE "//trim(sname))
