  ...
  IF (asw == 1_jpim) THEN
    ...
    DO i = 1, nchanprof
      IF (chanflag(i)) THEN
        IF (dosolar) THEN
          naz = dom_nstr - 1
          ALLOCATE(dom_state(i)%nazloops(0:nstreams),            &
                   dom_state(i)%z(dom_nstr,0:nlayers,0:1,0:naz), &
                   STAT = err)
        ELSE ! dothermal
          naz = 0
          ALLOCATE(dom_state(i)%y0(dom_nstr,nlayers,0:nstreams), &
                   dom_state(i)%y1(dom_nstr,nlayers,0:nstreams), &
                   STAT = err)
        ENDIF
        THROWM(err.NE.0,"allocation of thermal/solar dom_state")

        ALLOCATE(dom_state(i)%thisrad(0:nstreams),       &
                 dom_state(i)%radsurfup(0:nstreams),     &  ! also 0:naz if surface not Lambertian
                 dom_state(i)%radsurfup_sum(0:nstreams), &  ! also 0:naz if surface not Lambertian
                 dom_state(i)%eval(dom_nstr/2,0:nlayers,0:1,0:naz),         &
                 dom_state(i)%xp(dom_nstr/2,dom_nstr/2,nlayers,0:1,0:naz),  &
                 dom_state(i)%x(dom_nstr*maxnlayers,0:naz,0:nstreams),      &
                 STAT = err)
        THROWM(err.NE.0,"allocation of dom_state")
      ENDIF
    ENDDO
