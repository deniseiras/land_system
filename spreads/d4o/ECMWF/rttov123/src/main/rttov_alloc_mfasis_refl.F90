...
#include "rttov_errorreport.interface"
  ...
  INTEGER(KIND=jpim) :: i, ist
!- End of header --------------------------------------------------------
  TRY

  IF (asw == 1_jpim) THEN
    ALLOCATE(mfasis_refl(0:nstreams,nchanprof), STAT=err)
    THROWM(err.NE.0,"allocation of mfasis_refl")
    DO i = 1, nchanprof
      DO ist = 0, nstreams
        NULLIFY(mfasis_refl(ist,i)%refl_lin_coef)
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (asw == 0_jpim) THEN
    DO i = 1, nchanprof
      DO ist = 0, nstreams
        IF (ASSOCIATED(mfasis_refl(ist,i)%refl_lin_coef)) THEN
          DEALLOCATE(mfasis_refl(ist,i)%refl_lin_coef, STAT = err)
