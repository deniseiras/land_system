...
!- End of header --------------------------------------------------------
  ...
  IF (asw == 1) THEN
    ...
    IF (htfrtc) THEN
      ! Arrays for HTFRTC
      ALLOCATE (pccomp%pcscores(npcscores), pccomp%clear_pcscores(npcscores), &
                pccomp%total_pcscores(npcscores), STAT = err)
...
CONTAINS
  SUBROUTINE nullify_struct(pccomp)
    TYPE(rttov_pccomp), INTENT(INOUT) :: pccomp
    NULLIFY(pccomp%pcscores,          &
            pccomp%clear_pcscores,    &
