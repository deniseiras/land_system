...
!!
!!   The default value of gas_units is set here to ppmv over moist air.
...
!INTF_OFF
#include "throw.h"
  USE rttov_const, ONLY : gas_unit_ppmv
!INTF_ON
...
!- End of header --------------------------------------------------------
  ...
  IF (opts%rt_ir%addclouds .AND. .NOT. opts%rt_ir%user_cld_opt_param) THEN
    IF (.NOT. PRESENT(coefs) .AND. (asw .EQ. 1_jpim)) THEN
  ...
  IF (asw == 1) THEN
    IF (opts%rt_ir%addclouds .AND. .NOT. opts%rt_ir%user_cld_opt_param) &
    ...
    DO j = 1, nprofiles
      profiles(j)%gas_units = gas_unit_ppmv
