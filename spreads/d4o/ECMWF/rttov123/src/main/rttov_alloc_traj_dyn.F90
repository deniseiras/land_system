...
!!   each RTTOV model. Note that this subroutine takes nstreams, the
!!   number of cloudy streams (columns) in IR scattering simulations,
!!   as an argument. This means that in general it cannot be allocated
!!   outside of RTTOV as nstreams is determined dynamically at run-time.
!!   However for non-cloudy-IR simulations it would be possible to allocate
!!   this structure with nstreams=0 before calling rttov_direct and hence to
