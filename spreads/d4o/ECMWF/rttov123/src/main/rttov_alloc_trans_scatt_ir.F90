...
!!                                          an "nstreams" - cloud columns - dimension), optional
!! @param[in]     nstreams               number of cloud streams (columns) as calculated by rttov_cldstr, optional
...
#include "rttov_init_trans_scatt_ir.interface"
  ...
  LOGICAL(KIND=jplm) :: do_dom, do_chou, do_dom_ir, do_dom_vis, do_single_scatt, do_mfasis
!- End of header --------------------------------------------------------
  TRY
  ...
  IF (asw == 1) THEN
    ...
    IF (dynamic1 .AND. (.NOT. PRESENT(nstreams) .OR. .NOT. PRESENT(dom_nstr) .OR. &
                        .NOT. PRESENT(thermal) .OR. .NOT. PRESENT(solar))) THEN
      err = errorstatus_fatal
      THROWM(err.NE.0, "nstreams, dom_nstr, thermal and solar arguments required")
    ENDIF
    ...
    ! Layer quantities take one of two values in each stream: non-cloudy and cloudy.
    ! 0:0 if aerosol-only, 1:1 if cloud-only, 0:1 for aer+cld
    ! NB in cloud-only case the clear (non-cloudy) stream values are all zero for scattering particles

    IF (dynamic1 .AND. .NOT. do_mfasis) THEN
      ALLOCATE (trans_scatt_ir%opdpac(nlevels, 0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "mem allocation error")

      IF (opts%rt_ir%addsolar) THEN
        ALLOCATE (trans_scatt_ir%opdpacsun(nlevels, 0:nstreams, nchanprof), STAT=err)
        THROWM(err.NE.0, "mem allocation error")
      ENDIF
    ...
    ELSE
      ...
      IF (opts%rt_ir%addaerosl) THEN
        IF (opts%rt_ir%user_aer_opt_param) THEN
          naer = 1
        ELSE
          naer = coefs%coef_scatt_ir%fmv_aer_comp
        ENDIF
      ENDIF
      ...
      IF (opts%rt_ir%addclouds) THEN
        IF (opts%rt_ir%user_cld_opt_param) THEN
          ncld = 1
        ELSE
          ncld = coefs%coef_scatt_ir%fmv_wcl_comp + 1
        ENDIF
      ENDIF
...
CONTAINS
  ...
  SUBROUTINE nullify_struct()
    ...
    NULLIFY(trans_scatt_ir%cld)
    NULLIFY(trans_scatt_ir%phasefn)
    NULLIFY(trans_scatt_ir%ssa_solar)
    NULLIFY(trans_scatt_ir%ssa_thermal)
    NULLIFY(trans_scatt_ir%layerod_solar)
    NULLIFY(trans_scatt_ir%layerod_thermal)
    NULLIFY(trans_scatt_ir%phup)
    NULLIFY(trans_scatt_ir%phdo)
    NULLIFY(trans_scatt_ir%opdpabs)
    NULLIFY(trans_scatt_ir%opdpsca)
    NULLIFY(trans_scatt_ir%opdpaclsun)
    NULLIFY(trans_scatt_ir%opdpacsun)
    NULLIFY(trans_scatt_ir%opdpacl)
    NULLIFY(trans_scatt_ir%opdpac)
    NULLIFY(trans_scatt_ir%opdpext)
