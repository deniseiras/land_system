...
!INTF_OFF
  USE rttov_const, ONLY : ir_scatt_dom, vis_scatt_dom, vis_scatt_single
!INTF_ON
  IMPLICIT NONE
...
#include "rttov_init_transmission_aux.interface"
  ...
  LOGICAL(KIND=jplm) :: init1, direct1, do_scatt, do_dom_ir, do_dom_vis, do_single_scatt
!- End of header --------------------------------------------------------
  ...
  IF (asw == 1) THEN
    ...
    IF (direct1 .AND. .NOT. do_dom_ir) THEN
      ALLOCATE (transmission_aux%fac1(nlevels, 0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % fac1")
      ALLOCATE (transmission_aux%surf_fac(0:nstreams, nchanprof), STAT=err)
    ...
    ELSE
      ALLOCATE (transmission_aux%thermal_path1%tau_level(nlevels, 0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % tau_level")
      ALLOCATE (transmission_aux%thermal_path1%od_singlelayer(0:su, nlayers, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % od_singlelayer")
      ALLOCATE (transmission_aux%thermal_path1%od_sfrac(0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % od_sfrac")
      ALLOCATE (transmission_aux%thermal_path1%tau_surf(0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % tau_surf")
      ALLOCATE (transmission_aux%thermal_path1%tau_surf_ac(0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % tau_surf_ac")
    ENDIF

    IF (direct1 .AND. .NOT. do_dom_ir) THEN
      ALLOCATE (transmission_aux%thermal_path1%fac2(nlevels, 0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % fac2")
      ALLOCATE (transmission_aux%thermal_path1%tau_level_r(nlevels, 0:nstreams, nchanprof), STAT=err)
      ...
      ALLOCATE (transmission_aux%thermal_path1%tau_surf_r(0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % tau_surf_r")
      ALLOCATE (transmission_aux%thermal_path1%od_sfrac_r(0:nstreams, nchanprof), STAT=err)
    ...
    IF (opts%rt_all%do_lambertian .AND. .NOT. do_dom_ir) THEN
      ALLOCATE (transmission_aux%thermal_path1%tau_level_p(nlevels, 0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % tau_level_p")
      ALLOCATE (transmission_aux%thermal_path1%tau_surf_p(0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % tau_surf_p")
      IF (direct1) THEN
        ALLOCATE (transmission_aux%thermal_path1%tau_level_p_r(nlevels, 0:nstreams, nchanprof), STAT=err)
        THROWM(err.NE.0, "allocation of transmission_aux % thermal_path1 % tau_level_p_r")
        ALLOCATE (transmission_aux%thermal_path1%tau_surf_p_r(0:nstreams, nchanprof), STAT=err)
    ...
    IF (opts%rt_ir%addsolar) THEN
      ...
      ALLOCATE (transmission_aux%solar_path2%tau_level(nlevels, 0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % solar_path2 % tau_level")
      ALLOCATE (transmission_aux%solar_path2%tau_surf(0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % solar_path2 % tau_surf")
      ALLOCATE (transmission_aux%solar_path2%tau_surf_ac(0:nstreams, nchanprof), STAT=err)
      ...
      IF (do_single_scatt) THEN
        ...
        ALLOCATE (transmission_aux%solar_path2%od_sfrac(0:nstreams, nchanprof), STAT=err)
        THROWM(err.NE.0, "allocation of transmission_aux % solar_path2 % od_sfrac")
        ALLOCATE (transmission_aux%solar_path2%od_frac_ac(0:nstreams, nchanprof), STAT=err)
      ...
      ALLOCATE (transmission_aux%solar_path1%tau_level(nlevels, 0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % solar_path1 % tau_level")
      ALLOCATE (transmission_aux%solar_path1%tau_surf(0:nstreams, nchanprof), STAT=err)
      THROWM(err.NE.0, "allocation of transmission_aux % solar_path1 % tau_surf")

      IF (do_scatt) THEN
        ALLOCATE (transmission_aux%solar_path1%tau_surf_ac(0:nstreams, nchanprof), STAT=err)
      ...
      IF (do_single_scatt) THEN
        ...
        ALLOCATE (transmission_aux%solar_path1%od_sfrac(0:nstreams, nchanprof), STAT=err)
      ...
      IF (direct1 .AND. do_scatt .AND. .NOT. do_dom_vis) THEN
        ALLOCATE (transmission_aux%solar_path1%tau_level_r(nlevels, 0:nstreams, nchanprof), STAT=err)
      ...
      IF (direct1 .AND. do_single_scatt) THEN
        ALLOCATE (transmission_aux%solar_path1%fac2(nlevels, 0:nstreams, nchanprof), STAT=err)
        THROWM(err.NE.0, "allocation of transmission_aux % solar_path1 % fac2")
        ALLOCATE (transmission_aux%solar_path1%tau_surf_r(0:nstreams, nchanprof), STAT=err)
