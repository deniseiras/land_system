! Description:
!> @file
!!   Compute optical parameters for ice cloud from Baran parameterisation
!
!> @brief
!!   Compute optical parameters for ice cloud from Baran parameterisation
!!
!! @param[in]     optp             optical property coefficient structure
...
!    ENDIF
    ...
    LIWC = LOG10(iwc)
    iwn = optp%optpiclbaran%iwn(ichn)
    jwn = optp%optpiclbaran%jwn(ichn)
    dx_dwn = optp%optpiclbaran%dx_dwn(ichn)
