...
!!   Radiances are not calculated for channels at wavelengths below 3µm  (i.e.
...
!INTF_OFF
  ...
  USE rttov_const, ONLY : tcosmic, sensor_id_ir, sensor_id_hi, sensor_id_mw, sensor_id_po
...
! Calculate effective temperatures, store in auxrad
    ...
    IF (addcosmic) THEN
      ...
      IF (coef%ff_val_bc .AND. opts%rt_mw%apply_band_correction) THEN
