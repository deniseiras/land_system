! Description:
!> @file
!!   AD of calculation of surface BRDF values for solar-affected channels.
!
!> @brief
!!   AD of calculation of surface BRDF values for solar-affected channels.
!!
...
!! @param[in,out] reflectance_ad   BRDF increments
  ...
  DO j = 1, nchanprof
    ...
    IF (.NOT. calcrefl(j)) THEN
      IF (coef%ss_val_chn(chan) == 2) THEN
        reflectance_ad(j) = reflectance_ad(j) + diffuse_refl_ad(j) * pi
    ...
    IF (profiles(prof)%skin%surftype == surftype_sea) THEN
      ...
      ! The output reflectance_ad should not be scaled by cos(sunzenangle)
      ...
      IF (profiles(prof)%s2m%u**2 + profiles(prof)%s2m%v**2 > min_windsp**2) THEN
        tmp_refl_ad = reflectance_ad(j) / refl_norm(j)
      ELSE
        tmp_refl_ad = reflectance_ad(j)
      ENDIF
      reflectance_ad(j) = 0._jprb
    ...
    ELSE
      ...
      ELSEIF (coef%ss_val_chn(chan) == 1) THEN
        emissivity_ad(j)%emis_out = emissivity_ad(j)%emis_out - reflectance_ad(j) * pi_r
        reflectance_ad(j) = 0._jprb
      ENDIF
