! Description:
!> @file
!!   Jacobian of calculation of surface BRDF values for solar-affected channels.
!
!> @brief
!!   Jacobian of calculation of surface BRDF values for solar-affected channels.
!!
...
!! @param[in,out] reflectance_k    BRDF increments
  ...
  DO j = 1, nchanprof
    ...
    IF (.NOT. calcrefl(j)) THEN
      IF (coef%ss_val_chn(chan) == 2) THEN
        reflectance_k(j) = reflectance_k(j) + diffuse_refl_k(j) * pi
