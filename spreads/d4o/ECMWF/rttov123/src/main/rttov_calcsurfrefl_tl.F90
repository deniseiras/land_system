! Description:
!> @file
!!   TL of calculation of surface BRDF values for solar-affected channels.
!
!> @brief
!!   TL of calculation of surface BRDF values for solar-affected channels.
!!
...
!! @param[in,out] reflectance_tl   BRDF perturbations
...
!- End of header --------------------------------------------------------
  ...
  DO j = 1, nchanprof
    ...
    IF (.NOT. calcrefl(j)) THEN
      IF (coef%ss_val_chn(chan) == 2) diffuse_refl_tl(j) = reflectance_tl(j) * pi
      CYCLE
    ENDIF
...
!         reflectance_tl(j) = reflectance_tl(j) + diffuse_refl_tl(j) * pi_r  ! Commented to avoid null op
      ENDIF

    ELSE

      IF (coef%ss_val_chn(chan) == 1) THEN
        reflectance_tl(j) = - emissivity_tl(j)%emis_out * pi_r
      ELSE
! User should supply TL, same as for ISEM
!         reflectance_tl(j) = 0._jprb
      ENDIF
