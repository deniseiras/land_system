...
!!   rttov_check_profiles and rttov_checkinput and/or
...
!- End of header --------------------------------------------------------
  ...
  IF (.NOT. (opts%rt_ir%addclouds .AND. opts%rt_ir%addsolar .AND. &
  ...
  IF (opts%rt_ir%ozone_data .AND. coefs%coef%nozone > 0) THEN
    IF (.NOT. ASSOCIATED(profiles(1)%o3)) THEN
      ...
      msg = 'Profiles structure not allocated for ozone, but ozone_data flag is true; '// &
            'opts%rt_ir%ozone_data must be true when calling rttov_alloc_prof'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
  ...
  IF (opts%rt_ir%co2_data .AND. coefs%coef%nco2 > 0) THEN
    IF (.NOT. ASSOCIATED(profiles(1)%co2)) THEN
      ...
      msg = 'Profiles structure not allocated for co2, but co2_data flag is true; '// &
            'opts%rt_ir%co2_data must be true when calling rttov_alloc_prof'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
  ...
  IF (opts%rt_ir%co_data .AND. coefs%coef%nco > 0) THEN
    IF (.NOT. ASSOCIATED(profiles(1)%co)) THEN
      ...
      msg = 'Profiles structure not allocated for co, but co_data flag is true; '// &
            'opts%rt_ir%co_data must be true when calling rttov_alloc_prof'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
  ...
  IF (opts%rt_ir%n2o_data .AND. coefs%coef%nn2o > 0) THEN
    IF (.NOT. ASSOCIATED(profiles(1)%n2o)) THEN
      ...
      msg = 'Profiles structure not allocated for n2o, but n2o_data flag is true; '// &
            'opts%rt_ir%n2o_data must be true when calling rttov_alloc_prof'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
  ...
  IF (opts%rt_ir%ch4_data .AND. coefs%coef%nch4 > 0) THEN
    IF (.NOT. ASSOCIATED(profiles(1)%ch4)) THEN
      ...
      msg = 'Profiles structure not allocated for ch4, but ch4_data flag is true; '// &
            'opts%rt_ir%ch4_data must be true when calling rttov_alloc_prof'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
  ...
  IF (opts%rt_ir%so2_data .AND. coefs%coef%nso2 > 0) THEN
    IF (.NOT. ASSOCIATED(profiles(1)%so2)) THEN
      ...
      msg = 'Profiles structure not allocated for so2, but so2_data flag is true; '// &
            'opts%rt_ir%so2_data must be true when calling rttov_alloc_prof'
      THROWM(err.NE.0, msg)
    ENDIF
  ENDIF
  ...
  IF (coefs%coef%id_sensor == sensor_id_ir .OR. coefs%coef%id_sensor == sensor_id_hi) THEN
    ...
    IF (opts%rt_ir%addclouds) THEN
      IF (opts%rt_ir%cldstr_threshold >= 1._jprb) THEN
        err = errorstatus_fatal
        msg = 'Error in specified cldstr_threshold, must be less than 1.'
        THROWM(err.NE.0, msg)
      ENDIF
      ...
      IF (opts%rt_ir%user_cld_opt_param .AND. opts%rt_ir%cldstr_simple) THEN
        err = errorstatus_fatal
        msg = 'Simple cloud stream approach cannot be used with explicit optical properties'
        THROWM(err.NE.0, msg)
      ENDIF
    ENDIF
      ...
      IF ((opts%rt_ir%addsolar .AND. opts%rt_ir%vis_scatt_model == vis_scatt_dom) .OR. &
        ...
        IF (opts%rt_ir%addaerosl) THEN
          IF (opts%rt_ir%user_aer_opt_param) THEN
            ...
            ENDIF
          ELSE
            IF (opts%rt_ir%dom_nstreams > coefs%coef_scatt_ir%fmv_aer_maxnmom) THEN
              err = errorstatus_fatal
        ...
        IF (opts%rt_ir%addclouds) THEN
          ...
          ELSE
            IF (ANY(profiles(:)%clw_scheme == clw_scheme_opac)) THEN
              IF (opts%rt_ir%dom_nstreams > coefs%coef_scatt_ir%fmv_wcl_maxnmom) THEN
                err = errorstatus_fatal
            ...
            IF (ANY(profiles(:)%clw_scheme == clw_scheme_deff)) THEN
              IF (opts%rt_ir%dom_nstreams > coefs%coef_scatt_ir%fmv_wcldeff_maxnmom) THEN
                err = errorstatus_fatal
                msg = 'DOM nstreams exceeds the number of Legendre coefficients '// &
              ...
              ENDIF
            ENDIF
            IF (ANY(profiles(:)%ice_scheme == ice_scheme_ssec)) THEN
              IF (opts%rt_ir%dom_nstreams > coefs%coef_scatt_ir%fmv_icl_maxnmom) THEN
                err = errorstatus_fatal
                msg = 'DOM nstreams exceeds the number of Legendre coefficients '// &
                      'for SSEC ice cloud data in the cloud optical parameter file'
                THROWM(err.NE.0, msg)
              ENDIF
            ENDIF
          ENDIF
    ...
    IF (opts%rt_ir%addsolar .AND. ANY(coefs%coef%ss_val_chn > 0)) THEN
      IF (opts%rt_ir%addaerosl) THEN
        IF (opts%rt_ir%user_aer_opt_param) THEN
          ...
          ENDIF
        ELSE
          IF (.NOT. ASSOCIATED(coefs%coef_scatt_ir%aer_pha_index)) THEN
            err = errorstatus_fatal
          ...
          DO i = 1, SIZE(chanprof)
            ...
            IF (coefs%coef%ss_val_chn(chan) > 0 .AND. &
              coefs%coef_scatt_ir%aer_pha_index(chan) == 0) THEN
              err = errorstatus_fatal
      ...
      IF (opts%rt_ir%addclouds) THEN
        ...
        ELSE
          IF (ANY(profiles(:)%clw_scheme == clw_scheme_opac)) THEN
            IF (.NOT. ASSOCIATED(coefs%coef_scatt_ir%wcl_pha_index)) THEN
              err = errorstatus_fatal
          ...
          ENDIF
          IF (ANY(profiles(:)%clw_scheme == clw_scheme_deff)) THEN
            IF (.NOT. ASSOCIATED(coefs%coef_scatt_ir%wcldeff_pha_index)) THEN
              err = errorstatus_fatal
          ...
          ENDIF
          IF (ANY(profiles(:)%ice_scheme == ice_scheme_ssec)) THEN
            IF (.NOT. ASSOCIATED(coefs%coef_scatt_ir%icl_pha_index)) THEN
              err = errorstatus_fatal
              msg = 'Cloud optical parameter file does not support solar simulations for SSEC ice cloud data'
              THROWM(err.NE.0, msg)
            ENDIF
          ENDIF
          ...
          DO i = 1, SIZE(chanprof)
            ...
            IF (coefs%coef%ss_val_chn(chan) > 0) THEN
              IF (ANY(profiles(:)%clw_scheme == clw_scheme_opac)) THEN
                IF (coefs%coef_scatt_ir%wcl_pha_index(chan) == 0) THEN
                  err = errorstatus_fatal
              ...
              ENDIF
              IF (ANY(profiles(:)%clw_scheme == clw_scheme_deff)) THEN
                IF (coefs%coef_scatt_ir%wcldeff_pha_index(chan) == 0) THEN
                  err = errorstatus_fatal
              ...
              ENDIF
              IF (ANY(profiles(:)%ice_scheme == ice_scheme_ssec)) THEN
                IF (coefs%coef_scatt_ir%icl_pha_index(chan) == 0) THEN
                  err = errorstatus_fatal
    ...
    ! NLTE - check for v11 coefs instead of v12
  ...
  IF (coefs%coef%pmc_shift .AND. .NOT. opts%rt_ir%co2_data) THEN
    err = errorstatus_fatal
  ...
  IF (opts%rt_ir%pc%addpc) THEN
    ...
    IF (opts%rt_ir%addaerosl .AND. coefs%coef_scatt_ir%fmv_aer_comp /= naer_opac) THEN
      err = errorstatus_fatal
      msg = 'PC-RTTOV: must be used with an RTTOV OPAC aerosol optical property file'
      THROWM(err.NE.0, msg)
    ENDIF

    IF (opts%rt_ir%pc%npcscores > coefs%coef_pccomp%fmv_pc_mnum) THEN
      err = errorstatus_fatal
      msg = 'PC-RTTOV: opts%rt_ir%pc%npcscores exceeds maximum for PC coef file'
      THROWM(err.NE.0, msg)
    ENDIF
