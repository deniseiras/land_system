...
!- End of header --------------------------------------------------------
  ...
  IF (docld .OR. doaer) THEN
    ...
    DO iprof = 1, nprofiles
      ...
      IF (docld .AND. opts%rt_ir%grid_box_avg_cloud) THEN
        DO lay = 1, nlayers
          IF (profiles(iprof)%cfrac(lay) > cfrac_min) THEN
            ! The value of profiles_int(iprof)%cloud required is that before the cfrac scaling hence it is multiplied by cfrac
            profiles_ad(iprof)%cfrac(lay) = profiles_ad(iprof)%cfrac(lay) - &
                SUM(profiles_int_ad(iprof)%cloud(:,lay) * profiles_int(iprof)%cloud(:,lay)) / profiles(iprof)%cfrac(lay)

            ! This must be done after the above
            profiles_int_ad(iprof)%cloud(:,lay) = profiles_int_ad(iprof)%cloud(:,lay) / profiles(iprof)%cfrac(lay)
          ELSE
            profiles_int_ad(iprof)%cloud(:,lay) = 0._jprb
          ENDIF
        ENDDO
      ENDIF
        ...
        IF (doaer) THEN
          DO lay = nlayers, 1, -1
            cldaerfactor_ad(lay) = cldaerfactor_ad(lay) + SUM(profiles_int_ad(iprof)%aerosols(:,lay) * &
                profiles(iprof)%aerosols(:,lay) / coefs%coef_scatt_ir%aer_mmr2nd(:))
            profiles_ad(iprof)%aerosols(:,lay) = profiles_ad(iprof)%aerosols(:,lay) + &
                profiles_int_ad(iprof)%aerosols(:,lay) *  cldaerfactor(lay) / coefs%coef_scatt_ir%aer_mmr2nd(:)
          ENDDO
        ENDIF
