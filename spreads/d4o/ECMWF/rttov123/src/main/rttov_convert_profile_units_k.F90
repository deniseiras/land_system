  ...
  IF (docld .OR. doaer) THEN
    ...
    DO ichan = 1, nchanprof
      ...
      IF (docld .AND. opts%rt_ir%grid_box_avg_cloud) THEN
        DO lay = 1, nlayers
          IF (profiles(prof)%cfrac(lay) > cfrac_min) THEN
            ! The value of profiles_int(prof)%cloud required is that before the cfrac scaling hence it is multiplied by cfrac
            profiles_k(ichan)%cfrac(lay) = profiles_k(ichan)%cfrac(lay) - &
                SUM(profiles_int_k(ichan)%cloud(:,lay) * profiles_int(prof)%cloud(:,lay)) / profiles(prof)%cfrac(lay)

            ! This must be done after the above
            profiles_int_k(ichan)%cloud(:,lay) = profiles_int_k(ichan)%cloud(:,lay) / profiles(prof)%cfrac(lay)
          ELSE
            profiles_int_k(ichan)%cloud(:,lay) = 0._jprb
          ENDIF
        ENDDO
      ENDIF
        ...
        IF (doaer) THEN
          DO lay = nlayers, 1, -1
            cldaerfactor_k(lay) = cldaerfactor_k(lay) + SUM(profiles_int_k(ichan)%aerosols(:,lay) * &
                profiles(prof)%aerosols(:,lay) / coefs%coef_scatt_ir%aer_mmr2nd(:))
            profiles_k(ichan)%aerosols(:,lay) = profiles_k(ichan)%aerosols(:,lay) + &
                profiles_int_k(ichan)%aerosols(:,lay) *  cldaerfactor(lay) / coefs%coef_scatt_ir%aer_mmr2nd(:)
          ENDDO
        ENDIF
