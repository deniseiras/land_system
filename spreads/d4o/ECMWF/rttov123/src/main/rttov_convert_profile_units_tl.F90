          ...
          DO lay = 1, nlayers
            profiles_int_tl(iprof)%aerosols(:,lay) = &
                (profiles_tl(iprof)%aerosols(:,lay) * cldaerfactor(lay) + &
                 profiles(iprof)%aerosols(:,lay) * cldaerfactor_tl(lay)) / coefs%coef_scatt_ir%aer_mmr2nd(:)
          ENDDO
        ENDIF
      ELSE
      ...
      IF (docld .AND. opts%rt_ir%grid_box_avg_cloud) THEN
        DO lay = 1, nlayers
          IF (profiles(iprof)%cfrac(lay) > cfrac_min) THEN
            ...
            profiles_int_tl(iprof)%cloud(:,lay) = profiles_int_tl(iprof)%cloud(:,lay) / profiles(iprof)%cfrac(lay) - &
                profiles_tl(iprof)%cfrac(lay) * profiles_int(iprof)%cloud(:,lay) / profiles(iprof)%cfrac(lay)
          ELSE
            profiles_int_tl(iprof)%cloud(:,lay) = 0._jprb
          ENDIF
        ENDDO
      ENDIF
    ENDDO
