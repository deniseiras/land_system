...
#include "rttov_setpredictors_78.interface"
...
!-------------
  ...
  IF (opts%rt_ir%pc%addpc) THEN
    IF (opts%rt_ir%pc%npcscores > 0) THEN
      ...
      IF (npcscores > SIZE(pccomp%pcscores)) THEN
...
!-----------------------------------------------------------------------
  CALL rttov_convert_profile_units( &
      opts,                         &
      coefs,                        &
      profiles,                     &
      traj0%profiles_int)
  ...
  IF (opts%config%do_checkinput) THEN
    CALL rttov_check_profiles(err, opts, coefs, profiles, traj0%profiles_int, &
...
!------------------------------------------------------------------
! Check profile data against regression limits - coef levs
!------------------------------------------------------------------
    IF (opts%config%do_checkinput) THEN
...
!------------------------------------------------------------------------
    CALL rttov_profaux( &
        ...
        coefs%coef,            &
        traj0%aux_prof_coef,   &
        on_coef_levels = LOGICAL((coefs%coef%fmv_model_ver < 9), KIND=jplm))
...
!--------------------------------------------------------------------
! If clouds are present calculate the number of columns (streams) and
! the cloud distribution in each column - user levs
!--------------------------------------------------------------------
  ...
  CALL rttov_alloc_traj_dyn(err, traj0_dyn, opts, coefs, nchanprof, profiles(1)%nlayers, &
                            ...
                            traj0_sta%thermal, traj0_sta%solar, traj0_sta%do_mfasis, 1_jpim)
  THROW(err.NE.0)
...
!---------------------------------------------------------
  ...
  ELSEIF (.NOT. traj0_sta%do_mfasis) THEN
    IF (traj0_sta%dothermal) THEN
      CALL rttov_calcbt(opts, chanprof, coefs%coef, traj0_sta%thermal, radiance)
    ENDIF
    IF (traj0_sta%dosolar) THEN
