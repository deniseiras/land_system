...
!INTF_END
  ...
  DOUBLE PRECISION        :: kktmp_ad(nstr*maxnlayers)
...
!     done_lay_str_azi_ad = .FALSE.
    ...
    DO ist = ircld%nstream(prof), 0, -1
      profiles_dom_ad(ist,i)%layerod(:) = 0._jprb
      thisrad_ad   = 0._jprb
      ...
      IF (ist == 0) THEN
        ircld_ad%xstrclr(prof) = ircld_ad%xstrclr(prof) + dom_state(i)%thisrad(ist) * rad_ad%total(i)
        thisrad_ad = thisrad_ad + ircld%xstrclr(prof) * rad_ad%total(i)

! Do not accumulate from clear - input perturbation is only in total
!         thisrad_ad = thisrad_ad + rad_ad%clear(i)

      ELSE
        ircld_ad%xstr(ist+1,prof) = ircld_ad%xstr(ist+1,prof) + dom_state(i)%thisrad(ist) * rad_ad%total(i)
        ircld_ad%xstr(ist,prof) = ircld_ad%xstr(ist,prof) - dom_state(i)%thisrad(ist) * rad_ad%total(i)
        thisrad_ad = thisrad_ad + rad_ad%total(i) * (ircld%xstr(ist+1,prof) - ircld%xstr(ist,prof))
      ENDIF
      ...
      IF (dothermal) THEN
        thisnaz = 0
      ELSE
        thisnaz = dom_state(i)%nazloops(ist)
      ENDIF
...
!               bpmsave    = bpm  ! Used for solar order-n PI below - not implemented in AD (yet)
              ...
              xp = dom_state(i)%xp(:,:,laymap,isti,m)

              DO j = 1, n
                xm(:,j) = MATMUL(sp, xp(:,j)) / dom_state(i)%eval(j,laymap,isti,m)
              ENDDO
            ...
            IF (.NOT. done_lay_str_azi(laymap,isti,m)) THEN
              ...
              IF (dosolar) THEN
                fpmupi(laymap,isti,m) = 0.5_jprb * SUM(dom_state(i)%z(:,laymap,isti,m) * phasesat(:)) + &
            ...
            IF (dothermal) THEN
              fpmupi0(lay) = 0.5_jprb * SUM(dom_state(i)%y0(:,lay,ist) * phasesat(:)) + b0(lay)
              fpmupi1(lay) = 0.5_jprb * SUM(dom_state(i)%y1(:,lay,ist) * phasesat(:)) + b1(lay)
            ENDIF
          ...
          ! --------------------------------------------------------------------------
          tlayer(:,lay) = EXP(-ftau(lay) * profiles_dom(ist,i)%layerod(lay) * dom_state(i)%eval(:,laymap,isti,m))
        ...
        IF (m == 0 .AND. inc_surface) THEN
          ...
          tausat_ad(nlevels) = tausat_ad(nlevels) + radinc_ad * dom_state(i)%radsurfup(ist)
          ...
          IF (dosolar) THEN
            ...
            reflfac_ad = reflfac_ad + dom_state(i)%radsurfup_sum(ist) * radsurfup_ad

            DO j = n, 1, -1
              tausun_ad(nlevels) = tausun_ad(nlevels) + muqw(j) * dom_state(i)%z(n+j,laymap,isti,m) * radsurfup_sum_ad
              z_ad(n+j,laymap,isti,m) = z_ad(n+j,laymap,isti,m) + muqw(j) * tausun(nlevels) * radsurfup_sum_ad
            ENDDO
          ...
          ELSE
            ...
            DO j = n, 1, -1
              opdep_ad(nlevels) = opdep_ad(nlevels) + muqw(j) * dom_state(i)%y1(n+j,nlayers,ist) * radsurfup_sum_ad
          ...
          DO j = n, 1, -1
            fm2_ad(j,:,laymap,isti,m) = fm2_ad(j,:,laymap,isti,m) + &
                muqw(j) * dom_state(i)%x(row+n+1:row+nstr,m,ist) * radsurfup_sum_ad
            x_ad(row+n+1:row+nstr) = x_ad(row+n+1:row+nstr) + &
                muqw(j) * fm2(j,:,laymap,isti,m) * radsurfup_sum_ad
            tlayer_ad(:,nlayers) = tlayer_ad(:,nlayers) + &
                muqw(j) * dom_state(i)%x(row+1:row+n,m,ist) * fm1(j,:,laymap,isti,m) * radsurfup_sum_ad
            fm1_ad(j,:,laymap,isti,m) = fm1_ad(j,:,laymap,isti,m) + &
                muqw(j) * dom_state(i)%x(row+1:row+n,m,ist) * tlayer(:,nlayers) * radsurfup_sum_ad
            x_ad(row+1:row+n) = x_ad(row+1:row+n) + &
                muqw(j) * fm1(j,:,laymap,isti,m) * tlayer(:,nlayers) * radsurfup_sum_ad
          ENDDO
        ...
        DO lay = nlayers, 1, -1
          ...
          IF (fssa(lay) > 0._jprb) THEN
            ...
            IF (dosolar) THEN
              ...
              taufac_ad = &!taufac_ad +
                          radinc_ad * fpmupi(laymap,isti,m) / (1._jprb + musunr * musat)
              fpmupi_ad(laymap,isti,m) = fpmupi_ad(laymap,isti,m) + radinc_ad * &
            ...
            DO j = n, 1, -1
              ...
              tmpval = 1._jprb / (1._jprb - dom_state(i)%eval(j,laymap,isti,m) * musat)
              eval_ad(j,laymap,isti,m) = eval_ad(j,laymap,isti,m) + &
                  radinc_ad * musat * dom_state(i)%x(row+n+j,m,ist) * &
                  fpmuhsdn(j,laymap,isti,m) * taufac * tmpval**2
              taufac_ad = &!taufac_ad + &
                  radinc_ad * dom_state(i)%x(row+n+j,m,ist) * &
                  fpmuhsdn(j,laymap,isti,m) * tmpval
              fpmuhsdn_ad(j,laymap,isti,m) = fpmuhsdn_ad(j,laymap,isti,m) + &
                  radinc_ad * dom_state(i)%x(row+n+j,m,ist) * taufac * tmpval
              x_ad(row+n+j) = x_ad(row+n+j) + &
                  radinc_ad * fpmuhsdn(j,laymap,isti,m) * taufac * tmpval
              ...
              tmpval = 1._jprb / (1._jprb + dom_state(i)%eval(j,laymap,isti,m) * musat)
              eval_ad(j,laymap,isti,m) = eval_ad(j,laymap,isti,m) - &
                  radinc_ad * musat * dom_state(i)%x(row+j,m,ist) * &
                  fpmuhsup(j,laymap,isti,m) * taufac * tmpval**2
              taufac_ad = &!taufac_ad + &
                  radinc_ad * dom_state(i)%x(row+j,m,ist) * &
                  fpmuhsup(j,laymap,isti,m) * tmpval
              fpmuhsup_ad(j,laymap,isti,m) = fpmuhsup_ad(j,laymap,isti,m) + &
                  radinc_ad * dom_state(i)%x(row+j,m,ist) * taufac * tmpval
              x_ad(row+j) = x_ad(row+j) + &
                  radinc_ad * fpmuhsup(j,laymap,isti,m) * taufac * tmpval
        ...
        ! Do this using a temporary array for each column, solving and writing directly to kkpack_ad

        DO j = 1, nstrlay
          kktmp_ad(1:nstrlay) = -x_ad(1:nstrlay) * dom_state(i)%x(j,m,ist)
          CALL DGBTRS('Transpose', nstrlay, kl, ku, 1, kkpack, klku, &
                      ipiv1, kktmp_ad(1:nstrlay), nstrlay, info)
          IF (info /= 0) THEN
          ...
          lo = klu1 + ilo - j
          hi = klu1 + ihi - j
          kkpack_ad(lo:hi,j) = kktmp_ad(ilo:ihi)
        ENDDO

        ! 2. Solve for x_ad
        CALL DGBTRS('Transpose', nstrlay, kl, ku, 1, kkpack, klku, &
                    ipiv1, x_ad(1:nstrlay), nstrlay, info)
        IF (info /= 0) THEN
          WRITE(msg,'(a,i5)') 'DOM error: solving AD linear system, DGBTRS error, info = ', info
          err = errorstatus_fatal
          THROWM(err.NE.0,msg)
...
! ** Require phase_fn and phasesat on every layer, will have been overwritten in direct loop above **
          ...
          IF (dosolar .AND. m == 0 .AND. fssa(lay) > 0._jprb) THEN
            ...
            legcoef_ad(:,laymap,isti) = legcoef_ad(:,laymap,isti) - &
                delta_tms_ad * tmpval * taufac * legpolymuscata(:,0,0)
            ftau_ad(lay) = ftau_ad(lay) - &
                delta_tms_ad * tmpval * taufac * &
                ssa(isti,laymap,i) * trans_scatt_ir%phup(isti,laymap,i) * tmpval2**2
            trans_scatt_ir_ad%phup(isti,laymap,i) = trans_scatt_ir_ad%phup(isti,laymap,i) + &
                delta_tms_ad * tmpval * taufac * &
                ssa(isti,laymap,i) * tmpval2
            ssa_ad(isti,laymap,i) = ssa_ad(isti,laymap,i) + &
                delta_tms_ad * tmpval * taufac * &
                trans_scatt_ir%phup(isti,laymap,i) * tmpval2
            taufac_ad = &!taufac_ad + &
                delta_tms_ad * tmpval * &
                (ssa(isti,laymap,i) * trans_scatt_ir%phup(isti,laymap,i) * tmpval2 - &
          ...
          IF (lay < nlayers) THEN
            ...
            col = (lay-1) * nstr
            IF (dosolar) THEN
              tausun_ad(lev) = tausun_ad(lev) - SUM(x_ad(row+1:row+nstr) * dom_state(i)%z(:,laymap,isti,m))
              z_ad(:,laymap,isti,m) = z_ad(:,laymap,isti,m) - x_ad(row+1:row+nstr) * tausun(lev)
            ELSE
              opdep_ad(lev) = opdep_ad(lev) - SUM(x_ad(row+1:row+nstr) * dom_state(i)%y1(:,lay,ist))
              y1_ad(:,lay) = y1_ad(:,lay) - x_ad(row+1:row+nstr) * opdep(lev)
              y0_ad(:,lay) = y0_ad(:,lay) - x_ad(row+1:row+nstr)
            ENDIF
            DO j = n, 1, -1
              lo = klu1 + (row + 1) - (col + j)
              fp1_ad(:,j,laymap,isti,m) = fp1_ad(:,j,laymap,isti,m) + kkpack_ad(lo:lo+n-1,col+j) * tlayer(j,lay)
              tlayer_ad(j,lay) = tlayer_ad(j,lay) + SUM(kkpack_ad(lo:lo+n-1,col+j) * fp1(:,j,laymap,isti,m))

              lo = lo - n
              fp2_ad(:,j,laymap,isti,m) = fp2_ad(:,j,laymap,isti,m) + kkpack_ad(lo:lo+n-1,col+n+j)

              lo = klu1 + (row + n + 1) - (col + j)
              fm1_ad(:,j,laymap,isti,m) = fm1_ad(:,j,laymap,isti,m) + kkpack_ad(lo:lo+n-1,col+j) * tlayer(j,lay)
              tlayer_ad(j,lay) = tlayer_ad(j,lay) + SUM(kkpack_ad(lo:lo+n-1,col+j) * fm1(:,j,laymap,isti,m))

              lo = lo - n
              fm2_ad(:,j,laymap,isti,m) = fm2_ad(:,j,laymap,isti,m) + kkpack_ad(lo:lo+n-1,col+n+j)
            ENDDO
          ELSE
            row = (lay-1) * nstr + n
            col = (lay-1) * nstr
            ...
            IF (m == 0 .AND. inc_surface) THEN
              ...
              IF (dosolar) THEN
                ...
                z_ad(n+1:nstr,laymap,isti,m) = z_ad(n+1:nstr,laymap,isti,m) + &
                    tausun(lev) * tmpval * reflfac * muqw(:)
                reflfac_ad = reflfac_ad + &
                    tausun(lev) * tmpval * SUM(muqw(:) * dom_state(i)%z(n+1:nstr,laymap,isti,m))
                z_ad(1:n,laymap,isti,m) = z_ad(1:n,laymap,isti,m) - tausun(lev) * x_ad(row+1:row+n)
                tausun_ad(lev) = tausun_ad(lev) - SUM(x_ad(row+1:row+n) * (dom_state(i)%z(1:n,laymap,isti,m) - &
                                    reflfac * SUM(muqw(:) * dom_state(i)%z(n+1:nstr,laymap,isti,m)) - &
              ...
              ELSE
                ...
                opdep_ad(lev) = opdep_ad(lev) + &
                    reflfac * tmpval * SUM(muqw(:) * dom_state(i)%y1(n+1:nstr,lay,ist))
                ...
                reflfac_ad = reflfac_ad + tmpval * SUM(muqw(:) * (dom_state(i)%y0(n+1:nstr,lay,ist) + &
                                                                  dom_state(i)%y1(n+1:nstr,lay,ist) * opdep(lev)))

                opdep_ad(lev) = opdep_ad(lev) - SUM(x_ad(row+1:row+n) * dom_state(i)%y1(1:n,lay,ist))
              ...
              DO j = n, 1, -1
                ...
                tlayer_ad(j,lay) = tlayer_ad(j,lay) + &
                    SUM(kkpack_ad(lo:lo+n-1,col+j) * (fp1(:,j,laymap,isti,m) - surfterm(j)))
                surfterm_ad(j) = surfterm_ad(j) - SUM(kkpack_ad(lo:lo+n-1,col+j)) * tlayer(j,lay)
                fp1_ad(:,j,laymap,isti,m) = fp1_ad(:,j,laymap,isti,m) + &
              ...
              IF (reflfac > 0._jprb) THEN
                DO j = n, 1, -1
                  fm2_ad(:,j,laymap,isti,m) = fm2_ad(:,j,laymap,isti,m) + surfterm_ad(n+j) * reflfac * muqw(:)
                  fm1_ad(:,j,laymap,isti,m) = fm1_ad(:,j,laymap,isti,m) + surfterm_ad(j) * reflfac * muqw(:)
            ...
            ELSE
              IF (dosolar) THEN
                tausun_ad(lev) = tausun_ad(lev) - SUM(x_ad(row+1:row+n) * dom_state(i)%z(1:n,laymap,isti,m))
                z_ad(1:n,laymap,isti,m) = z_ad(1:n,laymap,isti,m) - x_ad(row+1:row+n) * tausun(lev)
              ELSE
                opdep_ad(lev) = opdep_ad(lev) - SUM(x_ad(row+1:row+n) * dom_state(i)%y1(1:n,lay,ist))
                y1_ad(1:n,lay) = y1_ad(1:n,lay) - x_ad(row+1:row+n) * opdep(lev)
                y0_ad(1:n,lay) = y0_ad(1:n,lay) - x_ad(row+1:row+n)
          ...
          IF (lay == 1) THEN
            ...
            col = 0
            IF (dosolar) THEN
              z_ad(n+1:nstr,laymap,isti,m) = z_ad(n+1:nstr,laymap,isti,m) - x_ad(1:n)
            ELSE
            ...
            DO j = n, 1, -1
              lo = klu1 + (row + 1) - (col + j)
              fm1_ad(:,j,laymap,isti,m) = fm1_ad(:,j,laymap,isti,m) + kkpack_ad(lo:lo+n-1,j)

              lo = lo - n
              tlayer_ad(j,lay) = tlayer_ad(j,lay) + SUM(kkpack_ad(lo:lo+n-1,n+j) * fm2(:,j,laymap,isti,m))
              fm2_ad(:,j,laymap,isti,m) = fm2_ad(:,j,laymap,isti,m) + kkpack_ad(lo:lo+n-1,n+j) * tlayer(j,lay)
            ENDDO
          ELSE
            row = (lay-2) * nstr + n
            col = (lay-1) * nstr
            IF (dosolar) THEN
              tausun_ad(lev-1) = tausun_ad(lev-1) + SUM(x_ad(row+1:row+nstr) * dom_state(i)%z(:,laymap,isti,m))
              z_ad(:,laymap,isti,m) = z_ad(:,laymap,isti,m) + x_ad(row+1:row+nstr) * tausun(lev-1)
            ELSE
              opdep_ad(lev-1) = opdep_ad(lev-1) + SUM(x_ad(row+1:row+nstr) * dom_state(i)%y1(:,lay,ist))
              y1_ad(:,lay) = y1_ad(:,lay) + x_ad(row+1:row+nstr) * opdep(lev-1)
              y0_ad(:,lay) = y0_ad(:,lay) + x_ad(row+1:row+nstr)
            ENDIF
            DO j = n, 1, -1
              lo = klu1 + (row + 1) - (col + j)
              fp1_ad(:,j,laymap,isti,m) = fp1_ad(:,j,laymap,isti,m) - kkpack_ad(lo:lo+n-1,col+j)

              lo = lo - n
              tlayer_ad(j,lay) = tlayer_ad(j,lay) - &
                  SUM(kkpack_ad(lo:lo+n-1,col+n+j) * fp2(:,j,laymap,isti,m))
              fp2_ad(:,j,laymap,isti,m) = fp2_ad(:,j,laymap,isti,m) - &
                  kkpack_ad(lo:lo+n-1,col+n+j) * tlayer(j,lay)

              lo = klu1 + (row + n + 1) - (col + j)
              fm1_ad(:,j,laymap,isti,m) = fm1_ad(:,j,laymap,isti,m) - kkpack_ad(lo:lo+n-1,col+j)

              lo = lo - n
              tlayer_ad(j,lay) = tlayer_ad(j,lay) - &
                  SUM(kkpack_ad(lo:lo+n-1,col+n+j) * fm2(:,j,laymap,isti,m))
              fm2_ad(:,j,laymap,isti,m) = fm2_ad(:,j,laymap,isti,m) - &
                  kkpack_ad(lo:lo+n-1,col+n+j) * tlayer(j,lay)
            ENDDO
          ENDIF
          ...
          ! Calculate layer "transmittances" with opdeps scaled by eigenvalues
          ! --------------------------------------------------------------------------
          eval_ad(:,laymap,isti,m) = eval_ad(:,laymap,isti,m) - &
              tlayer_ad(:,lay) * tlayer(:,lay) * ftau(lay) * profiles_dom(ist,i)%layerod(lay)
          tmpval = SUM(tlayer_ad(:,lay) * tlayer(:,lay) * dom_state(i)%eval(:,laymap,isti,m))
          profiles_dom_ad(ist,i)%layerod(lay) = profiles_dom_ad(ist,i)%layerod(lay) - tmpval * ftau(lay)
          ftau_ad(lay) = ftau_ad(lay) - tmpval * profiles_dom(ist,i)%layerod(lay)
          ...
          IF (fssa(lay) > 0._jprb) THEN
            ...
            IF (dothermal) THEN
              ...
              phasesat_ad(:) = phasesat_ad(:) + 0.5_jprb * fpmupi1_ad(lay) * dom_state(i)%y1(:,lay,ist)
              y1_ad(:,lay) = y1_ad(:,lay) + 0.5_jprb * fpmupi1_ad(lay) * phasesat(:)

              b0_ad(lay) = b0_ad(lay) + fpmupi0_ad(lay)
              phasesat_ad(:) = phasesat_ad(:) + 0.5_jprb * fpmupi0_ad(lay) * dom_state(i)%y0(:,lay,ist)
            ...
            ! Interpolate HS and PI for final radiance calculation
            ! --------------------------------------------------------------------------
!             IF (.NOT. done_lay_str_azi_ad(laymap,isti,m)) THEN

              ! Particular integral - solar source term, done once per layer (applies to all cloud streams)
              IF (dosolar) THEN
                legcoef_ad(m:nmom,laymap,isti) = legcoef_ad(m:nmom,laymap,isti) + &
                    sunsrcfac * fpmupi_ad(laymap,isti,m) * legpolymusat(m:nmom,0,m) * legpolymusun(m:nmom,0,m)
                phasesat_ad(:) = phasesat_ad(:) + &
                    0.5_jprb * fpmupi_ad(laymap,isti,m) * dom_state(i)%z(:,laymap,isti,m)
                z_ad(:,laymap,isti,m) = z_ad(:,laymap,isti,m) + &
                    0.5_jprb * fpmupi_ad(laymap,isti,m) * phasesat(:)
              ENDIF

              ! Homogenous part - diffuse radiation, done once per layer (applies to all cloud streams)
              DO j = n, 1, -1
                phasesat_ad(n+1:nstr) = phasesat_ad(n+1:nstr) + &
                    0.5_jprb * fpmuhsdn_ad(j,laymap,isti,m) * fm2(:,j,laymap,isti,m)
                fm2_ad(:,j,laymap,isti,m) = fm2_ad(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsdn_ad(j,laymap,isti,m) * phasesat(n+1:nstr)
                phasesat_ad(1:n) = phasesat_ad(1:n) + &
                    0.5_jprb * fpmuhsdn_ad(j,laymap,isti,m) * fp2(:,j,laymap,isti,m)
                fp2_ad(:,j,laymap,isti,m) = fp2_ad(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsdn_ad(j,laymap,isti,m) * phasesat(1:n)
                phasesat_ad(n+1:nstr) = phasesat_ad(n+1:nstr) + &
                    0.5_jprb * fpmuhsup_ad(j,laymap,isti,m) * fm1(:,j,laymap,isti,m)
                fm1_ad(:,j,laymap,isti,m) = fm1_ad(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsup_ad(j,laymap,isti,m) * phasesat(n+1:nstr)
                phasesat_ad(1:n) = phasesat_ad(1:n) + &
                    0.5_jprb * fpmuhsup_ad(j,laymap,isti,m) * fp1(:,j,laymap,isti,m)
                fp1_ad(:,j,laymap,isti,m) = fp1_ad(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsup_ad(j,laymap,isti,m) * phasesat(1:n)
              ENDDO
...
!               hh_ad = - MATMUL(hhinvt, &
!                   MATMUL(y0_ad(:,lay:lay), TRANSPOSE(dom_state(i)%y0(:,lay:lay,ist))))
...
!               y0_ad(:,lay) = MATMUL(hhinvt, y0_ad(:,lay))
              ...
              hh_ad = -MATMUL(y0_ad(:,lay:lay), TRANSPOSE(dom_state(i)%y0(:,lay:lay,ist)))
              DO j = 1, nstr
                CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, hh_ad(:,j), nstri, info)
                IF (info /= 0) THEN
...
!                   MATMUL(y1_ad(:,lay:lay), TRANSPOSE(dom_state(i)%y1(:,lay:lay,ist))))

!               y1_ad(:,lay) = MATMUL(hhinvt, y1_ad(:,lay))


              hhtmp_ad = -MATMUL(y1_ad(:,lay:lay), TRANSPOSE(dom_state(i)%y1(:,lay:lay,ist)))
              DO j = 1, nstr
                CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, hhtmp_ad(:,j), nstri, info)
                IF (info /= 0) THEN
                  WRITE(msg,'(a,3i5)') 'DOM error: solving AD PI thermal y1, DGETRS error, info = ', info, i, lay
                  err = errorstatus_fatal
                  THROWM(err.NE.0,msg)
                ENDIF
                hh_ad(:,j) = hh_ad(:,j) + hhtmp_ad(:,j)
              ENDDO
...
!                 ! No accumulation required
!                 hh_ad = - MATMUL(hhinvt, &
!                     MATMUL(z_ad(:,laymap:laymap,isti,m), TRANSPOSE(dom_state(i)%z(:,laymap:laymap,isti,m))))
!
!                 z_ad(:,laymap,isti,m) = MATMUL(hhinvt, z_ad(:,laymap,isti,m))
                ...
                hh_ad = -MATMUL(z_ad(:,laymap:laymap,isti,m), TRANSPOSE(dom_state(i)%z(:,laymap:laymap,isti,m)))
                DO j = 1, nstr
                  CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, hh_ad(:,j), nstri, info)
                  IF (info /= 0) THEN
                ...
                CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, z_ad(:,laymap,isti,m), nstri, info)
                IF (info /= 0) THEN
                  WRITE(msg,'(a,3i5)') 'DOM error: solving AD PI solar, DGETRS error, info = ', info, i, lay
                  err = errorstatus_fatal
                  THROWM(err.NE.0,msg)
                ...
                DO j = n, 1, -1
                  legcoef_ad(m:nmom,laymap,isti) = legcoef_ad(m:nmom,laymap,isti) + &
                      sunsrcfac * z_ad(n+j,laymap,isti,m) * legpolymuq(m:nmom,n+j,m) * legpolymusun(m:nmom,0,m)
                  legcoef_ad(m:nmom,laymap,isti) = legcoef_ad(m:nmom,laymap,isti) + &
                      sunsrcfac * z_ad(j,laymap,isti,m) * legpolymuq(m:nmom,j,m) * legpolymusun(m:nmom,0,m)
...
! ** Recompute xm or store above? **
              ...
              xm_ad = 0.5_jprb * (fp1_ad(:,:,laymap,isti,m) - fm1_ad(:,:,laymap,isti,m))
              xp_ad = 0.5_jprb * (fp1_ad(:,:,laymap,isti,m) + fm1_ad(:,:,laymap,isti,m))

              sp_ad = 0._jprb
              DO j = n, 1, -1
                eval_ad(j,laymap,isti,m) = eval_ad(j,laymap,isti,m) - &
                    SUM(xm_ad(:,j) * xm(:,j)) / dom_state(i)%eval(j,laymap,isti,m)
                sp_ad = sp_ad + MATMUL(xm_ad(:,j:j), TRANSPOSE(xp(:,j:j))) / dom_state(i)%eval(j,laymap,isti,m)
                xp_ad(:,j) = xp_ad(:,j) + MATMUL(TRANSPOSE(sp), xm_ad(:,j)) / dom_state(i)%eval(j,laymap,isti,m)
              ENDDO

              ! Calculate the required eigenvalues (+/- each one gives nstr evals)
              eval_ad(:,laymap,isti,m) = 0.5_jprb * eval_ad(:,laymap,isti,m) / dom_state(i)%eval(:,laymap,isti,m)
...
!               bpm_ad = 0._jprb ! Not accumulated in ASYMTX_AD
              eval = dom_state(i)%eval(:,laymap,isti,m)**2
              CALL ASYMTX_AD(err, n, xp, eval, xp_ad, eval_ad(:,laymap,isti,m), bpm_ad)
              IF (err /= 0) THEN
...
!             ENDIF ! not done_lay_str_azi_ad
            ...
            ! For scattering layers, only do calculation if we don't already have the results
            ! --------------------------------------------------------------------------
!             IF (dothermal .OR. .NOT. done_lay_str_azi_ad(laymap,isti,m)) THEN
              ...
              IF (m == 0) THEN  ! This calculation applies to all azi components
                DO l = nmom, 0, -1
                  f_lay_ad = f_lay_ad + legcoef_ad(l,laymap,isti) * &
                      fssa(lay) * (trans_scatt_ir_dyn%phasefn(i)%legcoef(l,isti,laymap) - &
                      (2._jprb * l + 1._jprb) * f_lay) / (1._jprb - f_lay)**2
                  f_lay_ad = f_lay_ad - legcoef_ad(l,laymap,isti) * &
                      fssa(lay) * (2._jprb * l + 1._jprb) / (1._jprb - f_lay)
                  trans_scatt_ir_dyn_ad%phasefn(i)%legcoef(l,isti,laymap) = &
                      trans_scatt_ir_dyn_ad%phasefn(i)%legcoef(l,isti,laymap) + &
                      legcoef_ad(l,laymap,isti) * fssa(lay) / (1._jprb - f_lay)
                  fssa_ad(lay) = fssa_ad(lay) + legcoef_ad(l,laymap,isti) * &
                      (trans_scatt_ir_dyn%phasefn(i)%legcoef(l,isti,laymap) - &
...
!             done_lay_str_azi_ad(laymap,isti,m) = .TRUE.
          ...
          IF (m == 0) THEN  ! This calculation applies to all azi components
            ...
            profiles_dom_ad(ist,i)%layerod(lay) = profiles_dom_ad(ist,i)%layerod(lay) + &
                opdep_ad(lev) * ftau(lay)
            ftau_ad(lay) = ftau_ad(lay) + opdep_ad(lev) * profiles_dom(ist,i)%layerod(lay)
          ...
          IF (m == 0) THEN
            IF (laymap > 0) THEN
              ftau_ad(lay) = ftau_ad(lay) - fssa_ad(lay) * (1._jprb - f_lay) * ssa(isti,laymap,i) / ftau(lay)**2
              ssa_ad(isti,laymap,i) = ssa_ad(isti,laymap,i) + fssa_ad(lay) * (1._jprb - f_lay) / ftau(lay)
              f_lay_ad = f_lay_ad - fssa_ad(lay) * ssa(isti,laymap,i) / ftau(lay)

              f_lay_ad = f_lay_ad - ftau_ad(lay) * ssa(isti,laymap,i)
              ssa_ad(isti,laymap,i) = ssa_ad(isti,laymap,i) - ftau_ad(lay) * f_lay
              trans_scatt_ir_dyn_ad%phasefn(i)%legcoef(nstr,isti,laymap) = &
                  trans_scatt_ir_dyn_ad%phasefn(i)%legcoef(nstr,isti,laymap) + f_lay_ad / (2._jprb * nstr + 1._jprb)
!             ELSE
!               f_lay_ad     = 0._jprb
!               ftau_ad(lay) = 0._jprb
!               fssa_ad(lay) = 0._jprb
