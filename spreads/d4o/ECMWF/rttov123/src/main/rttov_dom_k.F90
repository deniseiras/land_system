...
!INTF_END
  ...
  DOUBLE PRECISION        :: kktmp_k(nstr*maxnlayers)
...
!     done_lay_str_azi_k = .FALSE.
    ...
    DO ist = ircld%nstream(prof), 0, -1
      profiles_dom_k(ist,i)%layerod(:) = 0._jprb
      thisrad_k   = 0._jprb
      delta_tms_k = 0._jprb
      albedo_k = 0._jprb
      reflfac_k = 0._jprb
      ...
      IF (ist == 0) THEN
        ircld_k%xstrclr(i) = ircld_k%xstrclr(i) + dom_state(i)%thisrad(ist) * rad_k%total(i)
        thisrad_k = thisrad_k + ircld%xstrclr(prof) * rad_k%total(i)

! Do not accumulate from clear - input perturbation is only in total
!         thisrad_k = thisrad_k + rad_k%clear(i)

      ELSE
        ircld_k%xstr(ist+1,i) = ircld_k%xstr(ist+1,i) + dom_state(i)%thisrad(ist) * rad_k%total(i)
        ircld_k%xstr(ist,i) = ircld_k%xstr(ist,i) - dom_state(i)%thisrad(ist) * rad_k%total(i)
        thisrad_k = thisrad_k + rad_k%total(i) * (ircld%xstr(ist+1,prof) - ircld%xstr(ist,prof))
      ENDIF
...
!               bpmsave    = bpm  ! Used for solar order-n PI below - not implemented in AD (yet)
        ...
        IF (m == 0 .AND. inc_surface) THEN
          ...
          tausat_k(nlevels) = tausat_k(nlevels) + radinc_k * dom_state(i)%radsurfup(ist)
          ...
          IF (dosolar) THEN
            ...
            reflfac_k = reflfac_k + dom_state(i)%radsurfup_sum(ist) * radsurfup_k

            DO j = n, 1, -1
              tausun_k(nlevels) = tausun_k(nlevels) + muqw(j) * dom_state(i)%z(n+j,laymap,isti,m) * radsurfup_sum_k
              z_k(n+j,laymap,isti,m) = z_k(n+j,laymap,isti,m) + muqw(j) * tausun(nlevels) * radsurfup_sum_k
            ENDDO
          ...
          ELSE
            ...
            DO j = n, 1, -1
              opdep_k(nlevels) = opdep_k(nlevels) + muqw(j) * dom_state(i)%y1(n+j,nlayers,ist) * radsurfup_sum_k
          ...
          DO j = n, 1, -1
            fm2_k(j,:,laymap,isti,m) = fm2_k(j,:,laymap,isti,m) + &
                muqw(j) * dom_state(i)%x(row+n+1:row+nstr,m,ist) * radsurfup_sum_k
            x_k(row+n+1:row+nstr) = x_k(row+n+1:row+nstr) + &
                muqw(j) * fm2(j,:,laymap,isti,m) * radsurfup_sum_k
            tlayer_k(:,nlayers) = tlayer_k(:,nlayers) + &
                muqw(j) * dom_state(i)%x(row+1:row+n,m,ist) * fm1(j,:,laymap,isti,m) * radsurfup_sum_k
            fm1_k(j,:,laymap,isti,m) = fm1_k(j,:,laymap,isti,m) + &
                muqw(j) * dom_state(i)%x(row+1:row+n,m,ist) * tlayer(:,nlayers) * radsurfup_sum_k
            x_k(row+1:row+n) = x_k(row+1:row+n) + &
                muqw(j) * fm1(j,:,laymap,isti,m) * tlayer(:,nlayers) * radsurfup_sum_k
          ENDDO
        ...
        DO lay = nlayers, 1, -1
          ...
          IF (fssa(lay) > 0._jprb) THEN
            ...
            IF (dosolar) THEN
              ...
              taufac_k = &!taufac_k +
                         radinc_k * fpmupi(laymap,isti,m) / (1._jprb + musunr * musat)
              fpmupi_k(laymap,isti,m) = fpmupi_k(laymap,isti,m) + radinc_k * &
            ...
            DO j = n, 1, -1
              ...
              eval_k(j,laymap,isti,m) = eval_k(j,laymap,isti,m) + &
                  radinc_k * musat * dom_state(i)%x(row+n+j,m,ist) * &
                  fpmuhsdn(j,laymap,isti,m) * taufac * tmpval**2
              taufac_k = &!taufac_k + &
                  radinc_k * dom_state(i)%x(row+n+j,m,ist) * &
                  fpmuhsdn(j,laymap,isti,m) * tmpval
              fpmuhsdn_k(j,laymap,isti,m) = fpmuhsdn_k(j,laymap,isti,m) + &
                  radinc_k * dom_state(i)%x(row+n+j,m,ist) * taufac * tmpval
              x_k(row+n+j) = x_k(row+n+j) + &
                  radinc_k * fpmuhsdn(j,laymap,isti,m) * taufac * tmpval
              ...
              tmpval = 1._jprb / (1._jprb + dom_state(i)%eval(j,laymap,isti,m) * musat)
              eval_k(j,laymap,isti,m) = eval_k(j,laymap,isti,m) - &
                  radinc_k * musat * dom_state(i)%x(row+j,m,ist) * &
                  fpmuhsup(j,laymap,isti,m) * taufac * tmpval**2
              taufac_k = &!taufac_k + &
                  radinc_k * dom_state(i)%x(row+j,m,ist) * &
                  fpmuhsup(j,laymap,isti,m) * tmpval
              fpmuhsup_k(j,laymap,isti,m) = fpmuhsup_k(j,laymap,isti,m) + &
                  radinc_k * dom_state(i)%x(row+j,m,ist) * taufac * tmpval
              x_k(row+j) = x_k(row+j) + &
                  radinc_k * fpmuhsup(j,laymap,isti,m) * taufac * tmpval
        ...
        ! Do this using a temporary array for each column, solving and writing directly to kkpack_k

        DO j = 1, nstrlay
          kktmp_k(1:nstrlay) = -x_k(1:nstrlay) * dom_state(i)%x(j,m,ist)
          CALL DGBTRS('Transpose', nstrlay, kl, ku, 1, kkpack, klku, &
                      ipiv1, kktmp_k(1:nstrlay), nstrlay, info)
          IF (info /= 0) THEN
          ...
          lo = klu1 + ilo - j
          hi = klu1 + ihi - j
          kkpack_k(lo:hi,j) = kktmp_k(ilo:ihi)
        ENDDO

        ! 2. Solve for x_k
        CALL DGBTRS('Transpose', nstrlay, kl, ku, 1, kkpack, klku, &
                    ipiv1, x_k(1:nstrlay), nstrlay, info)
        IF (info /= 0) THEN
          WRITE(msg,'(a,i5)') 'DOM error: solving K linear system, DGBTRS error, info = ', info
          err = errorstatus_fatal
          THROWM(err.NE.0,msg)
...
! Require phase_fn and phasesat on every layer, will have been overwritten in direct loop above **
          ...
          IF (dosolar .AND. m == 0 .AND. fssa(lay) > 0._jprb) THEN
            ...
            legcoef_k(:,laymap,isti) = legcoef_k(:,laymap,isti) - &
            ...
            trans_scatt_ir_k%phup(isti,laymap,i) = trans_scatt_ir_k%phup(isti,laymap,i) + &
                delta_tms_k * tmpval * taufac * &
                ssa(isti,laymap,i) * tmpval2
            ssa_k(isti,laymap,i) = ssa_k(isti,laymap,i) + &
                delta_tms_k * tmpval * taufac * &
          ...
          IF (lay < nlayers) THEN
            ...
            col = (lay-1) * nstr
            IF (dosolar) THEN
              tausun_k(lev) = tausun_k(lev) - SUM(x_k(row+1:row+nstr) * dom_state(i)%z(:,laymap,isti,m))
              z_k(:,laymap,isti,m) = z_k(:,laymap,isti,m) - x_k(row+1:row+nstr) * tausun(lev)
            ELSE
              opdep_k(lev) = opdep_k(lev) - SUM(x_k(row+1:row+nstr) * dom_state(i)%y1(:,lay,ist))
              y1_k(:,lay) = y1_k(:,lay) - x_k(row+1:row+nstr) * opdep(lev)
              y0_k(:,lay) = y0_k(:,lay) - x_k(row+1:row+nstr)
            ENDIF
            DO j = n, 1, -1
              lo = klu1 + (row + 1) - (col + j)
              fp1_k(:,j,laymap,isti,m) = fp1_k(:,j,laymap,isti,m) + kkpack_k(lo:lo+n-1,col+j) * tlayer(j,lay)
              tlayer_k(j,lay) = tlayer_k(j,lay) + SUM(kkpack_k(lo:lo+n-1,col+j) * fp1(:,j,laymap,isti,m))

              lo = lo - n
              fp2_k(:,j,laymap,isti,m) = fp2_k(:,j,laymap,isti,m) + kkpack_k(lo:lo+n-1,col+n+j)

              lo = klu1 + (row + n + 1) - (col + j)
              fm1_k(:,j,laymap,isti,m) = fm1_k(:,j,laymap,isti,m) + kkpack_k(lo:lo+n-1,col+j) * tlayer(j,lay)
              tlayer_k(j,lay) = tlayer_k(j,lay) + SUM(kkpack_k(lo:lo+n-1,col+j) * fm1(:,j,laymap,isti,m))

              lo = lo - n
              fm2_k(:,j,laymap,isti,m) = fm2_k(:,j,laymap,isti,m) + kkpack_k(lo:lo+n-1,col+n+j)
            ENDDO
          ELSE
            row = (lay-1) * nstr + n
            col = (lay-1) * nstr
            ...
            IF (m == 0 .AND. inc_surface) THEN
              ...
              IF (dosolar) THEN
                ...
                z_k(n+1:nstr,laymap,isti,m) = z_k(n+1:nstr,laymap,isti,m) + &
                    tausun(lev) * tmpval * reflfac * muqw(:)
                reflfac_k = reflfac_k + &
                    tausun(lev) * tmpval * SUM(muqw(:) * dom_state(i)%z(n+1:nstr,laymap,isti,m))
                z_k(1:n,laymap,isti,m) = z_k(1:n,laymap,isti,m) - tausun(lev) * x_k(row+1:row+n)
                tausun_k(lev) = tausun_k(lev) - SUM(x_k(row+1:row+n) * (dom_state(i)%z(1:n,laymap,isti,m) - &
              ...
              ELSE
                ...
                opdep_k(lev) = opdep_k(lev) + &
                    reflfac * SUM(x_k(row+1:row+n)) * SUM(muqw(:) * dom_state(i)%y1(n+1:nstr,lay,ist))
                y1_k(n+1:nstr,lay) = y1_k(n+1:nstr,lay) + reflfac * muqw(:) * tmpval * opdep(lev)
                y0_k(n+1:nstr,lay) = y0_k(n+1:nstr,lay) + reflfac * muqw(:) * tmpval

                reflfac_k = reflfac_k + tmpval * SUM(muqw(:) * (dom_state(i)%y0(n+1:nstr,lay,ist) + &
                                                                dom_state(i)%y1(n+1:nstr,lay,ist) * opdep(lev)))

                opdep_k(lev) = opdep_k(lev) - SUM(x_k(row+1:row+n) * dom_state(i)%y1(1:n,lay,ist))
              ...
              DO j = n, 1, -1
                ...
                tlayer_k(j,lay) = tlayer_k(j,lay) + &
                    SUM(kkpack_k(lo:lo+n-1,col+j) * (fp1(:,j,laymap,isti,m) - surfterm(j)))
                surfterm_k(j) = surfterm_k(j) - SUM(kkpack_k(lo:lo+n-1,col+j)) * tlayer(j,lay)
                fp1_k(:,j,laymap,isti,m) = fp1_k(:,j,laymap,isti,m) + &
              ...
              IF (reflfac > 0._jprb) THEN
                DO j = n, 1, -1
                  fm2_k(:,j,laymap,isti,m) = fm2_k(:,j,laymap,isti,m) + surfterm_k(n+j) * reflfac * muqw(:)
                  fm1_k(:,j,laymap,isti,m) = fm1_k(:,j,laymap,isti,m) + surfterm_k(j) * reflfac * muqw(:)
            ...
            ELSE
              IF (dosolar) THEN
                tausun_k(lev) = tausun_k(lev) - SUM(x_k(row+1:row+n) * dom_state(i)%z(1:n,laymap,isti,m))
                z_k(1:n,laymap,isti,m) = z_k(1:n,laymap,isti,m) - x_k(row+1:row+n) * tausun(lev)
              ELSE
                opdep_k(lev) = opdep_k(lev) - SUM(x_k(row+1:row+n) * dom_state(i)%y1(1:n,lay,ist))
                y1_k(1:n,lay) = y1_k(1:n,lay) - x_k(row+1:row+n) * opdep(lev)
                y0_k(1:n,lay) = y0_k(1:n,lay) - x_k(row+1:row+n)
          ...
          IF (lay == 1) THEN
            ...
            col = 0
            IF (dosolar) THEN
              z_k(n+1:nstr,laymap,isti,m) = z_k(n+1:nstr,laymap,isti,m) - x_k(1:n)
            ELSE
            ...
            DO j = n, 1, -1
              lo = klu1 + (row + 1) - (col + j)
              fm1_k(:,j,laymap,isti,m) = fm1_k(:,j,laymap,isti,m) + kkpack_k(lo:lo+n-1,j)

              lo = lo - n
              tlayer_k(j,lay) = tlayer_k(j,lay) + SUM(kkpack_k(lo:lo+n-1,n+j) * fm2(:,j,laymap,isti,m))
              fm2_k(:,j,laymap,isti,m) = fm2_k(:,j,laymap,isti,m) + kkpack_k(lo:lo+n-1,n+j) * tlayer(j,lay)
            ENDDO
          ELSE
            row = (lay-2) * nstr + n
            col = (lay-1) * nstr
            IF (dosolar) THEN
              tausun_k(lev-1) = tausun_k(lev-1) + SUM(x_k(row+1:row+nstr) * dom_state(i)%z(:,laymap,isti,m))
              z_k(:,laymap,isti,m) = z_k(:,laymap,isti,m) + x_k(row+1:row+nstr) * tausun(lev-1)
            ELSE
              opdep_k(lev-1) = opdep_k(lev-1) + SUM(x_k(row+1:row+nstr) * dom_state(i)%y1(:,lay,ist))
              y1_k(:,lay) = y1_k(:,lay) + x_k(row+1:row+nstr) * opdep(lev-1)
              y0_k(:,lay) = y0_k(:,lay) + x_k(row+1:row+nstr)
            ENDIF
            DO j = n, 1, -1
              lo = klu1 + (row + 1) - (col + j)
              fp1_k(:,j,laymap,isti,m) = fp1_k(:,j,laymap,isti,m) - kkpack_k(lo:lo+n-1,col+j)

              lo = lo - n
              tlayer_k(j,lay) = tlayer_k(j,lay) - &
                  SUM(kkpack_k(lo:lo+n-1,col+n+j) * fp2(:,j,laymap,isti,m))
              fp2_k(:,j,laymap,isti,m) = fp2_k(:,j,laymap,isti,m) - &
                  kkpack_k(lo:lo+n-1,col+n+j) * tlayer(j,lay)

              lo = klu1 + (row + n + 1) - (col + j)
              fm1_k(:,j,laymap,isti,m) = fm1_k(:,j,laymap,isti,m) - kkpack_k(lo:lo+n-1,col+j)

              lo = lo - n
              tlayer_k(j,lay) = tlayer_k(j,lay) - &
                  SUM(kkpack_k(lo:lo+n-1,col+n+j) * fm2(:,j,laymap,isti,m))
              fm2_k(:,j,laymap,isti,m) = fm2_k(:,j,laymap,isti,m) - &
                  kkpack_k(lo:lo+n-1,col+n+j) * tlayer(j,lay)
            ENDDO
          ENDIF
          ...
          ! Calculate layer "transmittances" with opdeps scaled by eigenvalues
          ! --------------------------------------------------------------------------
          eval_k(:,laymap,isti,m) = eval_k(:,laymap,isti,m) - &
              tlayer_k(:,lay) * tlayer(:,lay) * ftau(lay) * profiles_dom(ist,i)%layerod(lay)
          tmpval = SUM(tlayer_k(:,lay) * tlayer(:,lay) * dom_state(i)%eval(:,laymap,isti,m))
          profiles_dom_k(ist,i)%layerod(lay) = profiles_dom_k(ist,i)%layerod(lay) - tmpval * ftau(lay)
          ftau_k(lay) = ftau_k(lay) - tmpval * profiles_dom(ist,i)%layerod(lay)
          ...
          IF (fssa(lay) > 0._jprb) THEN
            ...
            IF (dothermal) THEN
              ...
              phasesat_k(:) = phasesat_k(:) + 0.5_jprb * fpmupi1_k(lay) * dom_state(i)%y1(:,lay,ist)
              y1_k(:,lay) = y1_k(:,lay) + 0.5_jprb * fpmupi1_k(lay) * phasesat(:)

              b0_k(lay) = b0_k(lay) + fpmupi0_k(lay)
              phasesat_k(:) = phasesat_k(:) + 0.5_jprb * fpmupi0_k(lay) * dom_state(i)%y0(:,lay,ist)
            ...
            ! Interpolate HS and PI for final radiance calculation
            ! --------------------------------------------------------------------------
!             IF (.NOT. done_lay_str_azi_k(laymap,isti,m)) THEN

              ! Particular integral - solar source term, done once per layer (applies to all cloud streams)
              IF (dosolar) THEN
                legcoef_k(m:nmom,laymap,isti) = legcoef_k(m:nmom,laymap,isti) + &
                    sunsrcfac * fpmupi_k(laymap,isti,m) * legpolymusat(m:nmom,0,m) * legpolymusun(m:nmom,0,m)
                phasesat_k(:) = phasesat_k(:) + &
                    0.5_jprb * fpmupi_k(laymap,isti,m) * dom_state(i)%z(:,laymap,isti,m)
                z_k(:,laymap,isti,m) = z_k(:,laymap,isti,m) + &
                    0.5_jprb * fpmupi_k(laymap,isti,m) * phasesat(:)
              ENDIF

              ! Homogenous part - diffuse radiation, done once per layer (applies to all cloud streams)
              DO j = n, 1, -1
                phasesat_k(n+1:nstr) = phasesat_k(n+1:nstr) + &
                    0.5_jprb * fpmuhsdn_k(j,laymap,isti,m) * fm2(:,j,laymap,isti,m)
                fm2_k(:,j,laymap,isti,m) = fm2_k(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsdn_k(j,laymap,isti,m) * phasesat(n+1:nstr)
                phasesat_k(1:n) = phasesat_k(1:n) + &
                    0.5_jprb * fpmuhsdn_k(j,laymap,isti,m) * fp2(:,j,laymap,isti,m)
                fp2_k(:,j,laymap,isti,m) = fp2_k(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsdn_k(j,laymap,isti,m) * phasesat(1:n)
                phasesat_k(n+1:nstr) = phasesat_k(n+1:nstr) + &
                    0.5_jprb * fpmuhsup_k(j,laymap,isti,m) * fm1(:,j,laymap,isti,m)
                fm1_k(:,j,laymap,isti,m) = fm1_k(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsup_k(j,laymap,isti,m) * phasesat(n+1:nstr)
                phasesat_k(1:n) = phasesat_k(1:n) + &
                    0.5_jprb * fpmuhsup_k(j,laymap,isti,m) * fp1(:,j,laymap,isti,m)
                fp1_k(:,j,laymap,isti,m) = fp1_k(:,j,laymap,isti,m) + &
                    0.5_jprb * fpmuhsup_k(j,laymap,isti,m) * phasesat(1:n)
              ENDDO
...
!               hh_k = - MATMUL(hhinvt, &
!                   MATMUL(y0_k(:,lay:lay), TRANSPOSE(dom_state(i)%y0(:,lay:lay,ist))))
...
!               y0_k(:,lay) = MATMUL(hhinvt, y0_k(:,lay))
              ...
              hh_k = -MATMUL(y0_k(:,lay:lay), TRANSPOSE(dom_state(i)%y0(:,lay:lay,ist)))
              DO j = 1, nstr
                CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, hh_k(:,j), nstri, info)
                IF (info /= 0) THEN
...
!                   MATMUL(y1_k(:,lay:lay), TRANSPOSE(dom_state(i)%y1(:,lay:lay,ist))))

!               y1_k(:,lay) = MATMUL(hhinvt, y1_k(:,lay))


              hhtmp_k = -MATMUL(y1_k(:,lay:lay), TRANSPOSE(dom_state(i)%y1(:,lay:lay,ist)))
              DO j = 1, nstr
                CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, hhtmp_k(:,j), nstri, info)
                IF (info /= 0) THEN
                  WRITE(msg,'(a,3i5)') 'DOM error: solving K PI thermal y1, DGETRS error, info = ', info, i, lay
                  err = errorstatus_fatal
                  THROWM(err.NE.0,msg)
                ENDIF
                hh_k(:,j) = hh_k(:,j) + hhtmp_k(:,j)
              ENDDO
...
!                 ! No accumulation required
!                 hh_k = - MATMUL(hhinvt, &
!                     MATMUL(z_k(:,laymap:laymap,isti,m), TRANSPOSE(dom_state(i)%z(:,laymap:laymap,isti,m))))
! 
!                 z_k(:,laymap,isti,m) = MATMUL(hhinvt, z_k(:,laymap,isti,m))
                ...
                hh_k = -MATMUL(z_k(:,laymap:laymap,isti,m), TRANSPOSE(dom_state(i)%z(:,laymap:laymap,isti,m)))
                DO j = 1, nstr
                  CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, hh_k(:,j), nstri, info)
                  IF (info /= 0) THEN
                ...
                CALL DGETRS('Transpose', nstri, 1, hh, nstri, ipiv2, z_k(:,laymap,isti,m), nstri, info)
                IF (info /= 0) THEN
                  WRITE(msg,'(a,3i5)') 'DOM error: solving K PI solar, DGETRS error, info = ', info, i, lay
                  err = errorstatus_fatal
                  THROWM(err.NE.0,msg)
                ...
                DO j = n, 1, -1
                  legcoef_k(m:nmom,laymap,isti) = legcoef_k(m:nmom,laymap,isti) + &
                      sunsrcfac * z_k(n+j,laymap,isti,m) * legpolymuq(m:nmom,n+j,m) * legpolymusun(m:nmom,0,m)
                  legcoef_k(m:nmom,laymap,isti) = legcoef_k(m:nmom,laymap,isti) + &
                      sunsrcfac * z_k(j,laymap,isti,m) * legpolymuq(m:nmom,j,m) * legpolymusun(m:nmom,0,m)
...
! ** Recompute xm or store above? **
              ...
              xm_k = 0.5_jprb * (fp1_k(:,:,laymap,isti,m) - fm1_k(:,:,laymap,isti,m))
              xp_k = 0.5_jprb * (fp1_k(:,:,laymap,isti,m) + fm1_k(:,:,laymap,isti,m))

              sp_k = 0._jprb
              DO j = n, 1, -1
                eval_k(j,laymap,isti,m) = eval_k(j,laymap,isti,m) - &
                    SUM(xm_k(:,j) * xm(:,j)) / dom_state(i)%eval(j,laymap,isti,m)
                sp_k = sp_k + MATMUL(xm_k(:,j:j), TRANSPOSE(xp(:,j:j))) / dom_state(i)%eval(j,laymap,isti,m)
                xp_k(:,j) = xp_k(:,j) + MATMUL(TRANSPOSE(sp), xm_k(:,j)) / dom_state(i)%eval(j,laymap,isti,m)
              ENDDO

              ! Calculate the required eigenvalues (+/- each one gives nstr evals)
              eval_k(:,laymap,isti,m) = 0.5_jprb * eval_k(:,laymap,isti,m) / dom_state(i)%eval(:,laymap,isti,m)
...
!               bpm_k = 0._jprb ! Not accumulated in ASYMTX_AD
              ...
              CALL ASYMTX_AD(err, n, xp, eval, xp_k, eval_k(:,laymap,isti,m), bpm_k)
              IF (err /= 0) THEN
...
!             ENDIF ! not done_lay_str_azi_k
            ...
            ! For scattering layers, only do calculation if we don't already have the results
            ! --------------------------------------------------------------------------
!             IF (dothermal .OR. .NOT. done_lay_str_azi_k(laymap,isti,m)) THEN
              ...
              IF (m == 0) THEN  ! This calculation applies to all azi components
                DO l = nmom, 0, -1
                  f_lay_k = f_lay_k + legcoef_k(l,laymap,isti) * &
                  ...
                  trans_scatt_ir_dyn_k%phasefn(i)%legcoef(l,isti,laymap) = &
                      trans_scatt_ir_dyn_k%phasefn(i)%legcoef(l,isti,laymap) + &
                      legcoef_k(l,laymap,isti) * fssa(lay) / (1._jprb - f_lay)
                  fssa_k(lay) = fssa_k(lay) + legcoef_k(l,laymap,isti) * &
...
!             done_lay_str_azi_k(laymap,isti,m) = .TRUE.
          ...
          IF (m == 0) THEN  ! This calculation applies to all azi components
            ...
            profiles_dom_k(ist,i)%layerod(lay) = profiles_dom_k(ist,i)%layerod(lay) + &
                opdep_k(lev) * ftau(lay)
            ftau_k(lay) = ftau_k(lay) + opdep_k(lev) * profiles_dom(ist,i)%layerod(lay)
          ...
          IF (m == 0) THEN
            IF (laymap > 0) THEN
              ftau_k(lay) = ftau_k(lay) - fssa_k(lay) * (1._jprb - f_lay) * ssa(isti,laymap,i) / ftau(lay)**2
              ssa_k(isti,laymap,i) = ssa_k(isti,laymap,i) + fssa_k(lay) * (1._jprb - f_lay) / ftau(lay)
              f_lay_k = f_lay_k - fssa_k(lay) * ssa(isti,laymap,i) / ftau(lay)

              f_lay_k = f_lay_k - ftau_k(lay) * ssa(isti,laymap,i)
              ssa_k(isti,laymap,i) = ssa_k(isti,laymap,i) - ftau_k(lay) * f_lay
              trans_scatt_ir_dyn_k%phasefn(i)%legcoef(nstr,isti,laymap) = &
                  trans_scatt_ir_dyn_k%phasefn(i)%legcoef(nstr,isti,laymap) + f_lay_k / (2._jprb * nstr + 1._jprb)
!             ELSE
!               f_lay_k     = 0._jprb
!               ftau_k(lay) = 0._jprb
!               fssa_k(lay) = 0._jprb
