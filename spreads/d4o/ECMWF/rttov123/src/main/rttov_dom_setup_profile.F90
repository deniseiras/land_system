...
!!   structure to decrease run-time without any impact on radiances. (This
!!   ignores Rayleigh scattering).
!!
!!   For thermal simulations this is not possible because the emission
!!   source term depends on the layer temperature. However for some IR
...
!!   here for each cloud stream (column) and channel so the members of the
...
!! @param[in]     ircld                  infomration on cloud streams (columns)
...
! profiles_dom(maxnstreams,nchanprof):  (maxnstreams refers to cloud columns, not DOM streams)
...
!   %laymap(nlayers)              # index of user layer for each DOM layer containing scattering material
  ...
  DO i = 1, nchanprof
    ...
    DO isti = 0, su
      layerod(isti,:,i) = clropdep(:) + &
                          transmission_scatt_ir%opdpabs(isti,:,i) + &
                          transmission_scatt_ir%opdpsca(isti,:,i)
    ENDDO
    ...
    ! ------------------------------------------------------------------------
    ! Loop over cloud streams to determine the number of layers to pass into
    ! DOM for each stream independently
    ! ------------------------------------------------------------------------
    DO ist = 0, ircld%nstream(prof)
      ...
      DO lay = 1, nlayers
        ...
        absopdep(lay) = transmission_scatt_ir%opdpabs(isti,lay,i)
        scaopdep(lay) = transmission_scatt_ir%opdpsca(isti,lay,i)
      ENDDO
      ...
      ! (assume clropdep is almost entirely due to absorption, but in reality
      ! it does include Rayleigh extinction which includes some scattering)
      ! If the profile is optically thick set surface to .FALSE.
      opdep = totopdep_init
      profiles_dom(ist,i)%surface = .TRUE.
      DO lay = 1, maxnlayers
        ...
        IF (opts%rt_ir%dom_opdep_threshold > 0._jprb) THEN
          IF (opdep > opts%rt_ir%dom_opdep_threshold) THEN
            profiles_dom(ist,i)%surface = .FALSE.
            maxnlayers = lay
            EXIT
          ENDIF
        ENDIF
      ...
      ! domnlayers is the number of layers for the DOM calculation for this stream/channel
      profiles_dom(ist,i)%nlayers = domnlayers

      ! ------------------------------------------------------------------------
      ! Allocate DOM profile arrays
      ! ------------------------------------------------------------------------

      ALLOCATE(profiles_dom(ist,i)%layerod(domnlayers), &
               profiles_dom(ist,i)%laymap(domnlayers))

      ! ------------------------------------------------------------------------
      ! Populate arrays
      ! ------------------------------------------------------------------------

      profiles_dom(ist,i)%laymap(:) = laymaptouser(1:domnlayers)

      ! Deal with all full layers above surface
      profiles_dom(ist,i)%layerod(:) = 0._jprb
      ...
      profiles_dom(ist,i)%layerod(1) = totopdep_init

      DO lay = 1, maxnlayers
        profiles_dom(ist,i)%layerod(laymaptodom(lay)) = profiles_dom(ist,i)%layerod(laymaptodom(lay)) + &
                                                        totopdep(lay)
      ENDDO

      IF (profiles_dom(ist,i)%surface) THEN
        ! Handle partial layer above surface - either added to last layer from above or treated as new (last) layer
        profiles_dom(ist,i)%layerod(domnlayers) = profiles_dom(ist,i)%layerod(domnlayers) + &
                                                  surftotopdep
      ENDIF

      IF (.NOT. dosolar) THEN
        WHERE (profiles_dom(ist,i)%layerod(:) < min_od) profiles_dom(ist,i)%layerod(:) = min_od
      ENDIF
