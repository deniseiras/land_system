...
!INTF_END
  ...
  DO i = 1, nchanprof
    ...
    DO ist = ircld%nstream(prof), 0, -1
      ...
      IF (domnlayers /= profiles_dom(ist,i)%nlayers) THEN
      ...
      IF (.NOT. dosolar) THEN
        WHERE (profiles_dom(ist,i)%layerod(:) <= min_od) profiles_dom_ad(ist,i)%layerod(:) = 0._jprb
      ENDIF

      IF (profiles_dom(ist,i)%surface) THEN
        surftotopdep_ad = surftotopdep_ad + profiles_dom_ad(ist,i)%layerod(domnlayers)
      ENDIF

      DO lay = maxnlayers, 1, -1
        totopdep_ad(lay) = totopdep_ad(lay) + profiles_dom_ad(ist,i)%layerod(laymaptodom(lay))
      ENDDO

      totopdep_init_ad = totopdep_init_ad + profiles_dom_ad(ist,i)%layerod(1)
...
!       profiles_dom_ad(ist,i)%layerod(:) = 0._jprb
      ...
      DO lay = nlayers, 1, -1
        ...
        transmission_scatt_ir_ad%opdpsca(isti,lay,i) = &
            transmission_scatt_ir_ad%opdpsca(isti,lay,i) + scaopdep_ad(lay)
        transmission_scatt_ir_ad%opdpabs(isti,lay,i) = &
            transmission_scatt_ir_ad%opdpabs(isti,lay,i) + absopdep_ad(lay)
      ENDDO
...
!     ssa_ad(:,:,i) = 0._jprb
    ...
    DO isti = 0, su
      transmission_scatt_ir_ad%opdpsca(isti,:,i) = &
          transmission_scatt_ir_ad%opdpsca(isti,:,i) + layerod_ad(isti,:)
      transmission_scatt_ir_ad%opdpabs(isti,:,i) = &
          transmission_scatt_ir_ad%opdpabs(isti,:,i) + layerod_ad(isti,:)
      clropdep_ad(:) = clropdep_ad(:) + layerod_ad(isti,:)
    ENDDO
