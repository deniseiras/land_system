...
!INTF_END
  ...
  DO i = 1, nchanprof
    ...
    DO ist = ircld%nstream(prof), 0, -1
      ...
      IF (.NOT. dosolar) THEN
        WHERE (profiles_dom(ist,i)%layerod(:) <= min_od) profiles_dom_k(ist,i)%layerod(:) = 0._jprb
      ENDIF

      IF (profiles_dom(ist,i)%surface) THEN
        surftotopdep_k = surftotopdep_k + profiles_dom_k(ist,i)%layerod(domnlayers)
      ENDIF

      DO lay = maxnlayers, 1, -1
        totopdep_k(lay) = totopdep_k(lay) + profiles_dom_k(ist,i)%layerod(laymaptodom(lay))
      ENDDO

      totopdep_init_k = totopdep_init_k + profiles_dom_k(ist,i)%layerod(1)
...
!       profiles_dom_k(ist,i)%layerod(:) = 0._jprb
      ...
      DO lay = nlayers, 1, -1
        ...
        transmission_scatt_ir_k%opdpsca(isti,lay,i) = &
            transmission_scatt_ir_k%opdpsca(isti,lay,i) + scaopdep_k(lay)
        transmission_scatt_ir_k%opdpabs(isti,lay,i) = &
            transmission_scatt_ir_k%opdpabs(isti,lay,i) + absopdep_k(lay)
      ENDDO
...
!     ssa_k(:,:,i) = 0._jprb
    ...
    DO isti = 0, su
      transmission_scatt_ir_k%opdpsca(isti,:,i) = &
          transmission_scatt_ir_k%opdpsca(isti,:,i) + layerod_k(isti,:)
      transmission_scatt_ir_k%opdpabs(isti,:,i) = &
          transmission_scatt_ir_k%opdpabs(isti,:,i) + layerod_k(isti,:)
      clropdep_k(:) = clropdep_k(:) + layerod_k(isti,:)
    ENDDO
