...
!INTF_END
  ...
  DO i = 1, nchanprof
    ...
    DO isti = 0, su
      layerod_tl(isti,:) = clropdep_tl(:) + &
                           transmission_scatt_ir_tl%opdpabs(isti,:,i) + &
                           transmission_scatt_ir_tl%opdpsca(isti,:,i)
    ENDDO
    ...
    DO ist = 0, ircld%nstream(prof)
      ...
      DO lay = 1, nlayers
        ...
        absopdep_tl(lay) = transmission_scatt_ir_tl%opdpabs(isti,lay,i)
        scaopdep_tl(lay) = transmission_scatt_ir_tl%opdpsca(isti,lay,i)
      ENDDO
      ...
      profiles_dom_tl(ist,i)%layerod(:) = 0._jprb
      ...
      profiles_dom_tl(ist,i)%layerod(1) = totopdep_init_tl

      DO lay = 1, maxnlayers
        profiles_dom_tl(ist,i)%layerod(laymaptodom(lay)) = profiles_dom_tl(ist,i)%layerod(laymaptodom(lay)) + &
                                                           totopdep_tl(lay)
      ENDDO

      IF (profiles_dom(ist,i)%surface) THEN
        ! Handle partial layer above surface - either added to last layer from above or treated as new (last) layer
        profiles_dom_tl(ist,i)%layerod(domnlayers) = profiles_dom_tl(ist,i)%layerod(domnlayers) + &
                                                     surftotopdep_tl
      ENDIF

      IF (.NOT. dosolar) THEN
        WHERE (profiles_dom(ist,i)%layerod(:) <= min_od) profiles_dom_tl(ist,i)%layerod(:) = 0._jprb
      ENDIF
