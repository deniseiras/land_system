        ...
        DO lay = 1, nlayers
          ...
          IF (m == 0) THEN
            IF (laymap > 0) THEN
              ...
              f_lay_tl     = trans_scatt_ir_dyn_tl%phasefn(i)%legcoef(nstr,isti,laymap) / (2._jprb * nstr + 1._jprb)
              ftau_tl(lay) = - ssa_tl(isti,laymap,i) * f_lay - &
                               ssa(isti,laymap,i) * f_lay_tl
              fssa_tl(lay) = - f_lay_tl * ssa(isti,laymap,i) / ftau(lay) + &
                             (1._jprb - f_lay) * ssa_tl(isti,laymap,i) / ftau(lay) - &
                             ftau_tl(lay) * (1._jprb - f_lay) * ssa(isti,laymap,i) / ftau(lay)**2
            ELSE
              f_lay        = 0._jprb
              ftau(lay)    = 1._jprb
              fssa(lay)    = 0._jprb
          ...
          IF (m == 0) THEN  ! This calculation applies to all azi components
            ...
            opdep_tl(lev) = opdep_tl(lev-1) + ftau_tl(lay) * profiles_dom(ist,i)%layerod(lay) + &
                                              ftau(lay) * profiles_dom_tl(ist,i)%layerod(lay)
            ...
            IF (dothermal .OR. .NOT. done_lay_str_azi(laymap,isti,m)) THEN
              ...
              IF (m == 0) THEN  ! This calculation applies to all azi components
                DO l = 0, nmom
                  legcoef(l,laymap,isti) = fssa(lay) * (trans_scatt_ir_dyn%phasefn(i)%legcoef(l,isti,laymap) - &
                                           ...
                                           (1._jprb - f_lay)
                  legcoef_tl(l,laymap,isti) = fssa_tl(lay) * (trans_scatt_ir_dyn%phasefn(i)%legcoef(l,isti,laymap) - &
                      (2._jprb * l + 1._jprb) * f_lay) / &
                      (1._jprb - f_lay) + &
                      fssa(lay) * (trans_scatt_ir_dyn_tl%phasefn(i)%legcoef(l,isti,laymap) - &
                      (2._jprb * l + 1._jprb) * f_lay_tl) / &
                      (1._jprb - f_lay) + &
                      f_lay_tl * fssa(lay) * (trans_scatt_ir_dyn%phasefn(i)%legcoef(l,isti,laymap) - &
                      (2._jprb * l + 1._jprb) * f_lay) / &
                      (1._jprb - f_lay)**2
                ENDDO
              ENDIF
              ...
              DO j = 1, n
                DO k = j, n
                  ...
                  phase_fn_tl(k,j)   = SUM(legcoef_tl(m:nmom,laymap,isti) * legpolymuq(m:nmom,k,m) * legpolymuq(m:nmom,j,m))
                  phase_fn_tl(k,n+j) = SUM(legcoef_tl(m:nmom,laymap,isti) * legpolymuq(m:nmom,k,m) * legpolymuq(m:nmom,n+j,m))
              ...
              DO j = 1, n
                ...
                phasesat_tl(j) = muw(j) * SUM(legcoef_tl(m:nmom,laymap,isti) * &
                                              legpolymusat(m:nmom,0,m) * legpolymuq(m:nmom,j,m))
              ENDDO
              DO j = n+1, nstr
                phasesat(j) = muw(j-n) * SUM(legcoef(m:nmom,laymap,isti) * &
                                             legpolymusat(m:nmom,0,m) * legpolymuq(m:nmom,j,m))
                phasesat_tl(j) = muw(j-n) * SUM(legcoef_tl(m:nmom,laymap,isti) * &
                                                legpolymusat(m:nmom,0,m) * legpolymuq(m:nmom,j,m))
              ENDDO
...
!               bpmsave_tl = bpm_tl
              ...
              CALL ASYMTX_TL(err, n, xp, eval, bpm_tl, xp_tl, eval_tl(:,laymap,isti,m))
              IF (err /= 0) THEN
              ...
              eval_tl(:,laymap,isti,m) = 0.5_jprb * eval_tl(:,laymap,isti,m) / dom_state(i)%eval(:,laymap,isti,m)

              DO j = 1, n
                xm(:,j) = MATMUL(sp, xp(:,j)) / dom_state(i)%eval(j,laymap,isti,m)
                xm_tl(:,j) = (MATMUL(sp_tl, xp(:,j)) + MATMUL(sp, xp_tl(:,j)) - &
                             eval_tl(j,laymap,isti,m) * xm(:,j)) / dom_state(i)%eval(j,laymap,isti,m)
              ENDDO
              ...
              fp1_tl(:,:,laymap,isti,m) = 0.5_jprb * (xp_tl + xm_tl)
              fm1_tl(:,:,laymap,isti,m) = 0.5_jprb * (xp_tl - xm_tl)
...
!                 IF (.NOT. do_order_n_pi) THEN
                  ...
                  DO j = 1, n
                    ...
                    z_tl(j,laymap,isti,m)   = sunsrcfac * SUM(legcoef_tl(m:nmom,laymap,isti) * &
                                                              legpolymuq(m:nmom,j,m) * legpolymusun(m:nmom,0,m))
                    z_tl(n+j,laymap,isti,m) = sunsrcfac * SUM(legcoef_tl(m:nmom,laymap,isti) * &
                  ...
                  z_tl(:,laymap,isti,m) = z_tl(:,laymap,isti,m) - MATMUL(hh_tl, dom_state(i)%z(:,laymap,isti,m))
                  CALL DGETRS('No transpose', nstri, 1, hh, nstri, ipiv2, z_tl(:,laymap,isti,m), nstri, info)
                  IF (info /= 0) THEN
...
!                     so1_tl = SUM(legcoef_tl(m:nmom,laymap,isti) * legpolymuq(m:nmom,j,m) * legpolymusun(m:nmom,0,m))
!                     so2 = SUM(legcoef(m:nmom,laymap,isti) * legpolymuq(m:nmom,n+j,m) * legpolymusun(m:nmom,0,m))
!                     so2_tl = SUM(legcoef_tl(m:nmom,laymap,isti) * legpolymuq(m:nmom,n+j,m) * legpolymusun(m:nmom,0,m))
...
!                   z_tl(1:n,laymap,isti,m)      = 0.5_jprb * (gp_tl + som_tl)
!                   z_tl(n+1:nstr,laymap,isti,m) = 0.5_jprb * (gp_tl - som_tl)
! 
!                 ENDIF
            ...
            IF (dothermal) THEN
              ...
              y1_tl(:,lay) = y1_tl(:,lay) - MATMUL(hh_tl, dom_state(i)%y1(:,lay,ist))
              ...
              y0_tl(:,lay) = y0_tl(:,lay) - MATMUL(hh_tl, dom_state(i)%y0(:,lay,ist))
            ...
            IF (.NOT. done_lay_str_azi(laymap,isti,m)) THEN
              ...
              DO j = 1, n
                ...
                fpmuhsup_tl(j,laymap,isti,m) = 0.5_jprb * (SUM(fp1_tl(:,j,laymap,isti,m) * phasesat(1:n)) + &
                                                           SUM(fp1(:,j,laymap,isti,m) * phasesat_tl(1:n)) + &
                                                           SUM(fm1_tl(:,j,laymap,isti,m) * phasesat(n+1:nstr)) + &
                                                           SUM(fm1(:,j,laymap,isti,m) * phasesat_tl(n+1:nstr)))

                fpmuhsdn(j,laymap,isti,m) = 0.5_jprb * (SUM(fp2(:,j,laymap,isti,m) * phasesat(1:n)) + &
                                                        SUM(fm2(:,j,laymap,isti,m) * phasesat(n+1:nstr)))

                fpmuhsdn_tl(j,laymap,isti,m) = 0.5_jprb * (SUM(fp2_tl(:,j,laymap,isti,m) * phasesat(1:n)) + &
                                                           SUM(fp2(:,j,laymap,isti,m) * phasesat_tl(1:n)) + &
                                                           SUM(fm2_tl(:,j,laymap,isti,m) * phasesat(n+1:nstr)) + &
                                                           SUM(fm2(:,j,laymap,isti,m) * phasesat_tl(n+1:nstr)))
              ENDDO
              ...
              IF (dosolar) THEN
                ...
                fpmupi_tl(laymap,isti,m) = 0.5_jprb * SUM(z_tl(:,laymap,isti,m) * phasesat(:)) + &
                                           0.5_jprb * SUM(dom_state(i)%z(:,laymap,isti,m) * phasesat_tl(:)) + &
                                           sunsrcfac * SUM(legcoef_tl(m:nmom,laymap,isti) * &
                                                           legpolymusat(m:nmom,0,m) * legpolymusun(m:nmom,0,m))
              ENDIF
          ...
          tlayer_tl(:,lay) = -tlayer(:,lay) * &
                              (ftau_tl(lay) * profiles_dom(ist,i)%layerod(lay) * dom_state(i)%eval(:,laymap,isti,m) + &
                               ftau(lay) * profiles_dom_tl(ist,i)%layerod(lay) * dom_state(i)%eval(:,laymap,isti,m) + &
                               ftau(lay) * profiles_dom(ist,i)%layerod(lay) * eval_tl(:,laymap,isti,m))
          ...
          IF (lay == 1) THEN
            ...
            DO j = 1, n
              ...
              kkpack_tl(lo:lo+n-1,j)   = fm1_tl(:,j,laymap,isti,m)
              lo = lo - n
              kkpack(lo:lo+n-1,n+j)    = fm2(:,j,laymap,isti,m) * tlayer(j,lay)
              kkpack_tl(lo:lo+n-1,n+j) = fm2_tl(:,j,laymap,isti,m) * tlayer(j,lay) + &
                                         fm2(:,j,laymap,isti,m) * tlayer_tl(j,lay)
            ENDDO
            IF (dosolar) THEN
              x_tl(1:n) = -z_tl(n+1:nstr,laymap,isti,m)
            ELSE
          ...
          ELSE
            ...
            DO j = 1, n
              ...
              kkpack_tl(lo:lo+n-1,col+j)   = -fp1_tl(:,j,laymap,isti,m)
              lo = lo - n
              kkpack(lo:lo+n-1,col+n+j)    = -fp2(:,j,laymap,isti,m) * tlayer(j,lay)
              kkpack_tl(lo:lo+n-1,col+n+j) = -fp2_tl(:,j,laymap,isti,m) * tlayer(j,lay) - &
                                              fp2(:,j,laymap,isti,m) * tlayer_tl(j,lay)
              lo = klu1 + (row + n + 1) - (col + j)
              kkpack(lo:lo+n-1,col+j)      = -fm1(:,j,laymap,isti,m)
              kkpack_tl(lo:lo+n-1,col+j)   = -fm1_tl(:,j,laymap,isti,m)
              lo = lo - n
              kkpack(lo:lo+n-1,col+n+j)    = -fm2(:,j,laymap,isti,m) * tlayer(j,lay)
              kkpack_tl(lo:lo+n-1,col+n+j) = -fm2_tl(:,j,laymap,isti,m) * tlayer(j,lay) - &
                                              fm2(:,j,laymap,isti,m) * tlayer_tl(j,lay)
            ENDDO
            IF (dosolar) THEN
              x_tl(row+1:row+nstr) = x_tl(row+1:row+nstr) + z_tl(:,laymap,isti,m) * tausun(lev-1) + &
                                                            dom_state(i)%z(:,laymap,isti,m) * tausun_tl(lev-1)
            ELSE
              x_tl(row+1:row+nstr) = x_tl(row+1:row+nstr) + y0_tl(:,lay) + &
                                     y1_tl(:,lay) * opdep(lev-1) + dom_state(i)%y1(:,lay,ist) * opdep_tl(lev-1)
            ENDIF
          ENDIF
          ...
          IF (lay < nlayers) THEN
            ...
            DO j = 1, n
              ...
              kkpack_tl(lo:lo+n-1,col+j)   = fp1_tl(:,j,laymap,isti,m) * tlayer(j,lay) + &
                                             fp1(:,j,laymap,isti,m) * tlayer_tl(j,lay)
              lo = lo - n
              kkpack(lo:lo+n-1,col+n+j)    = fp2(:,j,laymap,isti,m)
              kkpack_tl(lo:lo+n-1,col+n+j) = fp2_tl(:,j,laymap,isti,m)
              lo = klu1 + (row + n + 1) - (col + j)
              kkpack(lo:lo+n-1,col+j)      = fm1(:,j,laymap,isti,m) * tlayer(j,lay)
              kkpack_tl(lo:lo+n-1,col+j)   = fm1_tl(:,j,laymap,isti,m) * tlayer(j,lay) + &
                                             fm1(:,j,laymap,isti,m) * tlayer_tl(j,lay)
              lo = lo - n
              kkpack(lo:lo+n-1,col+n+j)    = fm2(:,j,laymap,isti,m)
              kkpack_tl(lo:lo+n-1,col+n+j) = fm2_tl(:,j,laymap,isti,m)
            ENDDO
            IF (dosolar) THEN
              x_tl(row+1:row+nstr) = -z_tl(:,laymap,isti,m) * tausun(lev) - dom_state(i)%z(:,laymap,isti,m) * tausun_tl(lev)
            ELSE
              x_tl(row+1:row+nstr) = -y0_tl(:,lay) - y1_tl(:,lay) * opdep(lev) - dom_state(i)%y1(:,lay,ist) * opdep_tl(lev)
            ENDIF
          ELSE
            row = (lay-1) * nstr + n
            col = (lay-1) * nstr
            ...
            IF (m == 0 .AND. inc_surface) THEN
              IF (reflfac > 0._jprb) THEN
                DO j = 1, n
                  ...
                  surfterm_tl(j)   = reflfac_tl * surfterm(j) / reflfac + &
                                     reflfac * SUM(muqw(:) * fm1_tl(:,j,laymap,isti,m))
                  surfterm_tl(n+j) = reflfac_tl * surfterm(n+j) / reflfac + &
                                     reflfac * SUM(muqw(:) * fm2_tl(:,j,laymap,isti,m))
                ENDDO
              ELSE
                surfterm    = 0._jprb
                surfterm_tl = 0._jprb
              ...
              DO j = 1, n
                ...
                kkpack_tl(lo:lo+n-1,col+j)   = (fp1_tl(:,j,laymap,isti,m) - surfterm_tl(j)) * tlayer(j,lay) + &
                                               (fp1(:,j,laymap,isti,m) - surfterm(j)) * tlayer_tl(j,lay)
                lo = lo - n
                kkpack(lo:lo+n-1,col+n+j)    = fp2(:,j,laymap,isti,m) - surfterm(n+j)
                kkpack_tl(lo:lo+n-1,col+n+j) = fp2_tl(:,j,laymap,isti,m) - surfterm_tl(n+j)
              ENDDO

              IF (dosolar) THEN
                tmpval = SUM(muqw(:) * dom_state(i)%z(n+1:nstr,laymap,isti,m))
                x_tl(row+1:row+n) = -tausun_tl(lev) * (dom_state(i)%z(1:n,laymap,isti,m) - &
                                    reflfac * tmpval - (albedo / pi) * solar_spectrum(i) * musun) - &
                                    tausun(lev) * (z_tl(1:n,laymap,isti,m) - reflfac_tl * tmpval - &
                                    reflfac * SUM(muqw(:) * z_tl(n+1:nstr,laymap,isti,m)) - &
                                    (albedo_tl / pi) * solar_spectrum(i) * musun)
              ELSE
                x_tl(row+1:row+n) = -(y0_tl(1:n,lay) + y1_tl(1:n,lay) * opdep(lev) + &
                                      dom_state(i)%y1(1:n,lay,ist) * opdep_tl(lev) - &
                                    reflfac_tl * SUM(muqw(:) * (dom_state(i)%y0(n+1:nstr,lay,ist) + &
                                                     dom_state(i)%y1(n+1:nstr,lay,ist) * opdep(lev))) - &
                                    reflfac * SUM(muqw(:) * (y0_tl(n+1:nstr,lay) + &
                                                             y1_tl(n+1:nstr,lay) * opdep(lev) + &
                                                             dom_state(i)%y1(n+1:nstr,lay,ist) * opdep_tl(lev))) - &
            ...
            ELSE
              ...
              ENDDO
              IF (dosolar) THEN
                x_tl(row+1:row+n) = -z_tl(1:n,laymap,isti,m) * tausun(lev) - &
                                     dom_state(i)%z(1:n,laymap,isti,m) * tausun_tl(lev)
              ELSE
                x_tl(row+1:row+n) = -y0_tl(1:n,lay) - y1_tl(1:n,lay) * opdep(lev) - dom_state(i)%y1(1:n,lay,ist) * opdep_tl(lev)
              ENDIF
            ENDIF
          ENDIF
          ...
          IF (dosolar .AND. m == 0 .AND. fssa(lay) > 0._jprb) THEN
            ...
            delta_tms_tl = delta_tms_tl + tmpval * taufac_tl * &
              ...
              tmpval * taufac * &
              (ssa_tl(isti,laymap,i) * trans_scatt_ir%phup(isti,laymap,i) * tmpval2 + &
               ssa(isti,laymap,i) * trans_scatt_ir_tl%phup(isti,laymap,i) * tmpval2 - &
               ssa(isti,laymap,i) * trans_scatt_ir%phup(isti,laymap,i) * ftau_tl(lay) * tmpval2**2 - &
               SUM(legcoef_tl(:,laymap,isti) * legpolymuscata(:,0,0)))
          ENDIF
        ...
        DO j = 1, nstrlay
          ...
          lo = klu1 + ilo - j
          hi = klu1 + ihi - j
          x_tl(ilo:ihi) = x_tl(ilo:ihi) - kkpack_tl(lo:hi,j) * dom_state(i)%x(j,m,ist)
        ENDDO
        ...
        DO lay = 1, nlayers
          ...
          IF (fssa(lay) > 0._jprb) THEN
            ...
            DO j = 1, n
              ...
              radinc_tl = radinc_tl + x_tl(row+j) * fpmuhsup(j,laymap,isti,m) * taufac * tmpval + &
                                      dom_state(i)%x(row+j,m,ist) * fpmuhsup_tl(j,laymap,isti,m) * taufac * tmpval + &
                                      dom_state(i)%x(row+j,m,ist) * fpmuhsup(j,laymap,isti,m) * taufac_tl * tmpval - &
                                      eval_tl(j,laymap,isti,m) * musat * &
                                      dom_state(i)%x(row+j,m,ist) * fpmuhsup(j,laymap,isti,m) * taufac * tmpval**2
              ...
              tmpval = 1._jprb / (1._jprb - dom_state(i)%eval(j,laymap,isti,m) * musat)
              radinc_tl = radinc_tl + x_tl(row+n+j) * fpmuhsdn(j,laymap,isti,m) * taufac * tmpval + &
                                      dom_state(i)%x(row+n+j,m,ist) * fpmuhsdn_tl(j,laymap,isti,m) * taufac * tmpval + &
                                      dom_state(i)%x(row+n+j,m,ist) * fpmuhsdn(j,laymap,isti,m) * taufac_tl * tmpval + &
                                      eval_tl(j,laymap,isti,m) * musat * &
                                      dom_state(i)%x(row+n+j,m,ist) * fpmuhsdn(j,laymap,isti,m) * taufac * tmpval**2
            ENDDO
            ...
            IF (dosolar) THEN
              ...
              radinc_tl = radinc_tl + fpmupi_tl(laymap,isti,m) * taufac / (1._jprb + musunr * musat) + &
                                      fpmupi(laymap,isti,m) * taufac_tl / (1._jprb + musunr * musat)
            ENDIF
          ENDIF
        ...
        IF (m == 0 .AND. inc_surface) THEN
          ...
          DO j = 1, n
            radsurfup_sum_tl = radsurfup_sum_tl + muqw(j) * &
              (SUM(x_tl(row+1:row+n) * fm1(j,:,laymap,isti,m) * tlayer(:,nlayers) + &
                   dom_state(i)%x(row+1:row+n,m,ist) * fm1_tl(j,:,laymap,isti,m) * tlayer(:,nlayers) + &
                   dom_state(i)%x(row+1:row+n,m,ist) * fm1(j,:,laymap,isti,m) * tlayer_tl(:,nlayers)) + &
               SUM(x_tl(row+n+1:row+nstr) * fm2(j,:,laymap,isti,m) + &
                   dom_state(i)%x(row+n+1:row+nstr,m,ist) * fm2_tl(j,:,laymap,isti,m)))
          ENDDO

          IF (dosolar) THEN
            DO j = 1, n
              radsurfup_sum_tl = radsurfup_sum_tl + muqw(j) * &
                (z_tl(n+j,laymap,isti,m) * tausun(nlevels) + dom_state(i)%z(n+j,laymap,isti,m) * tausun_tl(nlevels))
            ENDDO

            radsurfup_tl = reflfac_tl * dom_state(i)%radsurfup_sum(ist) + reflfac * radsurfup_sum_tl + &
                           (albedo_tl / pi) * solar_spectrum(i) * musun * tausun(nlevels) + &
                           (albedo / pi) * solar_spectrum(i) * musun * tausun_tl(nlevels)
          ELSE
            DO j = 1, n
              radsurfup_sum_tl = radsurfup_sum_tl + muqw(j) * &
                (y0_tl(n+j,nlayers) + y1_tl(n+j,nlayers) * opdep(nlevels) + dom_state(i)%y1(n+j,nlayers,ist) * opdep_tl(nlevels))
            ENDDO
            radsurfup_tl = reflfac_tl * dom_state(i)%radsurfup_sum(ist) + reflfac * radsurfup_sum_tl + &
                           emissivity_tl(i)%emis_out * auxrad%skin(i) + emissivity(i)%emis_out * auxrad_tl%skin(i)
          ENDIF
          ...
          radinc_tl = radinc_tl + radsurfup_tl * tausat(nlevels) + &
                      dom_state(i)%radsurfup(ist) * tausat_tl(nlevels)
        ENDIF
      ...
      IF (ist == 0) THEN
        ...
        rad_tl%total(i) = rad_tl%total(i) + thisrad_tl * ircld%xstrclr(prof) + &
                          dom_state(i)%thisrad(ist) * ircld_tl%xstrclr(prof)
      ELSE
        rad_tl%total(i) = rad_tl%total(i) + thisrad_tl * (ircld%xstr(ist+1,prof) - ircld%xstr(ist,prof)) + &
                          dom_state(i)%thisrad(ist) * (ircld_tl%xstr(ist+1,prof) - ircld_tl%xstr(ist,prof))
      ENDIF
