...
! for rttov same set of weights for all elements, so one call suffices
    ...
    DO jo = 1, kno
      ...
      IF (opts%rt_ir%ozone_data) ProfGasIn_ad(prof)%o3(istart(jo):iend(jo))  =      &
