...
#include "rttov_setpredictors_78_k.interface"
...
!   CALL rttov_init_ircld(traj0_k%ircld)
  traj0_k%ircld%xstrclr = 0._jprb
  IF (opts%rt_ir%addclouds) THEN
    traj0_k%ircld%xstr = 0._jprb
  ENDIF
  ...
  ELSEIF (.NOT. traj0_sta%do_mfasis) THEN
    ...
    IF (opts%rt_all%switchrad) THEN
      IF (traj0_sta%dothermal) &
        CALL rttov_calcbt_ad(opts, chanprof, coefs%coef, traj0_sta%thermal, radiance, radiance_k)
...
!-----------------------------------------------------------------------
  CALL rttov_convert_profile_units_k(  &
      opts,                            &
      chanprof,                        &
      coefs,                           &
      profiles,                        &
