...
!INTF_OFF
  ...
  USE rttov_math_mod, ONLY : divide, invsqrt, invsqrt_ad, sintosec_ad, reciprocal, reciprocal_ad
!INTF_ON
  IMPLICIT NONE
...
! Start AD
  ...
  IF (plane_parallel) THEN
    raytracing_ad%pathsat_rsqrt = 0._jprb
    raytracing_ad%pathsat_sqrt  = 0._jprb
...
!   ra_ad = 0._jprb;
    ...
    IF (opts%config%fix_hgpl) THEN
      DO iprof = 1, nprofiles
        raytracing_ad%z_r(aux%s(iprof)%nearestlev_surf-1,iprof) = 0._jprb
      ENDDO
    ENDIF
  ...
  SUBROUTINE calc_ssu_co2_thickness_ad
    REAL(jprb) :: pstd
    REAL(jprb) :: tstd
    REAL(jprb) :: pa(profiles(1)%nlayers),    pa_ad(profiles(1)%nlayers)
    REAL(jprb) :: dpa(profiles(1)%nlayers),   dpa_ad(profiles(1)%nlayers)
    REAL(jprb) :: rho(profiles(1)%nlayers),   rho_ad(profiles(1)%nlayers)
    REAL(jprb) :: temp(profiles(1)%nlayers),  temp_ad(profiles(1)%nlayers)
    REAL(jprb) :: co2(profiles(1)%nlayers),   co2_ad(profiles(1)%nlayers)
    REAL(jprb) :: dz, dz_ad
    REAL(jprb) :: dzstp_ad
