...
!INTF_OFF
  ...
  USE rttov_math_mod, ONLY : invsqrt, invsqrt_k, sintosec_k, reciprocal, reciprocal_k
!INTF_ON
  IMPLICIT NONE
...
! Start K
  ...
  IF (plane_parallel) THEN
    raytracing_k%pathsat_rsqrt = 0._jprb
    raytracing_k%pathsat_sqrt  = 0._jprb
  ...
  SUBROUTINE calc_hgpl_k()
    ...
    DO i = 1, nchannels
      ...
      IF (prof .NE. last_prof) THEN
        ...
        CALL reciprocal(raytracing%dmair(2:nlevels,prof) - raytracing%dmair(1:nlevels-1,prof), ddmair_r)
      ...
      i_below     = aux%s(prof)%nearestlev_surf
      i_above     = aux%s(prof)%nearestlev_surf - 1
...
!   ra_k = 0._jprb;
    ...
    IF (opts%config%fix_hgpl) THEN
      DO i = 1, nchannels
        prof = chanprof(i)%prof
        raytracing_k%z_r(aux%s(prof)%nearestlev_surf-1,i) = 0._jprb
      ENDDO
    ENDIF
