...
!INTF_OFF
  ...
  USE rttov_math_mod, ONLY : divide, invsqrt, invsqrt_tl, sintosec_tl, reciprocal, reciprocal_tl
!INTF_ON
  IMPLICIT NONE
...
!-----end of header-------------------------------------------------------------
  ...
  IF (plane_parallel) THEN
    ...
    raytracing_tl%pathsat_rsqrt = 0._jprb
    raytracing_tl%pathsat_sqrt  = 0._jprb
  ELSE
...
CONTAINS
  ...
  SUBROUTINE calc_seczen_tl(ra,za,path,ra_tl,za_tl,path_tl,do_sat)
    ...
    IF (opts%config%fix_hgpl) THEN
      DO iprof = 1, nprofiles
        raytracing_tl%z_r(aux%s(iprof)%nearestlev_surf-1,iprof) = 0._jprb
      ENDDO
    ENDIF
  ...
  SUBROUTINE calc_ssu_co2_thickness_tl
    REAL(jprb) :: pstd
    REAL(jprb) :: tstd
    REAL(jprb) :: pa(profiles(1)%nlayers),    pa_tl(profiles(1)%nlayers)
    REAL(jprb) :: dpa(profiles(1)%nlayers),   dpa_tl(profiles(1)%nlayers)
    REAL(jprb) :: rho(profiles(1)%nlayers),   rho_tl(profiles(1)%nlayers)
    REAL(jprb) :: temp(profiles(1)%nlayers),  temp_tl(profiles(1)%nlayers)
    REAL(jprb) :: co2(profiles(1)%nlayers),   co2_tl(profiles(1)%nlayers)
    REAL(jprb) :: dz, dz_tl
    REAL(jprb) :: dzstp_tl
