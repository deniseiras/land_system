...
CONTAINS
  ...
  SUBROUTINE SQRT_2d_TL(x, x_tl, y_tl)
    REAL(jprb), INTENT(in) :: x(:,:), x_tl(:,:)
    REAL(jprb), INTENT(out) :: y_tl(:,:)

    CALL divide_2d(0.5_jprb * x_tl, x, y_tl, acc = .FALSE._jplm)

  END SUBROUTINE SQRT_2d_TL

  SUBROUTINE SQRT_2d_AD(x, x_ad, y_ad, acc)
    REAL(jprb), INTENT(in) :: x(:,:), y_ad(:,:)
    REAL(jprb), INTENT(inout) :: x_ad(:,:)
    LOGICAL(jplm), INTENT(in) :: acc

    CALL divide_2d(0.5_jprb * y_ad, x, x_ad, acc = acc)

  END SUBROUTINE SQRT_2d_AD

  SUBROUTINE SQRT_1d_AD(x, x_ad, y_ad, acc)
    REAL(jprb), INTENT(in) :: x(:), y_ad(:)
    REAL(jprb), INTENT(inout) :: x_ad(:)
    LOGICAL(jplm), INTENT(in) :: acc

    CALL divide_1d(0.5_jprb * y_ad, x, x_ad, acc = acc)
