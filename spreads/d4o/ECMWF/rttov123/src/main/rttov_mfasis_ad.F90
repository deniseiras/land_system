...
!INTF_END
  ...
  INTEGER(jpim)              :: nchanprof, prof, nstrms, npar, chan
  ...
  REAL(jprb)                 :: strwei_clr, strwei_cs
  REAL(jprb)                 :: refl_ad, rflstr_ad, refl_clr_ad, strwei_ad
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!                       - dvap   (jj) * aux_ad%s(prof)%pfraction_surf
    ...
    strwei_clr = ircld%xstrclr(prof) ! clear stream
...
!=====================================================================================
!  linear computations independent of stream cs
!=====================================================================================
!================================
...
! radiance%total and we assume all other radiance arrays are zero
    ...
    strwei_ad  =strwei_ad   + mfasis_refl(0,i)%refl*refl_ad
    refl_clr_ad=refl_clr_ad + strwei_clr     *refl_ad

!    strwei_ad = ircld_ad%xstrclr(prof) ! clear stream
    ircld_ad%xstrclr(prof)=ircld_ad%xstrclr(prof) + strwei_ad
    strwei_ad=0._jprb
...
!=====================================================================================
    ...
    DO cs = 1, nstrms
!==================================================================================
! First compute nonlinear quatities needed in linear computations (for given channel "i" and stream "cs")
!==================================================================================
      ! --------------------------------------------------------------------------
      ! Set stream weight, optical depth and effective diameters
      ! --------------------------------------------------------------------------
      IF ( mfasis_coefs%file_type .EQ. mfasis_cld ) THEN ! clouds
        strwei_cs = ircld%xstr(cs+1,prof) - ircld%xstr(cs,prof)
!       strwei_ad = ircld_ad%xstr(cs+1,prof) - ircld_ad%xstr(cs,prof)
        od   (:,:) = 0._jprb
!       od_ad(:,:) = 0._jprb
        ed   (:,:) = 0._jprb
!       ed_ad(:,:) = 0._jprb
...
!             ed_ad(1,j) = SUM(wcl_deff(1:nwcl_max) * trans_scatt_ir_ad%opdpext(1:nwcl_max,j,i))
              IF ( od(1,j) .GT. 0. ) THEN
...
!         ed_eff_ad(2) = SUM( &
!                              (od_ad(2,:)*ed(2,:) + od(2,:)*ed_ad(2,:))*(1._jprb-fac_sw*fac_mx(:)) - &
!                               od(2,:)*ed(2,:)* ( fac_sw_ad*fac_mx(:)+fac_sw*fac_mx_ad(:) )          &
!                             ) /od_eff(2)   &
!                          - ed_eff(2) * od_eff_ad(2)/od_eff(2)
        ELSE
          ed_eff   (2) = ed_aux(2)
!         ed_eff_ad(2) = 0._jprb
        ENDIF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ELSE  ! aerosols
        strwei_cs = 1._jprb     ! single stream for aerosols
!       strwei_ad = 0._jprb     ! single stream for aerosols
        od   (:,:)= 0._jprb
!       od_ad(:,:)= 0._jprb
        DO k = 1, npar
...
!==================================================================================
! Start linear computations for channel "i" and stream "cs"
!==================================================================================
!     refl_ad = refl_ad + rflstr_ad*strwei_cs + mfasis_refl(cs,i)%refl*strwei_ad
      rflstr_ad = rflstr_ad+  strwei_cs        * refl_ad
      strwei_ad = strwei_ad+   mfasis_refl(cs,i)%refl * refl_ad                  ! rflstr !NNNNNNNNN

!     rflstr_tl = rflstr_tl+sum(iw_wv_tl(1:n_wvdim)*mfasis_refl(cs,i)%refl_wv(1:n_wvdim))
      iw_wv_ad(1:n_wvdim) = iw_wv_ad(1:n_wvdim) + mfasis_refl(cs,i)%refl_wv(1:n_wvdim) * rflstr_ad

      DO d = 1, nid
        IF ( mfasis_coefs%lut_axes(di(d))%dim_type .EQ. mfasis_dim_scaangle ) THEN
!         rflstr_ad = rflstr_ad+mfasis_refl(cs,i)%refl_lin_coef(d)*albedo_ad
          albedo_ad = albedo_ad +mfasis_refl(cs,i)%refl_lin_coef(d)* rflstr_ad
        ELSE
!         rflstr_ad = rflstr_ad+mfasis_refl(cs,i)%refl_lin_coef(d)*iw_ad(d)
          iw_ad(d)  = iw_ad(d) +mfasis_refl(cs,i)%refl_lin_coef(d)* rflstr_ad
        ENDIF
      ENDDO
      rflstr_ad = 0
...
!                             ) /od_eff(2)   &
!                          - ed_eff(2) * od_eff_ad(2)/od_eff(2)
          od_ad(2,:)   = od_ad(2,:)    + ed(2,:)*(1._jprb-fac_sw*fac_mx(:))*ed_eff_ad(2)/od_eff(2)
          ed_ad(2,:)   = ed_ad(2,:)    + od(2,:)*(1._jprb-fac_sw*fac_mx(:))*ed_eff_ad(2)/od_eff(2)
          fac_sw_ad    = fac_sw_ad     - sum(od(2,:)*ed(2,:)*fac_mx(:))    *ed_eff_ad(2)/od_eff(2)
          fac_mx_ad(:) = fac_mx_ad(:)  - od(2,:)*ed(2,:)*fac_sw            *ed_eff_ad(2)/od_eff(2)
          od_eff_ad(2) = od_eff_ad(2)  - ed_eff(2)                      *ed_eff_ad(2)/od_eff(2)
          ed_eff_ad(2) = 0
        ELSE
          ed_eff_ad(2) = 0._jprb
        ENDIF
...
!         ENDIF
          ...
          IF ( ircld%icldarr(cs,j,prof) .NE. 1 ) THEN
            IF(j>1) THEN
              fac_mx_ad(j-1) = fac_mx_ad(j)
            ELSE
              fac_mx_ad(j)=0
            ENDIF
          ELSE !(ircld%icldarr(cs,j,prof) .EQ. 1 )
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!           ed_ad(2,j) = aux_ad%ice_dg(j,prof)
...
!!!!!!!!!!!!!!!!!!!!!!!
        ...
        ed_ad(:,:) = 0._jprb
!       strwei_ad = ircld_ad%xstr(cs+1,prof) - ircld_ad%xstr(cs,prof)
        ircld_ad%xstr(cs+1,prof) = ircld_ad%xstr(cs+1,prof) + strwei_ad
        ircld_ad%xstr(cs,prof)   = ircld_ad%xstr(cs,prof)   - strwei_ad
        strwei_ad = 0
...
!           od_ad(k,j) = trans_scatt_ir_ad%opdpext(par,j,i)
          ...
          ENDDO
        ENDDO
        strwei_ad = 0._jprb     ! single stream for aerosols A1
        od_ad(:,:)= 0._jprb     !                            A2
