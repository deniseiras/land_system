...
!INTF_END
  ...
  REAL(jprb)                 :: refl_k, rflstr_k, refl_clr_k, strwei_k
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! radiance%total and we assume all other radiance arrays are zero
    ...
    strwei_k  =strwei_k   + mfasis_refl(0,i)%refl*refl_k
    refl_clr_k=refl_clr_k + strwei_clr     *refl_k

!    strwei_k = ircld_k%xstrclr(i) ! clear stream
    ircld_k%xstrclr(i)=ircld_k%xstrclr(i) + strwei_k
    strwei_k=0._jprb
...
!       strwei_k = ircld_k%xstr(cs+1,i) - ircld_k%xstr(cs,i)
        od   (:,:) = 0._jprb
!       od_k(:,:) = 0._jprb
        ed   (:,:) = 0._jprb
!       ed_k(:,:) = 0._jprb
...
!             ed_k(1,j) = SUM(wcl_deff(1:nwcl_max) * trans_scatt_ir_k%opdpext(1:nwcl_max,j,i))
              IF ( od(1,j) .GT. 0. ) THEN
...
!         ed_eff_k(2) = SUM( &
!                              (od_k(2,:)*ed(2,:) + od(2,:)*ed_k(2,:))*(1._jprb-fac_sw*fac_mx(:)) - &
!                               od(2,:)*ed(2,:)* ( fac_sw_k*fac_mx(:)+fac_sw*fac_mx_k(:) )          &
!                             ) /od_eff(2)   &
!                          - ed_eff(2) * od_eff_k(2)/od_eff(2)
        ELSE
          ed_eff   (2) = ed_aux(2)
!         ed_eff_k(2) = 0._jprb
        ENDIF
...
!       strwei_k = 0._jprb     ! single stream for aerosols
        od   (:,:)= 0._jprb
!       od_k(:,:)= 0._jprb
        DO k = 1, npar
...
!==================================================================================
!     refl_k = refl_k + rflstr_k*strwei_cs + mfasis_refl(cs,i)%refl*strwei_k
      rflstr_k = rflstr_k+  strwei_cs        * refl_k
      strwei_k = strwei_k+   mfasis_refl(cs,i)%refl * refl_k                  ! rflstr !NNNNNNNNN

!     rflstr_tl = rflstr_tl+sum(iw_wv_tl(1:n_wvdim)*mfasis_refl(cs,i)%refl_wv(1:n_wvdim))
      iw_wv_k(1:n_wvdim) = iw_wv_k(1:n_wvdim) + mfasis_refl(cs,i)%refl_wv(1:n_wvdim) * rflstr_k

      DO d = 1, nid
        IF ( mfasis_coefs%lut_axes(di(d))%dim_type .EQ. mfasis_dim_scaangle ) THEN
!         rflstr_k = rflstr_k+mfasis_refl(cs,i)%refl_lin_coef(d)*albedo_k
          albedo_k = albedo_k +mfasis_refl(cs,i)%refl_lin_coef(d)* rflstr_k
        ELSE
!         rflstr_k = rflstr_k+mfasis_refl(cs,i)%refl_lin_coef(d)*iw_k(d)
          iw_k(d)  = iw_k(d) +mfasis_refl(cs,i)%refl_lin_coef(d)* rflstr_k
        ENDIF
      ENDDO
      rflstr_k = 0
...
!                             ) /od_eff(2)   &
!                          - ed_eff(2) * od_eff_k(2)/od_eff(2)
          od_k(2,:)   = od_k(2,:)    + ed(2,:)*(1._jprb-fac_sw*fac_mx(:))*ed_eff_k(2)/od_eff(2)
          ed_k(2,:)   = ed_k(2,:)    + od(2,:)*(1._jprb-fac_sw*fac_mx(:))*ed_eff_k(2)/od_eff(2)
          fac_sw_k    = fac_sw_k     - sum(od(2,:)*ed(2,:)*fac_mx(:))    *ed_eff_k(2)/od_eff(2)
          fac_mx_k(:) = fac_mx_k(:)  - od(2,:)*ed(2,:)*fac_sw            *ed_eff_k(2)/od_eff(2)
          od_eff_k(2) = od_eff_k(2)  - ed_eff(2)                      *ed_eff_k(2)/od_eff(2)
          ed_eff_k(2) = 0
        ELSE
          ed_eff_k(2) = 0._jprb
        ENDIF
...
!!!!!!!!!!!!!!!!!!!!!!!
        ...
        ed_k(:,:) = 0._jprb
!       strwei_k = ircld_k%xstr(cs+1,i) - ircld_k%xstr(cs,i)
        ircld_k%xstr(cs+1,i) = ircld_k%xstr(cs+1,i) + strwei_k
        ircld_k%xstr(cs,i)   = ircld_k%xstr(cs,i)   - strwei_k
        strwei_k = 0
...
!           od_k(k,j) = trans_scatt_ir_k%opdpext(par,j,i)
          ...
          ENDDO
        ENDDO
        strwei_k = 0._jprb     ! single stream for aerosols A1
        od_k(:,:)= 0._jprb     !                            A2
