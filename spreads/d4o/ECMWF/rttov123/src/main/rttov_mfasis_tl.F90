...
! REAL(jprb)                 :: iw_wv(3)
  ...
  REAL(jprb)                 :: refl_tl, rflstr_tl, refl_clr_tl, strwei_tl
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!       strwei_tl = ircld_tl%xstr(cs+1,prof) - ircld_tl%xstr(cs,prof)
        od   (:,:) = 0._jprb
!       od_tl(:,:) = 0._jprb
        ed   (:,:) = 0._jprb
!       ed_tl(:,:) = 0._jprb
...
!             ed_tl(1,j) = SUM(wcl_deff(1:nwcl_max) * trans_scatt_ir_tl%opdpext(1:nwcl_max,j,i))
              IF ( od(1,j) .GT. 0. ) THEN
...
!         ed_eff_tl(2) = SUM( &
!                              (od_tl(2,:)*ed(2,:) + od(2,:)*ed_tl(2,:))*(1._jprb-fac_sw*fac_mx(:)) - &
!                               od(2,:)*ed(2,:)* ( fac_sw_tl*fac_mx(:)+fac_sw*fac_mx_tl(:) )          &
!                             ) /od_eff(2)   &
!                          - ed_eff(2) * od_eff_tl(2)/od_eff(2)
        ELSE
          ed_eff   (2) = ed_aux(2)
!         ed_eff_tl(2) = 0._jprb
        ENDIF
...
!       strwei_tl = 0._jprb     ! single stream for aerosols
        od   (:,:)= 0._jprb
!       od_tl(:,:)= 0._jprb
        DO k = 1, npar
...
!                 / ( axis%values(j+1) - axis%values(j) )
      ...
      DO d = 1, nid
        IF ( mfasis_coefs%lut_axes(di(d))%dim_type .EQ. mfasis_dim_scaangle ) THEN
          rflstr_tl = rflstr_tl+mfasis_refl(cs,i)%refl_lin_coef(d)*albedo_tl
        ELSE
          rflstr_tl = rflstr_tl+mfasis_refl(cs,i)%refl_lin_coef(d)*iw_tl(d)
        ENDIF
      ENDDO
      ...
      refl_tl = refl_tl + rflstr_tl*strwei_cs + mfasis_refl(cs,i)%refl*strwei_tl
...
!   strwei_clr = ircld%xstrclr(prof) ! clear stream
    strwei_tl = ircld_tl%xstrclr(prof) ! clear stream

    refl_tl = refl_tl + refl_clr_tl*strwei_clr + mfasis_refl(0,i)%refl*strwei_tl
