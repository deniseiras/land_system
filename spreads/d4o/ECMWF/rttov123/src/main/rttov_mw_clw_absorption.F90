...
!INTF_END
  ...
  IF (opts%rt_mw%clw_calc_on_coef_lev) THEN
    ! This maintains consistency with the old code which didn't use the local path angles
    DO prof = 1, nprofiles
      DO lay = 1, nlayers
        lev = lay + 1
        aux%clw(lay,prof) = &
            0.1820_jprb * 100._jprb * (profiles(prof)%p(lev) - profiles(prof)%p(lev-1)) * ztemp * &
            angles(prof)%seczen * 0.5_jprb * (profiles(prof)%clw(lev-1) + profiles(prof)%clw(lev))
      ENDDO
    ENDDO
  ELSE
    DO prof = 1, nprofiles
