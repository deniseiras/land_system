...
!   LOGICAL(KIND=jplm) :: do_dom, do_chou_scaling, do_single_scatt
  ...
  REAL(KIND=jprb)    :: aer_phfn_ad(coef_scatt_ir%aer_nphangle)
  REAL(KIND=jprb)    :: wcldeff_phfn(coef_scatt_ir%wcldeff_nphangle)
  REAL(KIND=jprb)    :: wcldeff_phfn_ad(coef_scatt_ir%wcldeff_nphangle)
  REAL(KIND=jprb)    :: icl_phfn(coef_scatt_ir%icl_nphangle)
  REAL(KIND=jprb)    :: icl_phfn_ad(coef_scatt_ir%icl_nphangle)
        ...
        IF (ist == 0) THEN
          IF (opts%rt_ir%addaerosl) THEN
            DO lay = nlayers, 1, -1
              lev = lay + 1
              IF (solar(j)) THEN
                opdsun_ad = opdsun_ad + trans_scatt_ir_dyn_ad%opdpacsun(lev,ist,j)
                trans_scatt_ir_ad%opdpaclsun(0,lay,j) = &
                    trans_scatt_ir_ad%opdpaclsun(0,lay,j) + opdsun_ad
              ENDIF
              IF (thermal(j)) THEN
                opd_ad = opd_ad + trans_scatt_ir_dyn_ad%opdpac(lev,ist,j)
                trans_scatt_ir_ad%opdpacl(0,lay,j) = &
                    trans_scatt_ir_ad%opdpacl(0,lay,j) + opd_ad
              ENDIF
            ENDDO ! layers
          ELSE
            IF (solar(j)) trans_scatt_ir_dyn_ad%opdpacsun(:,ist,j) = 0._jprb
            IF (thermal(j)) trans_scatt_ir_dyn_ad%opdpac(:,ist,j) = 0._jprb
          ENDIF
        ELSE
          DO lay = nlayers, 1, -1
            lev = lay + 1
            ...
            IF (solar(j)) THEN
              opdsun_ad = opdsun_ad + trans_scatt_ir_dyn_ad%opdpacsun(lev,ist,j)
              trans_scatt_ir_ad%opdpaclsun(isti,lay,j) = &
                  trans_scatt_ir_ad%opdpaclsun(isti,lay,j) + opdsun_ad
            ENDIF
            IF (thermal(j)) THEN
              opd_ad = opd_ad + trans_scatt_ir_dyn_ad%opdpac(lev,ist,j)
              trans_scatt_ir_ad%opdpacl(isti,lay,j) = &
                  trans_scatt_ir_ad%opdpacl(isti,lay,j) + opd_ad
            ENDIF
          ENDDO ! layers
        ENDIF ! istream == 0
...
!         IF (solar(j)) THEN
!           opdsun_ad = 0._jprb
!           trans_scatt_ir_dyn_ad%opdpacsun(1,ist,j) = 0._jprb
!         ENDIF
!         IF (thermal(j)) THEN
!           opd_ad = 0._jprb
!           trans_scatt_ir_dyn_ad%opdpac(1,ist,j) = 0._jprb
!         ENDIF
      ENDDO ! istream
  ...
  IF (opts%rt_ir%addclouds) THEN
    ...
    IF (opts%rt_ir%user_cld_opt_param) THEN
      ...
      CALL calc_user_opt_param_ad(cld_opt_param, cld_opt_param_ad, cld, cld_ad, 1_jpim)
            ...
            IF (icld <= nwcl_max) THEN
              ...
              IF (clw_scheme == clw_scheme_deff) THEN
                ...
                ELSEIF (do_chan) THEN
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_ad(musat, musat_ad, musun, musun_ad, relazi, zminphadiff, &
                                           ...
                                           coef_scatt_ir%wcldeff_phfn_int%iphangle, phasint_ad, wcldeff_phfn_ad)
                  ...
                  clw_ad = clw_ad + &
                      sca_ad * (optp%optpwcldeff%sca(k1,chan) + dgfrac * &
                      (optp%optpwcldeff%sca(k2,chan) - optp%optpwcldeff%sca(k1,chan)))
                  dgfrac_ad = dgfrac_ad + sca_ad * (optp%optpwcldeff%sca(k2,chan) - optp%optpwcldeff%sca(k1,chan)) * &
                      clw
                ...
                IF (aux%clw_dg(lay,prof) >= optp%optpwcldeff%fmv_wcldeff_deff(1) .AND. &
                  ...
                  aux_ad%clw_dg(lay,prof) = aux_ad%clw_dg(lay,prof) + &
                      dgfrac_ad / (optp%optpwcldeff%fmv_wcldeff_deff(k2) - optp%optpwcldeff%fmv_wcldeff_deff(k1))
!                   ELSE
!                     dgfrac_ad = 0._jprb
                ENDIF
...
!                     legcoef(0:nmom) = legcoef(0:nmom) + optp%optpwcl(icld)%legcoef(1:nmom+1,chan,1) * sca
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_ad(musat, musat_ad, musun, musun_ad, relazi, zminphadiff, &
                                           ...
                                           coef_scatt_ir%wcl_phfn_int%iphangle, phasint_ad)
                  ...
                  IF (do_dom_chan) THEN
                    sca_ad = sca_ad + SUM(legcoef_ad(0:nmom) * optp%optpwcl(icld)%legcoef(1:nmom+1,chan,1))
                    sca_ad = sca_ad + sumsca_ad
                    profiles_int_ad(prof)%cloud(icld,lay) = profiles_int_ad(prof)%cloud(icld,lay) + &
                        optp%optpwcl(icld)%sca(chan,1) * sca_ad * coef_scatt_ir%confac(icld)
                  ENDIF
              ...
              IF (ice_scheme == ice_scheme_ssec) THEN
                ...
                ELSEIF (do_chan) THEN
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_ad(musat, musat_ad, musun, musun_ad, relazi, zminphadiff, &
                                           ...
                                           coef_scatt_ir%icl_phfn_int%iphangle, phasint_ad, icl_phfn_ad)
                  ...
                  IF (do_chou_scaling_chan) THEN
                    ...
                    bpr_ad = & !bpr_ad + &
                        cld%partsca(icld,lay,j) * cld_ad%opdpscabpr(lay,j)
                    dgfrac_ad = dgfrac_ad + bpr_ad * (optp%optpicl%bpr(k2,chan) - optp%optpicl%bpr(k1,chan))
                  ENDIF
                  ...
                  sca_ad = sca_ad + raytracing%ltick(lay,prof) * cld_ad%partsca(icld,lay,j)
                  profiles_int_ad(prof)%cloud(icld,lay) = profiles_int_ad(prof)%cloud(icld,lay) + &
                      sca_ad * (optp%optpicl%sca(k1,chan) + dgfrac * &
                      (optp%optpicl%sca(k2,chan) - optp%optpicl%sca(k1,chan)))
                  dgfrac_ad = dgfrac_ad + sca_ad * (optp%optpicl%sca(k2,chan) - optp%optpicl%sca(k1,chan)) * &
                ...
                IF (do_chan) THEN
                  IF (aux%ice_dg(lay,prof) >= optp%optpicl%fmv_icl_deff(1) .AND. &
                    ...
                    aux_ad%ice_dg(lay,prof) = aux_ad%ice_dg(lay,prof) + &
                        dgfrac_ad / (optp%optpicl%fmv_icl_deff(k2) - optp%optpicl%fmv_icl_deff(k1))
!                     ELSE
!                       dgfrac_ad = 0._jprb
                  ENDIF
                ENDIF
...
!                       legcoef(:) = legcoef(:) + thislegcoef(:) * sca
                ...
                IF (do_single_scatt_chan .OR. do_dom_chan) THEN
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_ad(musat, musat_ad, musun, musun_ad, relazi, zminphadiff, &
                                           ...
                                           optp%optpiclbaran2018%phfn_int%iphangle, phasint_ad, baran_phfn_ad)
                  ...
                  IF (do_dom_chan) THEN
                    ...
                    CALL calc_legendre_coef_gauss_ad(optp%optpiclbaran2018%q, optp%optpiclbaran2018%w, &
                                                     baran_phfn_interp, baran_phfn_interp_ad, dom_nstr, &
                                                     thislegcoef_ad(:))

                    CALL normalise_ad(baran_ngauss, optp%optpiclbaran2018%w, baran_phfn_interp_unnorm, baran_phfn_interp_ad)
                    ...
                    CALL spline_interp_ad(thisnphangle, thiscosphangle(thisnphangle:1:-1), &
                                          ...
                                          baran_ngauss, optp%optpiclbaran2018%q, baran_phfn_interp_ad)
                ...
                IF (ice_scheme == ice_scheme_baran2018) THEN
                  CALL rttov_baran2018_calc_optpar_ad(optp, chan, &
...
!     ENDIF
  ...
  IF (opts%rt_ir%addaerosl) THEN
    ...
    IF (opts%rt_ir%user_aer_opt_param) THEN
      ...
      CALL calc_user_opt_param_ad(aer_opt_param, aer_opt_param_ad, aer, aer_ad, 0_jpim)
              ...
              IF (solar(j)) THEN
                ...
                IF (do_single_scatt_chan) THEN
                  ...
                  CALL int_phase_fn_ad(musat, musat_ad, musun, musun_ad, relazi, zminphadiff, &
                                       ...
                                       coef_scatt_ir%aer_phfn_int%iphangle, phasint_ad, aer_phfn_ad)
...
CONTAINS
  ...
  SUBROUTINE calc_user_opt_param_ad(opt_param, opt_param_ad, aercld, aercld_ad, iaercld)
    ...
    LOGICAL(jplm) :: do_chou_scaling_chan, do_dom_chan, do_single_scatt_chan, do_opt_param_ad
        ...
        IF (opt_param%abs(lay,j) > 0._jprb .OR. &
          ...
          IF (do_opt_param_ad) THEN
            IF (solar(j)) THEN
              ...
              IF (do_single_scatt_chan) THEN
                phasint_ad = aercld_ad%phtotdo(lay,j) * aercld%opdpsca(lay,j)
                aercld_ad%opdpsca(lay,j) = aercld_ad%opdpsca(lay,j) + aercld_ad%phtotdo(lay,j) * aercld%phintdo(1,lay,j)
              ...
              phasint_ad = aercld_ad%phtotup(lay,j) * aercld%opdpsca(lay,j)
              aercld_ad%opdpsca(lay,j) = aercld_ad%opdpsca(lay,j) + aercld_ad%phtotup(lay,j) * aercld%phintup(1,lay,j)
            ...
            IF (do_dom_chan) THEN
              ...
              opt_param_ad%legcoef(1:nmom+1,lay,j) = opt_param_ad%legcoef(1:nmom+1,lay,j) + &
                trans_scatt_ir_dyn_ad%phasefn(j)%legcoef(0:nmom,iaercld,lay)
            ENDIF
            ...
            raytracing_ad%ltick(lay,prof) = raytracing_ad%ltick(lay,prof) + &
                                            opt_param%sca(lay,j) * aercld_ad%opdpsca(lay,j)
            opt_param_ad%sca(lay,j)       = opt_param_ad%sca(lay,j) + &
                                            raytracing%ltick(lay,prof) * aercld_ad%opdpsca(lay,j)
            raytracing_ad%ltick(lay,prof) = raytracing_ad%ltick(lay,prof) + &
