...
!   LOGICAL(KIND=jplm) :: do_dom, do_chou_scaling, do_single_scatt
  ...
  REAL(KIND=jprb)    :: aer_phfn_k(coef_scatt_ir%aer_nphangle)
  REAL(KIND=jprb)    :: wcldeff_phfn(coef_scatt_ir%wcldeff_nphangle)
  REAL(KIND=jprb)    :: wcldeff_phfn_k(coef_scatt_ir%wcldeff_nphangle)
  REAL(KIND=jprb)    :: icl_phfn(coef_scatt_ir%icl_nphangle)
  REAL(KIND=jprb)    :: icl_phfn_k(coef_scatt_ir%icl_nphangle)
        ...
        IF (ist == 0) THEN
          IF (opts%rt_ir%addaerosl) THEN
            DO lay = nlayers, 1, -1
              lev = lay + 1
              IF (solar(j)) THEN
                opdsun_k = opdsun_k + trans_scatt_ir_dyn_k%opdpacsun(lev,ist,j)
                trans_scatt_ir_k%opdpaclsun(0,lay,j) = &
                    trans_scatt_ir_k%opdpaclsun(0,lay,j) + opdsun_k
              ENDIF
              IF (thermal(j)) THEN
                opd_k = opd_k + trans_scatt_ir_dyn_k%opdpac(lev,ist,j)
                trans_scatt_ir_k%opdpacl(0,lay,j) = &
                    trans_scatt_ir_k%opdpacl(0,lay,j) + opd_k
              ENDIF
            ENDDO ! layers
          ELSE
            IF (solar(j)) trans_scatt_ir_dyn_k%opdpacsun(:,ist,j) = 0._jprb
            IF (thermal(j)) trans_scatt_ir_dyn_k%opdpac(:,ist,j) = 0._jprb
          ENDIF
        ELSE
          DO lay = nlayers, 1, -1
            lev = lay + 1
            ...
            IF (solar(j)) THEN
              opdsun_k = opdsun_k + trans_scatt_ir_dyn_k%opdpacsun(lev,ist,j)
              trans_scatt_ir_k%opdpaclsun(isti,lay,j) = &
                  trans_scatt_ir_k%opdpaclsun(isti,lay,j) + opdsun_k
            ENDIF
            IF (thermal(j)) THEN
              opd_k = opd_k + trans_scatt_ir_dyn_k%opdpac(lev,ist,j)
              trans_scatt_ir_k%opdpacl(isti,lay,j) = &
                  trans_scatt_ir_k%opdpacl(isti,lay,j) + opd_k
            ENDIF
          ENDDO ! layers
        ENDIF ! istream == 0
...
!         IF (solar(j)) THEN
!           opdsun_k = 0._jprb
!           trans_scatt_ir_dyn_k%opdpacsun(1,ist,j) = 0._jprb
!         ENDIF
!         IF (thermal(j)) THEN
!           opd_k = 0._jprb
!           trans_scatt_ir_dyn_k%opdpac(1,ist,j) = 0._jprb
!         ENDIF
      ENDDO ! istream
  ...
  IF (opts%rt_ir%addclouds) THEN
    ...
    IF (opts%rt_ir%user_cld_opt_param) THEN
      ...
      CALL calc_user_opt_param_k(cld_opt_param, cld_opt_param_k, cld, cld_k, 1_jpim)
            ...
            IF (icld <= nwcl_max) THEN
              ...
              IF (clw_scheme == clw_scheme_deff) THEN
                ...
                ELSEIF (do_chan) THEN
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_k(musat, musat_k, musun, musun_k, relazi, zminphadiff, &
                                           ...
                                           coef_scatt_ir%wcldeff_phfn_int%iphangle, phasint_k, wcldeff_phfn_k)
                  ...
                  clw_k = clw_k + &
                      sca_k * (optp%optpwcldeff%sca(k1,chan) + dgfrac * &
                      (optp%optpwcldeff%sca(k2,chan) - optp%optpwcldeff%sca(k1,chan)))
                  dgfrac_k = dgfrac_k + sca_k * (optp%optpwcldeff%sca(k2,chan) - optp%optpwcldeff%sca(k1,chan)) * &
                      clw
                ...
                IF (aux%clw_dg(lay,prof) >= optp%optpwcldeff%fmv_wcldeff_deff(1) .AND. &
                  ...
                  aux_k%clw_dg(lay,j) = aux_k%clw_dg(lay,j) + &
                      dgfrac_k / (optp%optpwcldeff%fmv_wcldeff_deff(k2) - optp%optpwcldeff%fmv_wcldeff_deff(k1))
!                   ELSE
!                     dgfrac_k = 0._jprb
                ENDIF
...
!                     legcoef(0:nmom) = legcoef(0:nmom) + optp%optpwcl(icld)%legcoef(1:nmom+1,chan,1) * sca
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_k(musat, musat_k, musun, musun_k, relazi, zminphadiff, &
                                           ...
                                           coef_scatt_ir%wcl_phfn_int%iphangle, phasint_k)
                  ...
                  IF (do_dom_chan) THEN
                    sca_k = sca_k + SUM(legcoef_k(0:nmom) * optp%optpwcl(icld)%legcoef(1:nmom+1,chan,1))
                    sca_k = sca_k + sumsca_k
                    profiles_int_k(j)%cloud(icld,lay) = profiles_int_k(j)%cloud(icld,lay) + &
                        optp%optpwcl(icld)%sca(chan,1) * sca_k * coef_scatt_ir%confac(icld)
                  ENDIF
              ...
              IF (ice_scheme == ice_scheme_ssec) THEN
                ...
                ELSEIF (do_chan) THEN
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_k(musat, musat_k, musun, musun_k, relazi, zminphadiff, &
                                           ...
                                           coef_scatt_ir%icl_phfn_int%iphangle, phasint_k, icl_phfn_k)
                  ...
                  IF (do_chou_scaling_chan) THEN
                    ...
                    bpr_k = & !bpr_k + &
                        cld%partsca(icld,lay,j) * cld_k%opdpscabpr(lay,j)
                    dgfrac_k = dgfrac_k + bpr_k * (optp%optpicl%bpr(k2,chan) - optp%optpicl%bpr(k1,chan))
                  ENDIF
                  ...
                  sca_k = sca_k + raytracing%ltick(lay,prof) * cld_k%partsca(icld,lay,j)
                  profiles_int_k(j)%cloud(icld,lay) = profiles_int_k(j)%cloud(icld,lay) + &
                      sca_k * (optp%optpicl%sca(k1,chan) + dgfrac * &
                      (optp%optpicl%sca(k2,chan) - optp%optpicl%sca(k1,chan)))
                  dgfrac_k = dgfrac_k + sca_k * (optp%optpicl%sca(k2,chan) - optp%optpicl%sca(k1,chan)) * &
                ...
                IF (do_chan) THEN
                  IF (aux%ice_dg(lay,prof) >= optp%optpicl%fmv_icl_deff(1) .AND. &
                    ...
                    aux_k%ice_dg(lay,j) = aux_k%ice_dg(lay,j) + &
                        dgfrac_k / (optp%optpicl%fmv_icl_deff(k2) - optp%optpicl%fmv_icl_deff(k1))
!                     ELSE
!                       dgfrac_k = 0._jprb
                  ENDIF
                ENDIF
...
!                       legcoef(:) = legcoef(:) + thislegcoef(:) * sca
                ...
                IF (do_single_scatt_chan .OR. do_dom_chan) THEN
                  ...
                  IF (solar(j)) THEN
                    ...
                    IF (do_single_scatt_chan) THEN
                      ...
                      CALL int_phase_fn_k(musat, musat_k, musun, musun_k, relazi, zminphadiff, &
                                          ...
                                          optp%optpiclbaran2018%phfn_int%iphangle, phasint_k, baran_phfn_k)
                  ...
                  IF (do_dom_chan) THEN
                    ...
                    CALL normalise_ad(baran_ngauss, optp%optpiclbaran2018%w, baran_phfn_interp_unnorm, baran_phfn_interp_k)
                    ...
                    CALL spline_interp_ad(thisnphangle, thiscosphangle(thisnphangle:1:-1), &
                                          ...
                                          baran_ngauss, optp%optpiclbaran2018%q, baran_phfn_interp_k)
...
!     ENDIF
  ...
  IF (opts%rt_ir%addaerosl) THEN
    ...
    IF (opts%rt_ir%user_aer_opt_param) THEN
      ...
      CALL calc_user_opt_param_k(aer_opt_param, aer_opt_param_k, aer, aer_k, 0_jpim)
              ...
              IF (solar(j)) THEN
                ...
                IF (do_single_scatt_chan) THEN
                  ...
                  CALL int_phase_fn_k(musat, musat_k, musun, musun_k, relazi, zminphadiff, &
                                      ...
                                      coef_scatt_ir%aer_phfn_int%iphangle, phasint_k, aer_phfn_k)
...
CONTAINS
  ...
  SUBROUTINE calc_user_opt_param_k(opt_param, opt_param_k, aercld, aercld_k, iaercld)
    ...
    LOGICAL(jplm) :: do_chou_scaling_chan, do_dom_chan, do_single_scatt_chan, do_opt_param_k
        ...
        IF (opt_param%abs(lay,j) > 0._jprb .OR. &
          ...
          IF (do_opt_param_k) THEN
            IF (solar(j)) THEN
              ...
              IF (do_single_scatt_chan) THEN
                phasint_k = aercld_k%phtotdo(lay,j) * aercld%opdpsca(lay,j)
                aercld_k%opdpsca(lay,j) = aercld_k%opdpsca(lay,j) + aercld_k%phtotdo(lay,j) * aercld%phintdo(1,lay,j)
              ...
              phasint_k = aercld_k%phtotup(lay,j) * aercld%opdpsca(lay,j)
              aercld_k%opdpsca(lay,j) = aercld_k%opdpsca(lay,j) + aercld_k%phtotup(lay,j) * aercld%phintup(1,lay,j)
            ...
            IF (do_dom_chan) THEN
              ...
              opt_param_k%legcoef(1:nmom+1,lay,j) = opt_param_k%legcoef(1:nmom+1,lay,j) + &
                trans_scatt_ir_dyn_k%phasefn(j)%legcoef(0:nmom,iaercld,lay)
            ENDIF
            ...
            raytracing_k%ltick(lay,j) = raytracing_k%ltick(lay,j) + &
                                        opt_param%sca(lay,j) * aercld_k%opdpsca(lay,j)
            opt_param_k%sca(lay,j)    = opt_param_k%sca(lay,j) + &
                                        raytracing%ltick(lay,prof) * aercld_k%opdpsca(lay,j)
            raytracing_k%ltick(lay,j) = raytracing_k%ltick(lay,j) + &
