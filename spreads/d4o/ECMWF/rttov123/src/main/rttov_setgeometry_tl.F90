...
#include "rttov_locpat_tl.interface"
  ...
  IF (LHOOK) CALL DR_HOOK('RTTOV_SETGEOMETRY_TL', 0_jpim, ZHOOK_HANDLE)
  CALL rttov_locpat_tl( &
          ...
          raytracing_tl,   &
          do_pmc_calc)
  IF (LHOOK) CALL DR_HOOK('RTTOV_SETGEOMETRY_TL', 1_jpim, ZHOOK_HANDLE)
END SUBROUTINE rttov_setgeometry_tl
