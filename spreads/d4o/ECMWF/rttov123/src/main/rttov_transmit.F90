...
!! @param[in,out] od_level                    level-to-space optical depths
!! @param[in,out] transmission_levels         level-to-space transmittances
!! @param[in,out] transmission_total          surface-to-space transmittances
...
!! @param[in,out] tau_ref                     transmission_levels before any clipping is applied
!! @param[in,out] tau_ref_surf                tau_surf before any clipping is applied
!! @param[in,out] tau_surf                    surface-to-space transmittances used by TL/AD/K
!! @param[in,out] tau_level                   level-to-space transmittances used by TL/AD/K
!!
!
! Copyright:
...
!INTF_END
  ...
  INTEGER(KIND=jpim) :: lev, lay, chan, j, prof, ist, isti, levsurf
...
!cdir COLLAPSE
  ...
  IF (ALL(chanflag)) THEN
    ...
    tau_level(:,:) = tau_ref(:,:)
    transmission_levels(:,1:nchanprof) = tau_ref(:,:)
  ELSE
    DO j = 1, nchanprof
      IF (chanflag(j)) THEN
        CALL exponential(od_level(:,j), tau_ref(:,j))
        tau_level(:,j) = tau_ref(:,j)
        transmission_levels(:,j) = tau_ref(:,j)
      ENDIF
    ENDDO
  ENDIF
...
! all arrays here based on layers, not levels
        DO ist = 0, ircld%nstream(prof)
          od_surf_ac(ist,j) = transmission_scatt_ir_dyn%opdpac(levsurf,ist,j) + aux%s(prof)%pfraction_surf * &
              (transmission_scatt_ir_dyn%opdpac(levsurf-1,ist,j) - &
               transmission_scatt_ir_dyn%opdpac(levsurf,ist,j))

          IF (aux%s(prof)%pfraction_surf >= 0._jprb) THEN
            od_frac_ac = od_surf_ac(ist,j) - transmission_scatt_ir_dyn%opdpac(levsurf-1,ist,j)
          ELSE
            od_frac_ac = od_surf_ac(ist,j) - transmission_scatt_ir_dyn%opdpac(levsurf,ist,j)
          ENDIF
  ...
  IF (ANY(do_lambertian) .AND. .NOT. do_dom) THEN
    DO j = 1, nchanprof
      IF (chanflag(j) .AND. do_lambertian(j)) THEN
        ...
        IF (do_scatt) THEN
          DO ist = 0, ircld%nstream(prof)
            WHERE (transmission_scatt_ir_dyn%opdpac(:,ist,j) < max_optical_depth)
              opdpac = transmission_scatt_ir_dyn%opdpac(:,ist,j)
            ELSEWHERE
...
!---------------------------------------------------------------------------------------
  IF (do_chou) THEN
    WHERE (chanflag(:)) transmission_total(1:nchanprof) = transmission_aux_path%tau_surf(0,:)
    ...
    DO j = 1, nchanprof
      IF (chanflag(j)) THEN
        ...
        DO ist = 0, ircld%nstream(prof)
          CALL exponential(-transmission_scatt_ir_dyn%opdpac(:,ist,j), &
            ztemp(:,ist))
        ENDDO
        ...
        DO ist = 0, ircld%nstream(prof)
          transmission_aux_path%tau_level(:,ist,j) = tau_level(:,j) * ztemp(:,ist)
        ENDDO
        ...
        CALL reciprocal(transmission_aux_path%tau_surf(0:ircld%nstream(prof),j), &
                        transmission_aux_path%tau_surf_r(0:ircld%nstream(prof),j))

        transmission_levels(:,j) = transmission_aux_path%tau_level(:,0,j)
      ENDIF
    ENDDO

  ELSE ! clear-sky or DOM
    WHERE (chanflag(:)) transmission_total(1:nchanprof) = tau_surf(:)

    DO j = 1, nchanprof
      IF (chanflag(j)) THEN
        prof = chanprof(j)%prof
        ist = 0
        IF (.NOT. do_dom) transmission_aux_path%od_sfrac(ist,j) = -od_frac(j)
...
! stop floating overflow
      transmission_aux_path%od_sfrac_r(0:ircld%nstream(prof),j) = &
        1.0_jprb / (transmission_aux_path%od_sfrac(0:ircld%nstream(prof),j) + small_val)
    ENDIF
  ENDDO
...
! DAR: separated fac1 and fac2 becuase fac2 can be used separately from fac1 in Phil Watts calcs
          IF (transmission_aux_path%od_singlelayer(isti,lay,j) < min_od) THEN
            transmission_aux%fac1(lay,ist,j) = 0.0_jprb
          ELSE
            IF (opts%rt_all%dtau_test .AND. &
                transmission_aux_path%tau_level(lay,ist,j) - &
                transmission_aux_path%tau_level(lev,ist,j) < min_od) THEN
              transmission_aux%fac1(lay,ist,j) = 0.0_jprb
            ELSE
              transmission_aux%fac1(lay,ist,j) = 1.0_jprb
            ENDIF
          ENDIF
        ENDDO
      ENDDO
      WHERE (transmission_aux_path%tau_level(:,0:ircld%nstream(prof),j) < min_tau)
        transmission_aux_path%fac2(:,0:ircld%nstream(prof),j) = 0.0_jprb
      ELSEWHERE
        transmission_aux_path%fac2(:,0:ircld%nstream(prof),j) = 1.0_jprb
      ENDWHERE
    ENDIF
  ENDDO
  ...
  DO j = 1, nchanprof
    IF (chanflag(j)) THEN
      prof = chanprof(j)%prof
      WHERE (transmission_aux_path%tau_surf(0:ircld%nstream(prof),j) > min_tau)
        transmission_aux%surf_fac(0:ircld%nstream(prof),j) = 1.0_jprb
      ELSEWHERE
        transmission_aux%surf_fac(0:ircld%nstream(prof),j) = 0.0_jprb
      ENDWHERE
    ENDIF
  ENDDO
