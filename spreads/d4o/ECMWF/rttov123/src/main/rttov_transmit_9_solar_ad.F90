    ...
    IF (solar(j)) THEN
      ...
      IF (do_single_scatt) THEN
        DO lay = nlayers, 1, -1
          ...
          ENDWHERE
          DO isti = 0, 1
            IF (transmission_scatt_ir%opdpext(isti,lay,j) > 0._jprb) &
              raytracing_ad%patheff(lay,prof) = &
                raytracing_ad%patheff(lay,prof) + &
                  transmission_scatt_ir_ad%ssa_solar(isti,lay,j) * transmission_scatt_ir%opdpsca(isti,lay,j) / &
                  transmission_scatt_ir%opdpext(isti,lay,j)
        ...
        DO ist = ircld%nstream(prof), 0, -1
          DO lev = nlevels, 1, -1
            IF (lev > 1) THEN
              ...
              tauacsunpath1 = EXP(-transmission_scatt_ir_dyn%opdpacsun(lev,ist,j) * &
                  raytracing%pathsat(lay,prof) / raytracing%patheff(lay,prof))

              tau_level_ad(lev,j) = tau_level_ad(lev,j) +      &
                  tauacsunpath1 * transmission_aux_ad%solar_path1%tau_level(lev,ist,j)

              transmission_scatt_ir_dyn_ad%opdpacsun(lev,ist,j) = &
                  transmission_scatt_ir_dyn_ad%opdpacsun(lev,ist,j) - &
              ...
              pathsat_patheff_ad(lay,prof) = pathsat_patheff_ad(lay,prof) - &
                  tauacsunpath1 * path1%tau_level(lev,j) * &
                  transmission_aux_ad%solar_path1%tau_level(lev,ist,j) * &
                  transmission_scatt_ir_dyn%opdpacsun(lev,ist,j)
            ELSE
            ...
            tausun_level_ad(lev,j) = tausun_level_ad(lev,j) + &
                ...
                EXP(-transmission_scatt_ir_dyn%opdpacsun(lev,ist,j))
            transmission_scatt_ir_dyn_ad%opdpacsun(lev,ist,j) = &
                transmission_scatt_ir_dyn_ad%opdpacsun(lev,ist,j) - &
      ...
      transmission_aux_ad%solar_path2%tau_level(:,0:ircld%nstream(prof),j) = 0._jprb
    ENDIF
  ENDDO
        ...
        DO ist = ircld%nstream(prof), 0, -1
          ...
          IF (do_single_scatt) THEN
            ...
            IF (aux%s(prof)%pfraction_surf >= 0._jprb) THEN
              ...
              transmission_scatt_ir_dyn_ad%opdpacsun(levsurf-1,ist,j) = &
                  transmission_scatt_ir_dyn_ad%opdpacsun(levsurf-1,ist,j) - &
                  transmission_aux_ad%solar_path2%od_frac_ac(ist,j)
            ELSE
              od_surf_ac_ad(j) = &
                  od_surf_ac_ad(j) + transmission_aux_ad%solar_path2%od_frac_ac(ist,j)
              transmission_scatt_ir_dyn_ad%opdpacsun(levsurf,ist,j) = &
                  transmission_scatt_ir_dyn_ad%opdpacsun(levsurf,ist,j) - &
                  transmission_aux_ad%solar_path2%od_frac_ac(ist,j)
            ENDIF
          ENDIF
          ...
          od_surf_ac = transmission_scatt_ir_dyn%opdpacsun(levsurf,ist,j) + aux%s(prof)%pfraction_surf * ( &
          ...
          aux_ad%s(prof)%pfraction_surf = aux_ad%s(prof)%pfraction_surf + &
              od_surf_ac_ad(j) * (transmission_scatt_ir_dyn%opdpacsun(levsurf-1,ist,j) - &
              transmission_scatt_ir_dyn%opdpacsun(levsurf,ist,j))
...
!-------------------------------------------
  ...
  DO j = 1, nchanprof
    IF (solar(j)) THEN
      chan = chanprof(j)%chan
      od_level_ad(:,j) = od_level_ad(:,j) + coef%ff_gam(chan) * tausun_level_ad(:,j) * path2%tau_ref(:,j)
