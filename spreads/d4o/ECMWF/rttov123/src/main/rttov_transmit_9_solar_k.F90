...
!---------------------------------------------------------------------------------------
  ...
  DO j = 1, nchanprof
    ...
    IF (solar(j)) THEN
          ...
          DO isti = 0, 1
            IF (transmission_scatt_ir%opdpext(isti,lay,j) > 0._jprb) &
              raytracing_k%patheff(lay,j) = &
                raytracing_k%patheff(lay,j) + &
                  transmission_scatt_ir_k%ssa_solar(isti,lay,j) * transmission_scatt_ir%opdpsca(isti,lay,j) / &
        ...
        DO ist = ircld%nstream(prof), 0, -1
          DO lev = nlevels, 1, -1
            IF (lev > 1) THEN
              ...
              transmission_scatt_ir_dyn_k%opdpacsun(lev,ist,j) = &
                  transmission_scatt_ir_dyn_k%opdpacsun(lev,ist,j) - &
      ...
      transmission_aux_k%solar_path2%tau_level(:,0:ircld%nstream(prof),j) = 0._jprb
    ENDIF
  ENDDO
        ...
        DO ist = ircld%nstream(prof), 0, -1
          ...
          IF (do_single_scatt) THEN
            ...
            IF (aux%s(prof)%pfraction_surf >= 0._jprb) THEN
              ...
              transmission_scatt_ir_dyn_k%opdpacsun(levsurf-1,ist,j) = &
                  transmission_scatt_ir_dyn_k%opdpacsun(levsurf-1,ist,j) - &
                  transmission_aux_k%solar_path2%od_frac_ac(ist,j)
            ELSE
              od_surf_ac_k(j) = &
                  od_surf_ac_k(j) + transmission_aux_k%solar_path2%od_frac_ac(ist,j)
              transmission_scatt_ir_dyn_k%opdpacsun(levsurf,ist,j) = &
                  transmission_scatt_ir_dyn_k%opdpacsun(levsurf,ist,j) - &
                  transmission_aux_k%solar_path2%od_frac_ac(ist,j)
            ENDIF
          ENDIF
...
!-------------------------------------------
  ...
  DO j = 1, nchanprof
    IF (solar(j)) THEN
      chan = chanprof(j)%chan
      od_level_k(:,j) = od_level_k(:,j) + coef%ff_gam(chan) * tausun_level_k(:,j) * path2%tau_ref(:,j)
