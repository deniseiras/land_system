...
!----------------------------------------
  DO j = 1, nchanprof
    IF (solar(j)) THEN
      ...
      tausun_level_tl(:,j) = od_level_tl(:,j) * path2%tau_ref(:,j)
    ENDIF
  ENDDO
    ...
    IF (solar(j)) THEN
      ...
      IF (do_scatt) THEN
        DO ist = 0, ircld%nstream(prof)
          od_surf_ac_tl(j) = transmission_scatt_ir_dyn_tl%opdpacsun(levsurf,ist,j) + aux%s(prof)%pfraction_surf * &
              (transmission_scatt_ir_dyn_tl%opdpacsun(levsurf-1,ist,j) - &
               transmission_scatt_ir_dyn_tl%opdpacsun(levsurf,ist,j)) + aux_tl%s(prof)%pfraction_surf * &
          ...
          IF (do_single_scatt) THEN
            IF (aux%s(prof)%pfraction_surf >= 0._jprb) THEN
              transmission_aux_tl%solar_path2%od_frac_ac(ist,j) = &
                  od_surf_ac_tl(j) - transmission_scatt_ir_dyn_tl%opdpacsun(levsurf-1,ist,j)
            ELSE
              transmission_aux_tl%solar_path2%od_frac_ac(ist,j) = &
                  od_surf_ac_tl(j) - transmission_scatt_ir_dyn_tl%opdpacsun(levsurf,ist,j)
            ENDIF
        ...
        DO ist = 0, ircld%nstream(prof)
          DO lev = 1, nlevels
            transmission_aux_tl%solar_path2%tau_level(lev,ist,j) = &
                ...
                transmission_scatt_ir_dyn_tl%opdpacsun(lev,ist,j) * path2%tau_level(lev,j)) * &
            ...
            IF (lev > 1) THEN
              ...
              transmission_aux_tl%solar_path1%tau_level(lev,ist,j) =      &
                  ...
                  (-transmission_scatt_ir_dyn_tl%opdpacsun(lev,ist,j) * &
                  raytracing%pathsat(lay,prof) / raytracing%patheff(lay,prof) + &
                  (-transmission_scatt_ir_dyn%opdpacsun(lev,ist,j) * pathsat_patheff_tl(lay,prof))))
            ELSE
