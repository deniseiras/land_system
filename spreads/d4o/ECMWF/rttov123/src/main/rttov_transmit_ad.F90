...
!! @param[in,out] transmission_levels_ad         level-to-space transmittance increments
!! @param[in,out] transmission_total_ad          surface-to-space transmittance increments
...
!INTF_END
  ...
  INTEGER(KIND=jpim) :: lev, lay, chan, prof, j, levsurf, ist
...
!start AD
        ...
        DO ist = 0, ircld%nstream(prof)
          CALL exponential_ad(-ztemp(:,ist) , &
                             transmission_scatt_ir_dyn_ad%opdpac(:,ist,j), &
  ...
  ELSE
    DO j = 1, nchanprof
      IF (chanflag(j)) THEN
        ...
        IF (.NOT. do_dom) THEN
          ist = 0
          tau_level_ad(:,j) = tau_level_ad(:,j) + transmission_aux_path_ad%tau_level(:,ist,j)
          ...
          DO ist = 0, ircld%nstream(prof)
            ...
            IF (.NOT. opts%rt_all%lambertian_fixed_angle) THEN
              ...
              ref_power_ad = ref_power_ad + (od_surf - od_surf_ac) * &
                transmission_aux_path%tau_surf_p(ist,j) * transmission_aux_path_ad%tau_surf_p(ist,j)

              ref_power_ad = ref_power_ad + SUM((od_level(:,j) - transmission_scatt_ir_dyn%opdpac(:,ist,j)) * &
                transmission_aux_path%tau_level_p(:,ist,j) * transmission_aux_path_ad%tau_level_p(:,ist,j))
            ENDIF
          ENDDO
        ELSE
          od_surf_ad(j) = od_surf_ad(j) + &
      ...
      IF (chanflag(j)) THEN
        ...
        DO ist = ircld%nstream(prof), 0, -1
          ...
          IF (aux%s(prof)%pfraction_surf >= 0._jprb) THEN
            ...
            transmission_scatt_ir_dyn_ad%opdpac(levsurf-1, ist,j) =     &
                transmission_scatt_ir_dyn_ad%opdpac(levsurf-1, ist,j) - od_frac_ac_ad
          ELSE
            od_surf_ac_ad(ist,j) = od_surf_ac_ad(ist,j) + od_frac_ac_ad
            transmission_scatt_ir_dyn_ad%opdpac(levsurf, ist,j) =     &
                transmission_scatt_ir_dyn_ad%opdpac(levsurf, ist,j) - od_frac_ac_ad
          ENDIF
          transmission_scatt_ir_dyn_ad%opdpac(levsurf, ist,j)     =      &
              transmission_scatt_ir_dyn_ad%opdpac(levsurf, ist,j) +      &
