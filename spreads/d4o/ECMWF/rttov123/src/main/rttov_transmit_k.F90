...
!! @param[in,out] transmission_levels_k          level-to-space transmittance increments
!! @param[in,out] transmission_total_k           surface-to-space transmittance increments
...
!start AD
        ...
        DO ist = 0, ircld%nstream(prof)
          CALL exponential_ad(-ztemp(:,ist), &
                             transmission_scatt_ir_dyn_k%opdpac(:,ist,j), &
  ...
  ELSE
    DO j = 1, nchanprof
      IF (chanflag(j)) THEN
        ...
        IF (.NOT. do_dom) THEN
          ist = 0
          tau_level_k(:,j) = tau_level_k(:,j) + transmission_aux_path_k%tau_level(:,ist,j)
          ...
          DO ist = 0, ircld%nstream(prof)
            ...
            IF (.NOT. opts%rt_all%lambertian_fixed_angle) THEN
              ...
              ref_power_k = ref_power_k + (od_surf - od_surf_ac) * &
                transmission_aux_path%tau_surf_p(ist,j) * transmission_aux_path_k%tau_surf_p(ist,j)

              ref_power_k = ref_power_k + SUM((od_level(:,j) - transmission_scatt_ir_dyn%opdpac(:,ist,j)) * &
                transmission_aux_path%tau_level_p(:,ist,j) * transmission_aux_path_k%tau_level_p(:,ist,j))
            ENDIF
          ENDDO
        ELSE
          od_surf_k(j) = od_surf_k(j) + &
      ...
      IF (chanflag(j)) THEN
        ...
        DO ist = ircld%nstream(prof), 0, -1
          ...
          IF (aux%s(prof)%pfraction_surf >= 0._jprb) THEN
            ...
            transmission_scatt_ir_dyn_k%opdpac(levsurf-1, ist,j) =     &
                transmission_scatt_ir_dyn_k%opdpac(levsurf-1, ist,j) - od_frac_ac_k
          ELSE
            od_surf_ac_k(ist,j) = od_surf_ac_k(ist,j) + od_frac_ac_k
            transmission_scatt_ir_dyn_k%opdpac(levsurf, ist,j) =      &
                transmission_scatt_ir_dyn_k%opdpac(levsurf, ist,j) - od_frac_ac_k
          ENDIF
          transmission_scatt_ir_dyn_k%opdpac(levsurf, ist,j)     =      &
              transmission_scatt_ir_dyn_k%opdpac(levsurf, ist,j) +      &
