...
!! @param[in,out] transmission_levels_tl         level-to-space transmittance perturbations
!! @param[in,out] transmission_total_tl          surface-to-space transmittance perturbations
...
!! @param[in]     tau_ref                        transmission_levels before any clipping is applied
!! @param[in]     tau_ref_surf                   tau_surf before any clipping is applied
...
!INTF_END
  ...
  INTEGER(KIND=jpim) :: lay, chan, prof, ist, j, levsurf
...
! - in rttov_opdep, already checked that values are sensible
  DO j = 1, nchanprof
    IF (chanflag(j)) THEN
      ...
      tau_level_tl(:,j) = od_level_tl(:,j) * tau_ref(:,j)
    ENDIF
  ENDDO
    ...
    DO j = 1, nchanprof
      IF (chanflag(j)) THEN
        ...
        DO ist = 0, ircld%nstream(prof)
          od_surf_ac_tl(ist,j) = transmission_scatt_ir_dyn_tl%opdpac(levsurf,ist,j) + aux%s(prof)%pfraction_surf * &
              (transmission_scatt_ir_dyn_tl%opdpac(levsurf-1,ist,j) -                                              &
               transmission_scatt_ir_dyn_tl%opdpac(levsurf,ist,j)) + aux_tl%s(prof)%pfraction_surf *               &
              (transmission_scatt_ir_dyn%opdpac(levsurf-1,ist,j) -                                                 &
               transmission_scatt_ir_dyn%opdpac(levsurf,ist,j))

          IF (aux%s(prof)%pfraction_surf >= 0._jprb) THEN
            od_frac_ac_tl = od_surf_ac_tl(ist,j) - transmission_scatt_ir_dyn_tl%opdpac(levsurf-1,ist,j)
          ELSE
            od_frac_ac_tl = od_surf_ac_tl(ist,j) - transmission_scatt_ir_dyn_tl%opdpac(levsurf,ist,j)
          ENDIF
        ...
        IF (do_chou) THEN
          DO ist = 0, ircld%nstream(prof)
            IF (opts%rt_all%lambertian_fixed_angle .OR. ref_power_tl == 0._jprb) THEN
              CALL exponential_tl((od_level_tl(:,j) - transmission_scatt_ir_dyn_tl%opdpac(:,ist,j)) * ref_power, &
            ...
            ELSE
              CALL exponential_tl((od_level_tl(:,j) - transmission_scatt_ir_dyn_tl%opdpac(:,ist,j)) * ref_power + &
                (od_level(:,j) - transmission_scatt_ir_dyn%opdpac(:,ist,j)) * ref_power_tl, &
                transmission_aux_path%tau_level_p(:,ist,j), &
                transmission_aux_path_tl%tau_level_p(:,ist,j))

              levsurf = aux%s(prof)%nearestlev_surf
              od_surf_ac = transmission_scatt_ir_dyn%opdpac(levsurf,ist,j) + aux%s(prof)%pfraction_surf * &
                  (transmission_scatt_ir_dyn%opdpac(levsurf-1,ist,j) - &
                   transmission_scatt_ir_dyn%opdpac(levsurf,ist,j))
      ...
      IF (chanflag(j)) THEN
        ...
        DO ist = 0, ircld%nstream(prof)
          ...
          CALL exponential_tl(ztemp(:,ist) , &
                             -transmission_scatt_ir_dyn_tl%opdpac(:,ist,j), &
                             ztemp_tl(:,ist))
        ENDDO
  ...
  ELSE ! clear-sky or DOM
    ...
    DO j = 1, nchanprof
      IF (chanflag(j)) THEN
        ...
        IF (.NOT. do_dom) THEN
          ist = 0
          transmission_aux_path_tl%tau_level(:,ist,j) = tau_level_tl(:,j)
          transmission_aux_path_tl%tau_surf(ist,j) = tau_surf_tl(j)
