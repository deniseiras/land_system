...
MODULE rttov_types
  ...
  TYPE rttov_skin
    ...
    REAL(jprb)    :: specularity       !< Mix of specular/Lambertian for do_lambertian option (0-1, 0=Lambertian/1=specular)
  END TYPE rttov_skin
  ...
  TYPE rttov_profile_cloud
    ...
    REAL(jprb), POINTER :: cc(:)         => NULL() !< nlevels of cloud cover (0-1)
    REAL(jprb), POINTER :: clw(:)        => NULL() !< nlevels of cloud liquid water (kg/kg)
    REAL(jprb), POINTER :: ciw(:)        => NULL() !< nlevels of cloud ice water (kg/kg)
    REAL(jprb), POINTER :: totalice(:)   => NULL() !< nlevels of total ice (kg/kg)
    REAL(jprb), POINTER :: rain(:)       => NULL() !< nlevels of rain (units: see mmr_snowrain)
    REAL(jprb), POINTER :: sp(:)         => NULL() !< nlevels of solid precipitation (units: see mmr_snowrain)
  ...
  TYPE rttov_scatt_emis_retrieval_type
    ...
    REAL(jprb), POINTER  :: up_cld(:)   => NULL() !< TOA upwelling radiance or TB from atmosphere (not inc. surface
                                                  !!  emission or reflection)
    REAL(jprb), POINTER  :: down_cld(:) => NULL() !< SFC downwelling radiance or TB (inc. cosmic term)
    REAL(jprb), POINTER  :: tau_clr(:)  => NULL() !< Along-path transmittance, surface to space (clear column)
    REAL(jprb), POINTER  :: up_clr(:)   => NULL() !< TOA upwelling radiance or TB from atmosphere (not inc. surface
                                                  !!  emission or reflection)
    REAL(jprb), POINTER  :: down_clr(:) => NULL() !< SFC downwelling radiance or TB (inc. cosmic term)
  ...
  TYPE rttov_pccomp
    REAL(jprb), POINTER  :: pcscores(:)            => NULL() !< Principal component (PC) scores
  ...
  TYPE rttov_opts_pc
    ...
    INTEGER(jpim) :: npcscores = -1        !< Number of PC scores to compute, if less than 1 npcscores is derived
  ...
  TYPE rttov_opts_rt_ir
    ...
    LOGICAL(jplm) :: grid_box_avg_cloud    = .FALSE.   !< Switch to supply grid-box average cloud concentration or cloud
    ...
    REAL(jprb)    :: cldstr_low_cloud_top  = 750._jprb !< Upper pressure limit for cldstr_simple option (hPa)
  ...
  TYPE rttov_options_scatt
    ...
    REAL(jprb)     :: cc_threshold          = 0.05_jprb     !< Minimum effective cloud fraction threshold to consider scattering
    LOGICAL(jplm)  :: apply_band_correction = .TRUE.        !< Apply band-correction for Planck radiance and BT calculations
    LOGICAL(jplm)  :: use_q2m               = .TRUE.        !< Switch to enable use of 2m q variable
    LOGICAL(jplm)  :: rad_down_lin_tau      = .TRUE.        !< Linear-in-tau or layer-mean for downwelling radiances
    LOGICAL(jplm)  :: dtau_test             = .TRUE.        !< Switch to apply dtau test in transmit/integrate calculations
  ...
  TYPE rttov_coef
    ...
    INTEGER(jpim), POINTER      :: fmv_lvl(:)     => NULL() ! number of levels(pres/absorber) by gas (fmv_gas)
    ...
    TYPE(rttov_fast_coef), POINTER :: thermal(:) => NULL() ! FAST_COEFFICIENTS
    ...
    REAL(jprb), POINTER      :: tstar(:)   => NULL() ! layer temp (K)
    REAL(jprb), POINTER      :: to3star(:) => NULL() ! layer temp for O3 calculations (K)
    REAL(jprb), POINTER      :: wstar(:)   => NULL() ! layer WV  (ppmv)
    REAL(jprb), POINTER      :: ostar(:)   => NULL() ! layer O3  (ppmv)
    REAL(jprb), POINTER      :: co2star(:) => NULL() ! layer co2 (ppmv)
    REAL(jprb), POINTER      :: n2ostar(:) => NULL() ! layer n2o (ppmv)
    REAL(jprb), POINTER      :: costar(:)  => NULL() ! layer co  (ppmv)
    REAL(jprb), POINTER      :: ch4star(:) => NULL() ! layer ch4 (ppmv)
    REAL(jprb), POINTER      :: so2star(:) => NULL() ! layer so2 (ppmv)
  ...
  TYPE rttov_scatt_coef
    ...
    REAL(jprb), POINTER :: mie_freq(:)  => NULL() ! list of frequencies in Mie table
  ...
  TYPE rttov_profile_aux
    ...
    REAL(jprb), POINTER :: fac1_ice_dg(:,:) => NULL() ! Intermediate variables used to compute the
    REAL(jprb), POINTER :: fac2_ice_dg(:,:) => NULL() !   generalized diameter
    REAL(jprb), POINTER :: fac3_ice_dg(:,:) => NULL()
    ...
    REAL(jprb), POINTER :: co2_layer(:,:) => NULL() ! avg layer co2 (ppmv)
    REAL(jprb), POINTER :: dt(:,:)        => NULL() ! deviation from ref temperature prof (K)
    REAL(jprb), POINTER :: dto(:,:)       => NULL() ! deviation from ref ozone temperature prof (K)
    REAL(jprb), POINTER :: tr(:,:)        => NULL() ! ratio t / ref_t
    REAL(jprb), POINTER :: tr_r(:,:)      => NULL()
    REAL(jprb), POINTER :: tr_sqrt(:,:)   => NULL()
    REAL(jprb), POINTER :: wr(:,:)        => NULL()
    REAL(jprb), POINTER :: wr_sqrt(:,:)   => NULL()
    REAL(jprb), POINTER :: wr_rsqrt(:,:)  => NULL()
    REAL(jprb), POINTER :: or(:,:)        => NULL()
    REAL(jprb), POINTER :: or_sqrt(:,:)   => NULL()
    REAL(jprb), POINTER :: tw(:,:)        => NULL()
    REAL(jprb), POINTER :: tw_sqrt(:,:)   => NULL()
    REAL(jprb), POINTER :: tw_4rt(:,:)    => NULL()
    REAL(jprb), POINTER :: ww(:,:)        => NULL()
    REAL(jprb), POINTER :: ww_r(:,:)      => NULL()
    REAL(jprb), POINTER :: wwr(:,:)       => NULL()
    REAL(jprb), POINTER :: wwr_r(:,:)     => NULL()
    REAL(jprb), POINTER :: ow(:,:)        => NULL()
    REAL(jprb), POINTER :: ow_r(:,:)      => NULL()
    REAL(jprb), POINTER :: ow_sqrt(:,:)   => NULL()
    REAL(jprb), POINTER :: ow_rsqrt(:,:)  => NULL()
    REAL(jprb), POINTER :: co2r(:,:)      => NULL()
    REAL(jprb), POINTER :: co2w(:,:)      => NULL()
    REAL(jprb), POINTER :: twr(:,:)       => NULL()
    REAL(jprb), POINTER :: sum(:,:)       => NULL()
  ...
  TYPE rttov_profile_scatt_aux
    ...
    REAL(jprb), POINTER :: clw(:,:)      => NULL() ! cloud liquid water (g/m3)
    REAL(jprb), POINTER :: ciw(:,:)      => NULL() ! cloud ice water (g/m3)
    REAL(jprb), POINTER :: totalice(:,:) => NULL() ! total ice (g/m3)
    REAL(jprb), POINTER :: rain(:,:)     => NULL() ! rain (g/m3)
    REAL(jprb), POINTER :: sp(:,:)       => NULL() ! solid precipitation (g/m3)
  ...
  TYPE eddington_sfc_type
    ...
    REAL(jprb), POINTER  :: up(:)   => NULL() ! TOA upwelling radiance or TB
    REAL(jprb), POINTER  :: down(:) => NULL() ! SFC downwelling radiance or TB
  ...
  TYPE rttov_path_transmission
    REAL(jprb), POINTER  :: tau_surf_p(:,:)         => NULL() ! Lambertian transmittance from surface (streams,channels)
    REAL(jprb), POINTER  :: tau_surf_p_r(:,:)       => NULL() ! reciprocal Lambertian trans. from surface (streams,channels)
    REAL(jprb), POINTER  :: tau_surf(:,:)           => NULL() ! transmittance from surface (streams,channels)
    REAL(jprb), POINTER  :: tau_surf_r(:,:)         => NULL() ! reciprocal transmittance from surface (streams,channels)
  ...
  TYPE rttov_raytracing
    ...
    REAL(jprb), POINTER :: hgpl(:,:)          => NULL() ! (levels,profiles) Level geopotential height (km)
    ...
    REAL(jprb), POINTER :: co2_cm(:)          => NULL() ! (profiles) CO2 thickness at STP
  ...
  TYPE rttov_transmission_scatt_ir
    ...
    REAL(jprb), POINTER :: opdpac(:,:,:)          => NULL() ! aer/aer+cld accumulated optical depth (nlev,0:nstream,nchan)
    REAL(jprb), POINTER :: opdpacl(:,:,:)         => NULL() ! aer/aer+cld layer optical depth (0:1,nlay,nchan)
    REAL(jprb), POINTER :: opdpacsun(:,:,:)       => NULL() ! aer/aer+cld accumulated op dep (solar path) (nlev,0:nstream,nchan)
  ...
  TYPE rttov_coef_scatt_ir
    ...
    CHARACTER(LEN=4), POINTER :: fmv_aer_comp_name(:) => NULL() ! Particle type names
    CHARACTER(LEN=4), POINTER :: fmv_wcl_comp_name(:) => NULL()
    INTEGER(jpim), POINTER    :: fmv_aer_rh(:)        => NULL() ! Number of relative humidity values for each particle type
    INTEGER(jpim), POINTER    :: fmv_wcl_rh(:)        => NULL()
    REAL(jprb),    POINTER    :: fmv_aer_rh_val(:)    => NULL() ! Relative humidity values for each particle type (%)
    REAL(jprb),    POINTER    :: fmv_wcl_rh_val(:)    => NULL()
    REAL(jprb),    POINTER    :: fmv_wcldeff_deff(:)  => NULL() ! Water cloud effective diameter values (microns)
    REAL(jprb),    POINTER    :: fmv_icl_deff(:)      => NULL() ! Ice cloud effective diameter values (microns)
    INTEGER(jpim), POINTER    :: aer_pha_chanlist(:)  => NULL() ! List of channels for which phase functions are stored
    INTEGER(jpim), POINTER    :: wcl_pha_chanlist(:)     => NULL()
    INTEGER(jpim), POINTER    :: wcldeff_pha_chanlist(:) => NULL()
    INTEGER(jpim), POINTER    :: icl_pha_chanlist(:)     => NULL()
    INTEGER(jpim), POINTER    :: aer_pha_index(:)        => NULL() ! Channel indices for stored phase fns
    INTEGER(jpim), POINTER    :: wcl_pha_index(:)        => NULL()
    INTEGER(jpim), POINTER    :: wcldeff_pha_index(:)    => NULL()
    INTEGER(jpim), POINTER    :: icl_pha_index(:)        => NULL()
    REAL(jprb),    POINTER    :: aer_phangle(:)          => NULL() ! Angles over which phase fns are defined
    REAL(jprb),    POINTER    :: wcl_phangle(:)          => NULL()
    REAL(jprb),    POINTER    :: wcldeff_phangle(:)      => NULL()
    REAL(jprb),    POINTER    :: icl_phangle(:)          => NULL()
    ...
    REAL(jprb),    POINTER    :: abs(:,:)                => NULL() ! Absorption coefs normalised to 1 part.cm-3 (km-1/part.cm-3)
    REAL(jprb),    POINTER    :: sca(:,:)                => NULL() ! Scattering coefs normalised to 1 part.cm-3 (km-1/part.cm-3)
    REAL(jprb),    POINTER    :: bpr(:,:)                => NULL() ! Backscattering parameters for Chou-scaling
    INTEGER(jpim), POINTER    :: nmom(:,:)               => NULL() ! Number of Legendre moments for each phase function
    REAL(jprb),    POINTER    :: legcoef(:,:,:)          => NULL() ! Legendre coefficients for each phase function
    REAL(jprb),    POINTER    :: pha(:,:,:)              => NULL() ! Phase functions
    REAL(jprb),    POINTER    :: confac(:)               => NULL() ! Cloud liquid water unit conversion factors (part.cm-3/g.m-3)
    REAL(jprb),    POINTER    :: aer_mmr2nd(:)           => NULL() ! Aerosol unit conversion factors (g.m-3/part.cm-3)
  ...
  TYPE rttov_coef_optpcld
    ...
    REAL(jprb),    POINTER  :: q(:) => NULL(), w(:) => NULL() ! Gaussian quadrature to calculate phase fn Leg. coefs
  ...
  TYPE rttov_optpar_ir
    TYPE(rttov_coef_scatt_ir), POINTER :: optpaer(:)       => NULL()
    TYPE(rttov_coef_scatt_ir), POINTER :: optpwcl(:)       => NULL()
    TYPE(rttov_coef_scatt_ir), POINTER :: optpwcldeff      => NULL()
    TYPE(rttov_coef_scatt_ir), POINTER :: optpicl          => NULL()
    TYPE(rttov_coef_optpcld),  POINTER :: optpiclbaran     => NULL()
    TYPE(rttov_coef_optpcld),  POINTER :: optpiclbaran2018 => NULL()
  ...
  TYPE rttov_coef_htfrtc
    ...
    REAL(jpim), POINTER    :: coef_pdt(:,:)       => NULL()
    REAL(jpim), POINTER    :: val_mean(:)         => NULL()
    REAL(jpim), POINTER    :: val_norm(:)         => NULL()
    INTEGER(jpim)          :: n_ch
    REAL(jprb), POINTER    :: sensor_freq(:)      => NULL()
    REAL(jprb), POINTER    :: ch_mean(:)          => NULL()
    REAL(jpim), POINTER    :: pc(:,:)             => NULL()
    REAL(jprb), POINTER    :: coef_aux(:)         => NULL()
    REAL(jprb), POINTER    :: mixed_ref_frac(:,:) => NULL()
    REAL(jprb), POINTER    :: solar(:)            => NULL()
    INTEGER(jpim)          :: n_mftlb
    REAL(jprb), POINTER    :: mftlb(:)            => NULL()
    INTEGER(jpim)          :: n_alpha
    REAL(jprb), POINTER    :: alpha(:)            => NULL()
    INTEGER(jpim)          :: n_gen_dim
    INTEGER(jpim)          :: n_fit_dim
    REAL(jprb), POINTER    :: gen_val(:,:)        => NULL()
  ...
  TYPE rttov_ircld
    INTEGER(jpim), POINTER  :: nstream(:)        => NULL()
    INTEGER(jpim), POINTER  :: nstreamref(:)     => NULL()
    INTEGER(jpim), POINTER  :: iloop(:)          => NULL()
    INTEGER(jpim), POINTER  :: icount(:)         => NULL()
    INTEGER(jpim), POINTER  :: icounstr(:)       => NULL()
    INTEGER(jpim), POINTER  :: icount1(:)        => NULL()
    REAL(jprb)   , POINTER  :: xstrclr(:)        => NULL()
    INTEGER(jpim), POINTER  :: icldarr   (:,:,:) => NULL()
    REAL(jprb)   , POINTER  :: xstrref1  (:,:,:) => NULL()
    REAL(jprb)   , POINTER  :: xstrref2  (:,:,:) => NULL()
    INTEGER(jpim), POINTER  :: indexstr  (:,:)   => NULL()
    INTEGER(jpim), POINTER  :: icount1ref(:,:)   => NULL()
    INTEGER(jpim), POINTER  :: iloopin   (:,:)   => NULL()
    INTEGER(jpim), POINTER  :: iflag     (:,:)   => NULL()
    REAL(jprb)   , POINTER  :: xstr      (:,:)   => NULL()
    REAL(jprb)   , POINTER  :: xstrminref(:,:)   => NULL()
    REAL(jprb)   , POINTER  :: xstrref   (:,:)   => NULL()
    REAL(jprb)   , POINTER  :: cldcfr    (:,:)   => NULL()
    REAL(jprb)   , POINTER  :: maxcov    (:,:)   => NULL()
    REAL(jprb)   , POINTER  :: xstrmax   (:,:)   => NULL()
    REAL(jprb)   , POINTER  :: xstrmin   (:,:)   => NULL()
  ...
  TYPE rttov_dom_state
    ...
    ! Solar only
    INTEGER(jpim), POINTER :: nazloops(:)      => NULL() ! 0:nstreams
    REAL(jprb),    POINTER :: z(:,:,:,:)       => NULL() ! nstr,0:profiles(1)%nlayers,0:1,0:nstr-1

    ! Thermal only
    REAL(jprb),    POINTER :: y0(:,:,:)        => NULL() ! nstr,nlayers,0:nstreams
    REAL(jprb),    POINTER :: y1(:,:,:)        => NULL() ! nstr,nlayers,0:nstreams

    ! Thermal and solar
    REAL(jprb),    POINTER :: thisrad(:)       => NULL() ! 0:nstreams
    REAL(jprb),    POINTER :: radsurfup(:)     => NULL() ! 0:nstreams
    REAL(jprb),    POINTER :: radsurfup_sum(:) => NULL() ! 0:nstreams
    REAL(jprb),    POINTER :: x(:,:,:)         => NULL() ! nstr*maxnlayers, 0:naz, 0:nstreams
...
!
  ...
  TYPE rttov_path_traj_trans
    ...
    REAL(jprb),     POINTER :: tau_ref       (:,:) => NULL()
    REAL(jprb),     POINTER :: tau_ref_surf  (:)   => NULL()
    ...
    REAL(jprb),     POINTER :: opdp_ref_coef (:,:) => NULL() ! layer optical depth before threshold
...
!
    ...
    TYPE(rttov_geometry), POINTER        :: angles_coef(:)       => NULL()
