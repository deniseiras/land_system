...
!   (c) a soft link to the relevant Mie coefficients, e.g.
!            mietable_noaa_amsua.dat
!
...
call rttov_alloc_scatt_prof( err, nprof, cld_profiles, nlev, .false._jplm, 1_jpim, init = .true._jplm)
...
call rttov_alloc_scatt_prof( err, nchannels, cld_profiles_k, nlev, .false._jplm, 1_jpim, init = .true._jplm)
...
call rttov_alloc_scatt_prof   ( err, nprof, cld_profiles, nlev, .false., 0_jpim)
call rttov_alloc_rad          ( err, nchannels, radiance, nlev,          0_jpim)
call rttov_alloc_prof         ( err, nchannels, profiles_k, nlev, opts,    0_jpim)
call rttov_alloc_scatt_prof   ( err, nchannels, cld_profiles_k, nlev, .false., 0_jpim)
