...
!* Local variables
  ...
  if (lradiance) then
    ...
    ! Previous and current default of computations in TB (Rayleigh-Jeans approx.) to be removed at v.13
    do ichan = 1, nchannels
      iprof   = chanprof(ichan)%prof
      if (adk == adk_adjoint) then
        iprofad = iprof  
      ...
      scatt_aux%b0(ichan,:) = scatt_aux%tbd(ichan,2:nlevels+1)
      scatt_aux%bn(ichan,:) = scatt_aux%tbd(ichan,1:nlevels  )

      scatt_aux%b1(ichan,:) = (scatt_aux%bn(ichan,:) - scatt_aux%b0(ichan,:)) / scatt_aux%dz(iprof,:)

      scatt_aux%bsfc(ichan) = scatt_aux%tsfc(ichan)
      scatt_aux%btop(ichan) = scatt_aux%tcosmic(ichan) 

      scatt_aux_ad%tsfc(ichan)    = scatt_aux_ad%tsfc(ichan)    + scatt_aux_ad%bsfc(ichan)
      scatt_aux_ad%tcosmic(ichan) = scatt_aux_ad%tcosmic(ichan) + scatt_aux_ad%btop(ichan)
      scatt_aux_ad%bsfc(ichan) = 0.0_JPRB   
      scatt_aux_ad%btop(ichan) = 0.0_JPRB   
      ...
      scatt_aux_ad%tbd(ichan,2:nlevels+1) = scatt_aux_ad%tbd(ichan,2:nlevels+1) + scatt_aux_ad%b0(ichan,:)
      scatt_aux_ad%tbd(ichan,1:nlevels  ) = scatt_aux_ad%tbd(ichan,1:nlevels  ) + scatt_aux_ad%bn(ichan,:)
      scatt_aux_ad%b0(ichan,:) = 0.0_JPRB   
      scatt_aux_ad%bn(ichan,:) = 0.0_JPRB   
