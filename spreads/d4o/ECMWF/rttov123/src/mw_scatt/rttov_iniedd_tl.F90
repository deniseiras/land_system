...
!* Local variables
  ...
  if (lradiance) then
    ...
    do ichan = 1, nchannels
      ...
      scatt_aux_tl%b0(ichan,:) = scatt_aux_tl%tbd(ichan,2:nlevels+1)
      scatt_aux   %bn(ichan,:) = scatt_aux   %tbd(ichan,1:nlevels  )
      scatt_aux_tl%bn(ichan,:) = scatt_aux_tl%tbd(ichan,1:nlevels  )
      ...
      scatt_aux_tl%bsfc(ichan) = scatt_aux_tl%tsfc(ichan)
      scatt_aux   %btop(ichan) = scatt_aux   %tcosmic(ichan)
      scatt_aux_tl%btop(ichan) = scatt_aux_tl%tcosmic(ichan)
