...
!* Coefficients
     aa_tl  =  scatt_aux_tl % b0  (ichan,ilayer) - 1.5_JPRB * angles( iprof) % coszen &
     ...
     aa    = scatt_aux % b0 (ichan,ilayer) - 1.5_JPRB * scatt_aux % asm (ichan,ilayer) * scatt_aux % ssa (ichan,ilayer) &
            & * angles (iprof) % coszen * scatt_aux % b1 (ichan,ilayer) / scatt_aux % h (ichan,ilayer) 
