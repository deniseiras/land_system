...
!             cont_ad(itype) = cont_ad(itype) + 0.0_JPRB
        ...
        if (opts_scatt%zero_hydro_tlad .or. scatt_aux    % rain (iprof,ilayer) > 0.0_JPRB) &
         & scatt_aux_ad % rain (iprofad,ilayer) = scatt_aux_ad % rain (iprofad,ilayer) + cont_ad(1)
        if (opts_scatt%zero_hydro_tlad .or. scatt_aux    % sp   (iprof,ilayer) > 0.0_JPRB) &
         & scatt_aux_ad % sp   (iprofad,ilayer) = scatt_aux_ad % sp   (iprofad,ilayer) + cont_ad(2)
        if (opts_scatt%zero_hydro_tlad .or. scatt_aux    % clw  (iprof,ilayer) > 0.0_JPRB) &
         & scatt_aux_ad % clw  (iprofad,ilayer) = scatt_aux_ad % clw  (iprofad,ilayer) + cont_ad(3)
        if (opts_scatt%zero_hydro_tlad .or. scatt_aux    % ciw  (iprof,ilayer) > 0.0_JPRB ) &
         & scatt_aux_ad % ciw  (iprofad,ilayer) = scatt_aux_ad % ciw  (iprofad,ilayer) + cont_ad(4)
        if (opts_scatt%zero_hydro_tlad .or. scatt_aux    % totalice  (iprof,ilayer) > 0.0_JPRB ) &
