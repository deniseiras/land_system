...
! Check we do not have more bands than channels
  ...
  IF (adjoint) THEN
    ...
    DO iband = 1, nbands
      ...
      CALL rttov_alloc_scatt_prof(errorstatus_ad_x, nprofiles12max, cld_profiles_ad_x(:,iband), nlevels, &
        cld_profiles(1)%use_totalice, 1_jpim, .TRUE._jplm, cld_profiles(1)%mmr_snowrain)
      IF (errorstatus_ad_x .NE. 0) GOTO 100
    ENDDO
  ENDIF
...
!  ENDDO
  ...
  IF (adjoint) THEN
    DO iband = 1, nbands
      CALL rttov_alloc_prof(errorstatus_ad_x, nprofiles12max, profiles_ad_x(:,iband), nlevels, opts, 0_jpim, coefs = coef_rttov)
      CALL rttov_alloc_scatt_prof(errorstatus_ad_x, nprofiles12max, cld_profiles_ad_x(:,iband), nlevels, &
