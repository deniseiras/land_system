...
!*         1.   Gas absorption
  ...
  opts%rt_all%switchrad          = .not. opts_scatt%lradiance ! RTTOV AD/K input perturbations in BT rather than radiance
  ...
  if (opts_scatt%lradiance) rad_allsky = radiance % total
...
!* ADJOINT PART
  ...
  if (opts_scatt%lradiance) then 
    ...
    ! Radiance-based radiative transfer 
    call rttov_calcbt_ad(opts, chanprof, coef_rttov%coef, lthermal, radiance, radiance_ad)
    ...
    do ichan = 1, nchannels
      ...
      if (scatt_aux % cfrac (iprof) > opts_scatt % cc_threshold .and. .not. lpolarimetric(ichan)) then
        ...
        scatt_aux_ad % cfrac (iprofad) = scatt_aux_ad % cfrac (iprofad) + &
         & (rad_cld (ichan) - radiance % bt_clear (ichan)) * radiance_ad % bt (ichan) 
  ...
  !*  4b. Restore trajectory allsky values - because rttov_ad/k has overwritten them with clear-sky
  if (opts_scatt%lradiance) radiance % total = rad_allsky 
