...
!!   if the conversion factor is set to a value less than or equal to zero then
!!   a value for the density (rho) of the aerosol material must be supplied in
!!   units of [g.cm^-3]. The conversion factor is then computed from the size
!!   distribution for RH=0% and this density value.
!!
...
!!   > --rtcoef_file rtcoef_rttov12/rttov7pred54L/rtcoef_msg_3_seviri.dat \
...
#include "rttov_hdf_save.interface"
  ...
  INTEGER(jpim), ALLOCATABLE :: solar_chanlist(:) ! Temp array for identifying solar channels
  ...
  ! Initialise max_nmom
  coefs%coef_scatt_ir%fmv_aer_maxnmom = 0
  ...
  IF (coefs%coef_scatt_ir%fmv_aer_pha_chn > 0) THEN
    ...
    ALLOCATE(coefs%coef_scatt_ir%aer_phangle(nphangle_hires))
    coefs%coef_scatt_ir%aer_phangle(:) = phangle_hires(:)
    coefs%coef_scatt_ir%aer_nphangle = nphangle_hires
  ENDIF
  ...
  READ(fid_config, *, iostat=err) coefs%coef_scatt_ir%fmv_aer_comp
  THROWM(err.NE.0,'Error reading config file, number of aerosol types')

  ntypes = coefs%coef_scatt_ir%fmv_aer_comp

  ! Allocate arrays
  ALLOCATE(coefs%coef_scatt_ir%fmv_aer_comp_name(ntypes))
  ALLOCATE(coefs%coef_scatt_ir%aer_mmr2nd(ntypes))
  ALLOCATE(coefs%coef_scatt_ir%fmv_aer_rh(ntypes))

  ALLOCATE(coefs%optp%optpaer(ntypes))
  DO iaer = 1, ntypes
    CALL rttov_nullify_coef_scatt_ir(coefs%optp%optpaer(iaer))
  ENDDO
  ...
  DO iaer = 1, ntypes
    ...
    CALL rttov_skipcommentline(fid_config, err)
    THROWM(err.NE.0,'Error reading config file')
    READ(fid_config, *, iostat=err) coefs%coef_scatt_ir%fmv_aer_comp_name(iaer)
    THROWM(err.NE.0,'Error reading config file, aerosol name')

    PRINT *,'Aerosol name ', coefs%coef_scatt_ir%fmv_aer_comp_name(iaer)
    ...
    ENDIF
    coefs%coef_scatt_ir%fmv_aer_rh(iaer) = nrh
    ALLOCATE(coefs%optp%optpaer(iaer)%fmv_aer_rh_val(nrh))

    ! RH values
    CALL rttov_skipcommentline(fid_config, err)
    THROWM(err.NE.0,'Error reading config file')
    READ(fid_config, *, iostat=err) coefs%optp%optpaer(iaer)%fmv_aer_rh_val
    THROWM(err.NE.0,'Error reading config file, RH values')
    IF (ANY(coefs%optp%optpaer(iaer)%fmv_aer_rh_val < 0._jprb) .OR. &
        ANY(coefs%optp%optpaer(iaer)%fmv_aer_rh_val > 99._jprb)) THEN
      err = errorstatus_fatal
    ...
    CALL rttov_skipcommentline(fid_config, err)
    THROWM(err.NE.0,'Error reading config file')
    READ(fid_config, *, iostat=err) coefs%coef_scatt_ir%aer_mmr2nd(iaer)
    ...
    ALLOCATE(coefs%optp%optpaer(iaer)%abs(coefs%coef%fmv_chn,nrh))
    ALLOCATE(coefs%optp%optpaer(iaer)%sca(coefs%coef%fmv_chn,nrh))
    ALLOCATE(coefs%optp%optpaer(iaer)%bpr(coefs%coef%fmv_chn,nrh))
    coefs%optp%optpaer(iaer)%abs(:,:) = 0._jprb
    coefs%optp%optpaer(iaer)%sca(:,:) = 0._jprb
    coefs%optp%optpaer(iaer)%bpr(:,:) = 0._jprb
    ALLOCATE(coefs%optp%optpaer(iaer)%nmom(coefs%coef%fmv_chn,nrh))
    IF (chou_only) THEN
      ALLOCATE(coefs%optp%optpaer(iaer)%legcoef(1,coefs%coef%fmv_chn,nrh))
      coefs%optp%optpaer(iaer)%nmom = 0
      coefs%optp%optpaer(iaer)%legcoef = 0
    ELSE
      ALLOCATE(coefs%optp%optpaer(iaer)%legcoef(1:max_nmom+1,coefs%coef%fmv_chn,nrh))
      coefs%optp%optpaer(iaer)%nmom(:,:) = 0
      coefs%optp%optpaer(iaer)%legcoef(:,:,:) = 0._jprb
      IF (coefs%coef_scatt_ir%fmv_aer_pha_chn > 0) THEN
        ALLOCATE(coefs%optp%optpaer(iaer)%pha(nphangle_hires,coefs%coef_scatt_ir%fmv_aer_pha_chn,nrh))
        coefs%optp%optpaer(iaer)%pha(:,:,:) = 0._jprb
      ENDIF
    ENDIF

    ! Allocate temporary storage for phase functions for all channels
    ALLOCATE(coef_phase(nphangle_hires,coefs%coef%fmv_chn,nrh))
    coef_phase(:,:,:) = 0._jprb

    DO irh = 1, nrh

      WRITE(rhumstr, '(i2.2)') INT(coefs%optp%optpaer(iaer)%fmv_aer_rh_val(irh))
      PRINT *,'RH = '//rhumstr
      ...
      IF (irh == 1) THEN
        IF (coefs%coef_scatt_ir%aer_mmr2nd(iaer) <= 0._jprb) THEN
          IF (rho <= 0._jprb) THEN
            err = errorstatus_fatal
          ...
          coefs%coef_scatt_ir%aer_mmr2nd(iaer) = 1.E-06_jprb * mass
        ENDIF
      ENDIF
      ...
      DO iwav = 1, coefs%coef%fmv_chn
        ...
        CALL calc_mie_ext_sca(nrarr, rarr, n, wvl_chan(iwav), refind_chan(iwav), ecoef, scoef)

        coefs%optp%optpaer(iaer)%abs(iwav,irh) = (ecoef - scoef) * 0.001_jprb
        coefs%optp%optpaer(iaer)%sca(iwav,irh) = scoef * 0.001_jprb
        ...
        CALL calc_mie_phasefn(thisnphangle, thisphangle(1:thisnphangle), nrarr, rarr, &
                              ...
                              coef_phase(1:thisnphangle,iwav,irh), nthreads)
        ...
        IF (.NOT. chou_only) THEN
          ...
          CALL spline_interp(thisnphangle, COS(thisphangle(thisnphangle:1:-1) * deg2rad), &
                             coef_phase(thisnphangle:1:-1,iwav,irh), ngauss, q, phase_interp)
          CALL normalise(ngauss, w, phase_interp)
          CALL calc_legendre_coef_gauss(q, w, phase_interp, min_nmom, max_nmom, &
              coefs%optp%optpaer(iaer)%nmom(iwav,irh), coefs%optp%optpaer(iaer)%legcoef(:,iwav,irh))
        ENDIF
        ...
        IF (coefs%coef%ss_val_chn(iwav) < 2) THEN
          ...
          CALL rttov_bpr_calc(err, coef_phase(1:thisnphangle,iwav,irh), &
                              thisphangle, ecoef, nthreads=nthreads)
          coefs%optp%optpaer(iaer)%bpr(iwav,irh) = ecoef
        ENDIF
      ...
      IF (coefs%coef_scatt_ir%fmv_aer_pha_chn > 0) THEN
        DO i = 1, coefs%coef_scatt_ir%fmv_aer_pha_chn
          coefs%optp%optpaer(iaer)%pha(:,i,irh) = &
              coef_phase(:,coefs%coef_scatt_ir%aer_pha_chanlist(i),irh)
        ENDDO
      ENDIF
  ...
  IF (.NOT. chou_only) coefs%coef_scatt_ir%fmv_aer_maxnmom = max_nmom
  ...
  ELSE IF (TRIM(file_format) == 'hdf5') THEN
    ...
    CALL rttov_hdf_save(err, f1, "/SCAER", create=.TRUE._jplm, &
                        ...
                        optp=coefs%optp, compress=.TRUE._jplm)
