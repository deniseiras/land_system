...
!INTF_END
  ...
  WRITE(iu,'(2x,a,l1)')     "so2_data               ", opts%rt_ir%so2_data
  ...
  IF (opts%rt_ir%addclouds) THEN
    ...
    IF (opts%rt_ir%cldstr_simple) THEN
      WRITE(tmp_text,'(f10.4)') opts%rt_ir%cldstr_low_cloud_top
      WRITE(iu,'(2x,a,a)')      "cldstr_low_cloud_top   ", TRIM(ADJUSTL(tmp_text))
    ENDIF
  ENDIF
  ...
  IF (opts%rt_mw%clw_data) THEN
    ...
    WRITE(iu,'(2x,a,l1)')     "clw_calc_on_coef_lev   ", opts%rt_mw%clw_calc_on_coef_lev
  ...
  WRITE(iu,'(2x,a,l1)')     "apply_band_correction  ", opts%rt_mw%apply_band_correction
  WRITE(iu,'(a)',advance='yes')
