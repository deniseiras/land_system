...
!INTF_END
  ...
  ELSE
    IF (BTEST(quality, qflag_reg_limits)) THEN
      WRITE(iu,'(a)') 'Regression limits exceeded for some variable'
    ENDIF
    ...
    ENDIF
    IF (BTEST(quality, qflag_mfasis_hiscaangle)) THEN
      WRITE(iu,'(a)') 'MFASIS warning: scattering angle greater than max valid value'
    ENDIF
    IF (BTEST(quality, qflag_mfasis_loscaangle)) THEN
      WRITE(iu,'(a)') 'MFASIS warning: scattering angle less than min valid value'
    ENDIF
