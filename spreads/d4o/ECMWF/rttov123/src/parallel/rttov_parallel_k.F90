...
!
  ...
  IF (opts%htfrtc_opts%htfrtc) THEN
    IF (do_htfrtc_radrec) THEN
      nchannels = coefs%coef_htfrtc%n_ch * nprofiles
    ELSE
      nchannels = nprofiles
    ENDIF
  ELSE
...
! Check we do not have more bands than channels
  ...
  IF (strategy1 == 1) THEN
    IF (do_pc) THEN
      IF (opts%rt_ir%pc%addpc) THEN
        IF (opts%rt_ir%pc%npcscores > 0) THEN
          npcscores = opts%rt_ir%pc%npcscores
        ELSE
          npcscores = SIZE(pccomp%pcscores) / SIZE(profiles)
        ENDIF
      ELSE
        npcscores = coefs%coef_htfrtc%n_pc
        IF (opts%htfrtc_opts%n_pc_in > 0 .AND. opts%htfrtc_opts%n_pc_in <= coefs%coef_htfrtc%n_pc) THEN
          npcscores = opts%htfrtc_opts%n_pc_in
        ENDIF
      ENDIF
      IF (do_pcrttov_radrec) THEN
        nchan_rec = SIZE(channels_rec)
      ELSEIF (do_htfrtc_radrec) THEN
    ...
    DO iband = 1, nbands
      ...
      IF (do_pc) THEN
        ...
        IF (do_radrec) THEN
          ...
          ! For HTFRTC rec rads, overwrite the channel bounds
          IF (do_htfrtc_radrec) THEN
            ichannel1(iband) = ichannelrec1(iband)
            ichannel2(iband) = ichannelrec2(iband)
          ENDIF
        ENDIF
      ENDIF
    ENDDO
  ...
  ELSE
    ...
    IF (mstat .NE. 0) GOTO 100
    emissivity_k_x(:)%emis_in  = 0.
    emissivity_k_x(:)%emis_out = 0.
  ENDIF
  ...
  ELSE
    ...
    IF (mstat .NE. 0) GOTO 100
    reflectance_k_x(:)%refl_in  = 0.
    reflectance_k_x(:)%refl_out = 0.
  ENDIF
...
!    " ichannel1 = ", ichannel1(iband), " ichannel2 = ", ichannel2(iband)
  ...
  IF (do_htfrtc_radrec) THEN
    CALL &
        ...
        , radiance_x(iband)                                                        &
        , radiance_k_x(iband)                                                      &
        , calcemis_x(ichannel1(iband):ichannel2(iband))                            &
        , emissivity_x(ichannel1(iband):ichannel2(iband))                          &
        , emissivity_k_x(ichannel1(iband):ichannel2(iband))                        &
        , calcrefl_x(ichannel1(iband):ichannel2(iband))                            &
        , reflectance_x(ichannel1(iband):ichannel2(iband))                         &
        , reflectance_k_x(ichannel1(iband):ichannel2(iband))                       &
        , aer_opt_param_x(iband)                                                   &
        , aer_opt_param_k_x(iband)                                                 &
        , cld_opt_param_x(iband)                                                   &
        , cld_opt_param_k_x(iband)                                                 &
...
CONTAINS
  ...
  SUBROUTINE AssociatePCcomp(ipc1, ipc2, pccomp_x, pccomp, ichannelrec1, ichannelrec2)
    ...
    IF (ASSOCIATED(pccomp%total_pcscores)) &
