...
#include "rttov_deallocate_emis_atlas.interface"
  ...
  ! calculates sea surface emissivities using the PC-RTTOV emissivity model
  ! and uses 0.98 and 0.99 for land and sea-ice respectively. Alternatively
  ! it is possible to pass emissivities in as for RTTOV. The calcemis and
  ! emissivity arguments must be of size nchanprof = nchannels_rec * nprofiles
  ! (note this is different to PC-RTTOV). This represents all reconstructed
  ! channels for each profile in turn (analogous to how the arguments work for
  ! RTTOV). To pass emissivities in for a given profile you must set the
  ! elements of calcemis corresponding to that profile to false for all 
  ! channels. Note that passing the emissivity array provides access to the
  ! emissivities used/computed by HTFRTC for the reconstructed radiance
  ! channels in the emis_out member, similar to RTTOV.
  ...
  opts % rt_ir % ozone_data         = .TRUE.  ! Set the relevant flags to .TRUE.
  opts % rt_ir % co2_data           = .FALSE. !   when supplying profiles of the
  opts % rt_ir % n2o_data           = .FALSE. !   given trace gas
  ...
  !    filled with any values). For HTFRTC these arrays are 
  !    nchannels_rec * nprofiles in size. If you wish to allow HTFRTC to
  !    provide all emissivities then you can omit the calcemis and emissivity
  !    arguments (and there is no need to allocate them).
  nchanprof = nchannels_rec * nprof
  ...
  CALL rttov_alloc_prof(errorstatus, nprof, profiles, nlevels, opts, asw=1_jpim, init=.TRUE._jplm)
  IF (errorstatus /= errorstatus_success) THEN
    WRITE(*,*) 'allocation error for profiles structure'
    CALL rttov_exit(errorstatus)
  ENDIF

  CALL rttov_alloc_pccomp(errorstatus, pccomp=pccomp, npcscores=npcscores * nprof, asw=1_jpim, &
                          init=.TRUE._jplm, nchannels_rec = nchannels_rec * nprof, &
  ...
  IF (imonth >= 1 .AND. imonth <= 12) THEN
    CALL rttov_setup_emis_atlas(          &
                ...
                coefs = coefs) ! If supplied for IR atlases they are initialised for this sensor
  ...
  DO iprof = 1, nprof
    ...
    WRITE(ioout,'(10(G15.6E3))') emissivity(lo:hi) % emis_out
  ...
  CALL rttov_alloc_pccomp(errorstatus, pccomp=pccomp, npcscores=npcscores * nprof, &
                          asw=0_jpim, nchannels_rec=nchannels_rec * nprof)
  IF (errorstatus /= errorstatus_success) THEN
    WRITE(*,*) 'deallocation error for pccomp structure'
    CALL rttov_exit(errorstatus)
  ENDIF

  CALL rttov_alloc_prof(errorstatus, nprof, profiles, nlevels, opts, asw=0_jpim)
  IF (errorstatus /= errorstatus_success) THEN
    WRITE(*,*) 'deallocation error for profiles structure'
    CALL rttov_exit(errorstatus)
  ENDIF

  DEALLOCATE(profiles, chanprof, calcemis, emissivity, channels_rec, stat=errorstatus)
  IF (errorstatus /= errorstatus_success) THEN
