          ...
          DO i5 = ubounda(5), lbounda(5), -1
            ...
            DO i6 = ubounda(6), lbounda(6), -1
              ...
              DO i7 = ubounda(7), lbounda(7), -1
                ...
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%CC", p1 = p%cc )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%CLW", p1 = p%clw )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%CIW", p1 = p%ciw )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%RAIN", p1 = p%rain )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%SP", p1 = p%sp )
                IF( err .NE. 0 ) RETURN
                CALL chain_real_JPRB( err, chn, trim( str ) // "%TOTALICE", p1 = p%totalice )
                IF( err .NE. 0 ) RETURN
