...
#include "rttov_errorreport.interface"
  ...
  LOGICAL(KIND=jplm) :: dopc, doradrec, do_rttovscatt
  ...
  dopc = opts%rt_ir%pc%addpc .OR. opts%htfrtc_opts%htfrtc
  doradrec = (opts%rt_ir%pc%addpc .AND. opts%rt_ir%pc%addradrec) .OR. &
  ...
  IF (do_rttovscatt) THEN
    CALL rttov_alloc_scatt_prof( &
          ...
          cld_profiles_bf,              &
          nlevels,                      &
          cld_profiles(1)%use_totalice, &
          1_jpim,                       &
          init=.TRUE._jplm,             &
          mmr_snowrain=cld_profiles(1)%mmr_snowrain)
    THROW(err.NE.0)
  ...
  DO i = 1, nsize
    ...
    IF (do_rttovscatt .OR. opts%rt_all%switchrad) THEN
      IF (dopc) THEN
        CALL assign_chain_profile (c_k, (radiance_bf%total-radiance%total)*w)
        IF (doradrec) THEN
  ...
  DO ichan = 1, nchannels
    ...
    emissivity_bf(ichan)%emis_in  = emissivity_bf(ichan)%emis_in + demis
    ...
    IF (do_rttovscatt .OR. &
        (opts%rt_all%switchrad .AND. .NOT. dopc .AND. &
  ...
  IF (opts%rt_ir%addsolar) THEN
    ...
    DO ichan = 1, nchannels
      ...
      reflectance_bf(ichan)%refl_in = reflectance_bf(ichan)%refl_in + drefl
