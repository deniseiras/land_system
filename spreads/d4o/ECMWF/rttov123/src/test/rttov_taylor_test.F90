...
!!
!!   For RTTOV-SCATT the test is always calculated on BTs.
!!
...
#include "rttov_scatt_tl.interface"
  ...
  REAL(jprb)                         :: ratio
  INTEGER(jpim)                      :: nchanprof, nprofiles, nlevels, nchannels_rec, nphangle
  INTEGER(jpim)                      :: i, j, chan, prof, iter, file_id
  ...
  IF (do_rttovscatt) THEN
    ...
    CALL rttov_alloc_scatt_prof(err, nprofiles, cld_profiles_pert, nlevels, &
        cld_profiles(1)%use_totalice, 1_jpim, init=.TRUE._jplm, mmr_snowrain=cld_profiles(1)%mmr_snowrain)
    THROWM(err.NE.0, "Allocation for Taylor test")
  ...
  IF (taylor_by_chan .AND. .NOT. opts%rt_ir%pc%addpc) THEN
    ALLOCATE(taylor_direct(niter,nchanprof), taylor_tl(niter,nchanprof))
  ELSE
    ALLOCATE(taylor_direct(niter,nprofiles), taylor_tl(niter,nprofiles))
  ENDIF
  ...
  DO iter = 1, niter
    ...
    IF (opts%rt_ir%pc%addpc) THEN
      DO i = 1, nprofiles
        ...
        ELSE
          j = (i - 1) * npcscores + 1
          taylor_direct(iter,i) = SUM(pccomp%pcscores(j:j+npcscores-1) - &
                                      pccomp_saved%pcscores(j:j+npcscores-1))
          taylor_tl(iter,i) = SUM(pccomp_tl%pcscores(j:j+npcscores-1))
        ENDIF
      ENDDO
    ELSE
      IF (taylor_by_chan) THEN
        DO i = 1, nchanprof
          IF (taylor_on_btrefl .OR. do_rttovscatt) THEN
    ...
    DO iter = 1, niter
      ...
      IF (taylor_by_chan .AND. .NOT. opts%rt_ir%pc%addpc) THEN
        WRITE(file_id, '(14x,i5,4x,i5,4x,i6,5x,7f20.16)') &
            chanprof(i)%prof, chanprof(i)%chan, iter, ratio
      ELSE
  ...
  IF (ASSOCIATED(cld_profiles)) THEN
    CALL rttov_alloc_scatt_prof(err, nprofiles, cld_profiles_pert, nlevels, &
        cld_profiles(1)%use_totalice, 0_jpim)
