...
! dimensions
    INTEGER(jpim) :: nlevels = -1_jpim, nchannels = -1_jpim, nprofiles = -1_jpim, mult = 1_jpim
...
! what we will do
   ...
   LOGICAL(jplm) :: do_rttovscatt = .FALSE., calc_emis_terms = .FALSE.
   ...
   INTEGER(jpim) :: ntimes = 1, nthreads = 0
   ...
   LOGICAL(jplm) :: use_totalice = .FALSE.
...
!
  ...
  DO itime = 1, ts%defn%ntimes
    ...
    IF (ts%defn%do_direct) THEN
      ...
      IF (.NOT. ts%defn%do_rttovscatt .AND. ts%defn%calc_rad2) THEN

        IF ((ts%defn%nthreads <= 0) .AND. ts%defn%prof_by_prof) THEN
        ...
        ELSEIF ((ts%defn%nthreads <= 0) .AND. ts%defn%chan_by_chan) THEN
        ...
        ELSEIF (ts%defn%nthreads <= 0) THEN
      ...
      ELSEIF (ts%defn%do_rttovscatt .AND. ts%defn%calc_emis_terms) THEN
      ...
      IF (ts%defn%do_print) THEN
        CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/direct", "", .FALSE._jplm, &
                          ts%defn%calc_rad2, ts%defn%do_rttovscatt, ts%defn%calc_emis_terms, ts%data%direct)
        THROW(err.NE.0)
      ENDIF
    ...
    IF (ts%defn%do_tl) THEN
      ...
      IF ((ts%defn%nthreads <= 0) .AND. ts%defn%prof_by_prof) THEN
        ...
        ELSE
          CALL rttov_parallel_tl(                                             &
              ...
              ts%data%direct%aer_opt_param,   ts%data%tl%aer_opt_param,       &
              ts%data%direct%cld_opt_param,   ts%data%tl%cld_opt_param,       &
              nthreads = 1_jpim,              strategy = 1_jpim)
        ENDIF
      ...
      ELSEIF (ts%defn%nthreads <= 0) THEN
        ...
        ELSE
          CALL rttov_tl(                             &
              ...
              ts%data%direct%aer_opt_param,          &
              ts%data%tl%aer_opt_param,              &
              ts%data%direct%cld_opt_param,          &
              ts%data%tl%cld_opt_param,              &
              channels_rec = ts%data%channels_rec,   &
              pccomp       = ts%data%direct%pccomp,  &
      ...
      IF (ts%defn%do_print) THEN
        ...
        CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/tl", "", .FALSE._jplm, &
                          .FALSE._jplm, ts%defn%do_rttovscatt, .FALSE._jplm, ts%data%direct)
        THROW(err.NE.0)
        CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/tl", "_tl", .FALSE._jplm, &
                          .FALSE._jplm, ts%defn%do_rttovscatt, .FALSE._jplm, ts%data%tl)
        THROW(err.NE.0)
      ENDIF
    ...
    IF (ts%defn%do_ad) THEN
      ...
      IF ((ts%defn%nthreads <= 0) .AND. ts%defn%prof_by_prof) THEN
        ...
        ELSE
          CALL rttov_parallel_ad(                                               &
              ...
              ts%data%direct%aer_opt_param,    ts%data%ad%aer_opt_param,        &
              ts%data%direct%cld_opt_param,    ts%data%ad%cld_opt_param,        &
              nthreads = 1_jpim,               strategy = 1_jpim)
        ENDIF
      ...
      ELSEIF (ts%defn%nthreads <= 0) THEN
        ...
        ELSE
          CALL rttov_ad(                             &
              ...
              ts%data%direct%aer_opt_param,          &
              ts%data%ad%aer_opt_param,              &
              ts%data%direct%cld_opt_param,          &
              ts%data%ad%cld_opt_param,              &
              channels_rec = ts%data%channels_rec,   &
              pccomp       = ts%data%direct%pccomp,  &
      ...
      IF (ts%defn%do_print) THEN
        ...
        CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/ad", "_ad", .TRUE._jplm, &
                          .FALSE._jplm, ts%defn%do_rttovscatt, .FALSE._jplm, ts%data%ad)
        THROW(err.NE.0)
      ENDIF
    ...
    IF (ts%defn%do_k) THEN
      ...
      IF ((ts%defn%nthreads <= 0) .AND. ts%defn%prof_by_prof) THEN
        ...
        ELSE
          CALL rttov_parallel_k(                                           &
              ...
              ts%data%direct%aer_opt_param,  ts%data%k%aer_opt_param,      &
              ts%data%direct%cld_opt_param,  ts%data%k%cld_opt_param,      &
              nthreads = 1_jpim,             strategy = 1_jpim)
        ENDIF
      ...
      ELSEIF (ts%defn%nthreads <= 0) THEN
        ...
        ELSE
          CALL rttov_k(                                       &
              ...
              ts%data%direct%aer_opt_param,                   &
              ts%data%k%aer_opt_param,                        &
              ts%data%direct%cld_opt_param,                   &
              ts%data%k%cld_opt_param,                        &
              traj           = traj,                          &
              traj_k         = traj_k,                        &
              pccomp         = ts%data%direct%pccomp,         &
      ...
      IF (ts%defn%do_print) THEN
        ...
        CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/k", "_k", .TRUE._jplm, &
                          .FALSE._jplm, ts%defn%do_rttovscatt, .FALSE._jplm, ts%data%k, ts%data%data_k)
        THROW(err.NE.0)
      ENDIF
  ...
  IF (ts%defn%do_k_bf) THEN
    ...
    IF (ts%defn%do_print) THEN
      ...
      CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/k_bf", "_k", .TRUE._jplm, &
                        .FALSE._jplm, ts%defn%do_rttovscatt, .FALSE._jplm, ts%data%k_bf, ts%data%data_k_bf)
      THROW(err.NE.0)
    ENDIF
  ...
  IF (ts%defn%do_k_tl) THEN
    ...
    IF (ts%defn%do_print) THEN
      ...
      CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/k_tl", "_k", .TRUE._jplm, &
                        .FALSE._jplm, ts%defn%do_rttovscatt, .FALSE._jplm, ts%data%k_tl, ts%data%data_k_tl)
      THROW(err.NE.0)
    ENDIF
  ...
  IF (ts%defn%do_k_ad) THEN
    ...
    IF (ts%defn%do_print) THEN
      ...
      CALL printoutput(err, ts%defn%opts, TRIM(ts%path)//"/k_ad", "_k", .TRUE._jplm, &
                        .FALSE._jplm, ts%defn%do_rttovscatt, .FALSE._jplm, ts%data%k_ad, ts%data%data_k_ad)
      THROW(err.NE.0)
    ENDIF
...
!     ENDIF
 ...
 !
    ...
    IF (.NOT. defn%do_rttovscatt) THEN
      defn%opts%rt_ir%ozone_data = defn%f_o3     .NE. "" .OR. defn%o3_col_int  > 0._jprb .OR. defn%o3_max_ppmv > 0._jprb .OR. &
                                                              defn%o3_col_int_du > 0._jprb
      defn%opts%rt_ir%co2_data   = defn%f_co2    .NE. "" .OR. defn%co2_col_int > 0._jprb .OR. defn%co2_max_ppmv > 0._jprb
      defn%opts%rt_ir%n2o_data   = defn%f_n2o    .NE. "" .OR. defn%n2o_col_int > 0._jprb .OR. defn%n2o_max_ppmv > 0._jprb
      defn%opts%rt_ir%co_data    = defn%f_co     .NE. "" .OR. defn%co_col_int  > 0._jprb .OR. defn%co_max_ppmv  > 0._jprb
      defn%opts%rt_ir%ch4_data   = defn%f_ch4    .NE. "" .OR. defn%ch4_col_int > 0._jprb .OR. defn%ch4_max_ppmv > 0._jprb
      defn%opts%rt_ir%so2_data   = defn%f_so2    .NE. "" .OR. defn%so2_col_int > 0._jprb .OR. defn%so2_max_ppmv > 0._jprb
    ...
    IF (ts%defn%opts%htfrtc_opts%htfrtc) THEN
      ...
      ts%defn%opts%htfrtc_opts%reconstruct = ts%defn%opts%rt_ir%pc%addradrec
      ts%defn%user_check_opts = .FALSE.

      IF (ts%defn%opts%htfrtc_opts%reconstruct) THEN
        ! For reconstructed radiances, if we want to supply calcemis/emissivity they must be allocated for the number
        ! of reconstructed radiances so we overwrite nchannels
        nchannels1 = ts%defn%nchannels_rec * ts%defn%nprofiles
...
!             file_mfasis_aer = TRIM(ts%defn%coef_prefix)//'/'//TRIM(ts%defn%f_mfasis_aer), &
      ...
      IF (ts%defn%do_rttovscatt) THEN
        ...
        CALL rttov_scatt_setupindex(nprofiles1, ts%data%coefs%coef%fmv_chn, ts%data%coefs, &
              nchannels1, ts%data%chanprof, ts%data%frequencies, use_chan)
...
!
    ...
    IF (ts%defn%fixed2m) THEN
      ...
      DO i = 1, defn%nprofiles
        ...
        IF (defn%opts%rt_ir%ozone_data) ts%data%direct%profiles(i)%s2m%o = ts%data%direct%profiles(i)%o3(nlevels)
      ENDDO
    ENDIF
    ...
    IF (ts%defn%fixedspecularity >= 0)      ts%data%direct%profiles(:)%skin%specularity = ts%defn%fixedspecularity
...
#include "rttov_hdf_save.interface"
      ...
      IF (defn%opts%rt_ir%addclouds) THEN
        ...
        IF (.NOT. defn%opts%rt_ir%user_cld_opt_param) THEN
          ...
          CALL reada(err, TRIM(in_dir)//'/atm/clw_scheme.txt', &
                     clw_scheme = direct%profiles(iprof)%clw_scheme, &
                     clw_scheme_a = direct%profiles(1)%clw_scheme, &
                     foe = foe)
          THROW(err.NE.0)
          ...
          CALL reada(err, TRIM(in_dir)//'/atm/ice_scheme.txt', &
                     idg = direct%profiles(iprof)%idg, ice_scheme = direct%profiles(iprof)%ice_scheme, &
                     idg_a = direct%profiles(1)%idg, ice_scheme_a = direct%profiles(1)%ice_scheme, &
                     foe = foe)
          THROW(err.NE.0)
        ENDIF
      ENDIF
    ...
    IF (ts%defn%savehdf5) THEN
      ...
      opts%rt_ir%so2_data   = defn%opts%rt_ir%so2_data
  ...
  SUBROUTINE read_cld_profile_data(ts, err)
    ...
    TYPE(rttov_input_data), POINTER :: direct
    INTEGER(jpim) :: iprof, kprof, imult, i
    CHARACTER(LEN=6)   :: sproffmt
    CHARACTER(LEN=8)   :: sprof
    ...
    DO iprof = 1, nprofiles1
      ...
      IF (file_exists) THEN
        ...
        READ(file_id, *) ts%data%direct%cld_profiles(iprof)%mmr_snowrain
        IF (ts%data%direct%cld_profiles(iprof)%mmr_snowrain .NEQV. &
            ts%data%direct%cld_profiles(1)%mmr_snowrain) THEN
          err = errorstatus_fatal
          THROWM(err.NE.0,"mmr_snowrain must be the same for all profiles")
        ENDIF
        ...
        DO i = 1, ts%defn%nlevels
          READ(file_id, *) ts%data%direct%cld_profiles(iprof)%ph(i), &
                           ts%data%direct%cld_profiles(iprof)%cc(i), &
                           ts%data%direct%cld_profiles(iprof)%clw(i), &
                           ts%data%direct%cld_profiles(iprof)%rain(i), &
                           ciw, sp
          IF (ts%data%direct%cld_profiles(iprof)%use_totalice) THEN
            ts%data%direct%cld_profiles(iprof)%totalice(i) = ciw + sp
          ELSE
            ts%data%direct%cld_profiles(iprof)%ciw(i) = ciw
            ts%data%direct%cld_profiles(iprof)%sp(i)  = sp
          ENDIF
        ENDDO
  ...
  SUBROUTINE reada(err, f,                                      &
    ...
    INTEGER(jpim),      INTENT(INOUT), OPTIONAL :: clw_scheme
    INTEGER(jpim),      INTENT(IN),    OPTIONAL :: clw_scheme_a
    INTEGER(jpim),      INTENT(INOUT), OPTIONAL :: idg,   ice_scheme
    INTEGER(jpim),      INTENT(IN),    OPTIONAL :: idg_a, ice_scheme_a
  ...
  SUBROUTINE alloc_data(err, ts, nprofiles, dat, dat_k, do_ad)
    ...
    IF (ts%defn%do_rttovscatt) THEN
      CALL rttov_alloc_scatt_prof(err, nprofiles, dat%cld_profiles, ts%defn%nlevels, ts%defn%use_totalice, &
            1_jpim, init = .TRUE._jplm)
    ENDIF
  ...
  !! @param[in]     calcrad2        flag to indicate radiance2 should be output (direct model only)
  ...
  SUBROUTINE printoutput(err, opts, path, suffix, adk, calcrad2, do_rttovscatt, calcemisterms, data, data_k)
    ...
    IF (calcrad2) THEN
      ! Direct only - print radiance and radiance2
    ...
    IF (adk) THEN
      ...
      CALL printemissivity(err, data%emissivity%emis_in, "EMISSIVITY"//TRIM(suffixuc), &
              TRIM(path)//"/emissivity"//TRIM(suffixlc)//".txt")
      THROW(err.NE.0)

      IF (opts%rt_ir%addsolar) THEN
        CALL printreflectance(err, data%reflectance%refl_in, "REFLECTANCE"//TRIM(suffixuc), &
    ...
    ELSE
      ...
      CALL printemissivity(err, data%emissivity%emis_out, "EMISSIVITY_OUT"//TRIM(suffixuc), &
              TRIM(path)//"/emissivity_out"//TRIM(suffixlc)//".txt")
      THROW(err.NE.0)

      IF (opts%rt_ir%addsolar) THEN
        CALL printreflectance(err, data%reflectance%refl_out, "REFLECTANCE_OUT"//TRIM(suffixuc), &
