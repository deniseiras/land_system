...
#include "throw.h"
  ...
  PRIVATE
  PUBLIC :: printprofiles, printcldprofiles, printoptparam, &
            printradiance, printtransmission, printemisterms, &
            printemissivity, printreflectance, printpcscores, &
...
CONTAINS
  ...
  SUBROUTINE printemissivity(err, emissivity, name, f)
    ...
    INTEGER(jpim), INTENT(OUT) :: err
    REAL(jprb),    INTENT(IN)  :: emissivity(:)
    CHARACTER(LEN=*),   INTENT(IN)  :: name, f
