...
!------------------------------------------------------------------------------
  ...
  IF (rth%verbose) THEN
    ...
    IF (TRIM(rth%file_mietable) .NE. '') &
      INFO(rth%file_mietable)
  ENDIF
...
!! @param[in]     clwscheme         profile clw_scheme (nprofiles)
!! @param[in]     icecloud          profile ice scheme: ice_scheme, idg (2,nprofiles)
...
!! @param[in]     surfemisrefl      input/output surface emissivities and BRDFs (nchannels,nprofiles,2)
...
SUBROUTINE rttov_call_direct( &
    ...
    skin,              &    ! skin T, salinity, snow_frac, foam_frac, fastem_coefsx5, specularity (10,nprofiles)
    s2m,               &    ! 2m p, 2m t, 2m q, 10m wind u, v, wind-fetch             (6,nprofiles)
    simplecloud,       &    ! ctp, cfraction                                          (2,nprofiles)
    clwscheme,         &    ! clw_scheme                                              (nprofiles)
    icecloud,          &    ! ice_scheme, idg                                         (2,nprofiles)
...
#include "throw.h"
  ...
  REAL(jprb),       INTENT(IN)    :: skin(10,nprofiles)
  REAL(jprb),       INTENT(IN)    :: s2m(6,nprofiles)
  REAL(jprb),       INTENT(IN)    :: simplecloud(2,nprofiles)
  INTEGER(jpim),    INTENT(IN)    :: clwscheme(nprofiles)
...
!------------------------------------------------------------------------------
  ...
  CALL rttov_wrapper_handle_alloc(err, rth, nprofiles, nchannels, nlevels, 0_jpim)
  THROW(err.NE.0)
  CALL rttov_wrapper_handle_alloc(err, rth, nprofiles, nchannels, nlevels, 1_jpim)
  THROW(err.NE.0)
...
!! @param[in]     use_totalice      if non-zero sets use_totalice option to true
!! @param[in]     mmr_snowrain      if non-zero sets mmr_snowrain option to true
...
SUBROUTINE rttov_scatt_call_direct( &
    ...
    use_totalice,      &    ! set use_totalice flag
    mmr_snowrain,      &    ! set mmr_snowrain flag
...
#include "throw.h"
  ...
  INTEGER(jpim),    INTENT(IN)    :: use_totalice
  INTEGER(jpim),    INTENT(IN)    :: mmr_snowrain
  ...
  INTEGER(jpim)      :: i, k, iprof, iprof1, lochan, hichan
  INTEGER(jpim)      :: nchanprof, thisnchanprof, nprof, thisnprof, ncalls
  LOGICAL(jplm)      :: luse_totalice, lmmr_snowrain
  CHARACTER(LEN=256) :: msg

  TYPE(rttov_chanprof),         POINTER :: chanprof(:)     => NULL()
  INTEGER(KIND=jpim),           POINTER :: frequencies(:)  => NULL()
  LOGICAL(KIND=jplm),           POINTER :: use_chan(:,:)   => NULL()
  LOGICAL(jplm),                POINTER :: calcemis(:)     => NULL()
  TYPE(rttov_emissivity),       POINTER :: emissivity(:)   => NULL()
  TYPE(rttov_profile),          POINTER :: profiles(:)     => NULL()
...
!------------------------------------------------------------------------------
  ...
  nprof = MIN(nprofiles, rth%nprofs_per_call)
  nchanprof = nprof * nchannels
  luse_totalice = (use_totalice .NE. 0)
  lmmr_snowrain = (mmr_snowrain .NE. 0)
  ...
  ALLOCATE(cld_profiles(nprof), stat=err)
  THROWM(err.NE.0, 'Error allocating RTTOV-SCATT cloudy profiles')
  ...
  CALL rttov_alloc_scatt_prof( &
      ...
      cld_profiles,            &
      nlevels,                 &
      luse_totalice,           &    ! false => separate ciw and snow; true => totalice
      asw,                     &
      init = .TRUE._jplm,      &
      mmr_snowrain = lmmr_snowrain) ! snow/rain input units: false => kg/m2/s; true => kg/kg
  THROWM(err.NE.0, 'Error allocating cld_profiles')

  CALL rttov_alloc_emis_ret_terms(err, nchanprof, emis_terms, asw)
  THROWM(err.NE.0, 'Error allocating emis_terms')
  ...
  DEALLOCATE(cld_profiles, use_chan, frequencies, stat=err)
  THROWM(err.NE.0, 'Error dellocating RTTOV-SCATT arrays')

  CALL rttov_alloc_emis_ret_terms(err, nchanprof, emis_terms, asw)
  THROWM(err.NE.0, 'Error deallocating emis_terms')
...
!! @param[in]     skin              profile skin data: skin T, salinity, snow_frac, foam_frac, fastem_coefsx5,
!!                                                     specularity (10,nprofiles)
!! @param[in,out] skin_k            Jacobians for skin data (10,nchannels,nprofiles)
!! @param[in]     s2m               profile s2m data: 2m p, 2m t, 2m q, 10m wind u, v, wind-fetch (6,nprofiles)
...
!! @param[in]     surfemisrefl      input/output surface emissivities and BRDFs (nchannels,nprofiles,2)
!! @param[in,out] surfemisrefl_k    emissivity/BRDF Jacobians (nchannels,nprofiles,2)
!! @param[in,out] btrefl            returned BTs/reflectances (nchannels,nprofiles)
!! @param[in,out] rads              returned radiances (nchannels,nprofiles)
...
#include "throw.h"
  ...
  INTEGER(jpim),    INTENT(IN)    :: surftype(2,nprofiles)
  REAL(jprb),       INTENT(IN)    :: skin(10,nprofiles)
  REAL(jprb),       INTENT(INOUT) :: skin_k(10,nchannels,nprofiles)
  REAL(jprb),       INTENT(IN)    :: s2m(6,nprofiles)
...
#include "throw.h"
  ...
  TYPE(rttov_profile),       POINTER :: profiles_k(:)     => NULL()
  TYPE(rttov_profile_cloud), POINTER :: cld_profiles(:)   => NULL()
  TYPE(rttov_profile_cloud), POINTER :: cld_profiles_k(:) => NULL()
  TYPE(rttov_radiance)               :: radiance
  TYPE(rttov_radiance)               :: radiance_k
...
!------------------------------------------------------------------------------
  ...
  ALLOCATE(cld_profiles_k(nchanprof), stat=err)
  THROWM(err.NE.0, 'Error allocating RTTOV-SCATT cloudy profiles K')
  ...
  CALL rttov_alloc_scatt_prof( &
      ...
      init = .TRUE._jplm,      &
      mmr_snowrain = lmmr_snowrain) ! snow/rain input units: false => kg/m2/s; true => kg/kg
  THROWM(err.NE.0, 'Error allocating cld_profiles_k')
  ...
  CALL rttov_alloc_scatt_prof( &
      ...
      luse_totalice,  &
      asw)
  THROWM(err.NE.0, 'Error deallocating cld_profiles_k')

  DEALLOCATE(cld_profiles, cld_profiles_k, use_chan, frequencies, stat=err)
  THROWM(err.NE.0, 'Error dellocating RTTOV-SCATT arrays')
...
!------------------------------------------------------------------------------
  ...
  IF (exists) rth%opts%rt_all%addrefrac = (ival .NE. 0)
  ...
  CALL getoption('opts%rt_mw%apply_band_correction', ival, exists = exists)
  IF (exists) THEN
    rth%opts%rt_mw%apply_band_correction = (ival .NE. 0)
    rth%opts_scatt%apply_band_correction = (ival .NE. 0)
  ENDIF

  CALL getoption('opts_scatt%apply_band_correction', ival, exists = exists)
  IF (exists) THEN
    rth%opts%rt_mw%apply_band_correction = (ival .NE. 0)
    rth%opts_scatt%apply_band_correction = (ival .NE. 0)
  ENDIF
  ...
  CALL getoption('opts%rt_mw%clw_calc_on_coef_lev', ival, exists = exists)
  IF (exists) rth%opts%rt_mw%clw_calc_on_coef_lev = (ival .NE. 0)
  ...
  CALL getoption('opts_scatt%lradiance', ival, exists = exists)
  IF (exists) rth%opts_scatt%lradiance = (ival .NE. 0)
  ...
  IF (exists) rth%opts%rt_ir%ozone_data = (ival .NE. 0)

  CALL getoption('opts%rt_ir%co2_data', ival, exists = exists)
  IF (exists) rth%opts%rt_ir%co2_data = (ival .NE. 0)

  CALL getoption('opts%rt_ir%n2o_data', ival, exists = exists)
  IF (exists) rth%opts%rt_ir%n2o_data = (ival .NE. 0)

  CALL getoption('opts%rt_ir%co_data', ival, exists = exists)
  IF (exists) rth%opts%rt_ir%co_data = (ival .NE. 0)

  CALL getoption('opts%rt_ir%ch4_data', ival, exists = exists)
  IF (exists) rth%opts%rt_ir%ch4_data = (ival .NE. 0)

  CALL getoption('opts%rt_ir%so2_data', ival, exists = exists)
  IF (exists) rth%opts%rt_ir%so2_data = (ival .NE. 0)
  ...
  IF (exists) rth%opts%rt_ir%cldstr_threshold = rval

  CALL getoption('opts%rt_ir%cldstr_simple', ival, exists = exists)
  IF (exists) rth%opts%rt_ir%cldstr_simple = (ival .NE. 0)

  CALL getoption('opts%rt_ir%cldstr_low_cloud_top', rval, exists = exists)
  IF (exists) rth%opts%rt_ir%cldstr_low_cloud_top = rval
...
!   IF (exists) rth%file_pccoef = TRIM(strval)
  ...
  IF (exists) rth%file_mietable = TRIM(strval)
...
!------------------------------------------------------------------------------
  ...
  IF (rth%file_mietable .NE. '') &
    WRITE(error_unit,'(2x,a,a)')      "RTTOV-SCATT mietable ", rth%file_mietable
