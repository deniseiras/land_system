...
#include "throw.h"
  ...
  INTEGER(jpim), PARAMETER :: gas_id_scatt_cc       = 60
  ...
  INTEGER(jpim), PARAMETER :: gas_id_scatt_sp       = 64
  INTEGER(jpim), PARAMETER :: gas_id_scatt_totalice = 65
  ! --------------------------
...
#include "rttov_nullify_scattcoeffs.interface"
  ...
  allchannels = (SIZE(channels) == 1 .AND. channels(1) <= 0) .OR. (rth%file_mietable .NE. '')
...
!                           file_mfasis_aer = rth%file_mfasis_aer, &
  ...
  IF (rth%file_mietable .NE. '') THEN
    CALL rttov_read_scattcoeffs(err, rth%opts_scatt, rth%coefs, rth%coefs_scatt, &
                                file_coef = rth%file_mietable)
    THROWM(err.NE.0, 'Error reading mietable file')
  ENDIF
