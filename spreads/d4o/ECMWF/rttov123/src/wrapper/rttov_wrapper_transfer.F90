...
!! @param[in]     skin              profile skin data: skin T, salinity, snow_frac, foam_frac, fastem_coefsx5, specularity
...
!! @param[in]     clwscheme         profile clw_scheme; optional
!! @param[in]     icecloud          profile ice scheme: ice_scheme, idg; optional
...
SUBROUTINE rttov_copy_to_profiles( &
  ...
  USE rttov_const, ONLY : nwcl_max, naer_opac, naer_cams
  ...
  INTEGER(jpim),                 INTENT(IN), OPTIONAL :: clwscheme(:)     !(nprofiles)
...
!------------------------------------------------------------------------------
  ...
  DO i = 1, nprofiles
    ...
    DO g = 1, ngases
      ...
      ELSEIF (gas_id(g) == gas_id_so2 .AND. rth%opts%rt_ir%so2_data) THEN
      ...
      IF (rth%opts%rt_ir%addaerosl .AND. .NOT. rth%opts%rt_ir%user_aer_opt_param) THEN
        DO k = 1, SIZE(profiles(i)%aerosols, DIM=1)
          IF (SIZE(profiles(i)%aerosols, DIM=1) == naer_opac .AND. &
            ...
            ENDIF
          ELSEIF (SIZE(profiles(i)%aerosols, DIM=1) == naer_cams .AND. &
    ...
    profiles(i)%skin%specularity   = skin(10,j)
    ...
    IF (PRESENT(clwscheme)) THEN
      profiles(i)%clw_scheme         = clwscheme(j)
    ELSE
...
!! @param[in]     use_totalice      use_totalice flag
SUBROUTINE rttov_scatt_copy_to_profiles( &
  ...
  LOGICAL(jplm),                 INTENT(IN)    :: use_totalice

  INTEGER(jpim) :: i, j, g, nprofiles, ngases
!------------------------------------------------------------------------------
  ...
  DO i = 1, nprofiles
    ...
    DO g = 1, ngases
      ...
      ELSEIF (gas_id(g) == gas_id_scatt_cc) THEN
        cld_profiles(i)%cc = gases(:,j,g)

      ELSEIF (gas_id(g) == gas_id_scatt_clw) THEN
        cld_profiles(i)%clw = gases(:,j,g)
      ELSEIF (gas_id(g) == gas_id_scatt_rain) THEN
        cld_profiles(i)%rain = gases(:,j,g)
      ENDIF

      IF (use_totalice) THEN
        IF (gas_id(g) == gas_id_scatt_totalice) THEN
          cld_profiles(i)%totalice = gases(:,j,g)
        ENDIF
      ELSE
        IF (gas_id(g) == gas_id_scatt_ciw) THEN
          cld_profiles(i)%ciw = gases(:,j,g)
        ELSEIF (gas_id(g) == gas_id_scatt_sp) THEN
          cld_profiles(i)%sp = gases(:,j,g)
        ENDIF
      ENDIF
    ENDDO
...
SUBROUTINE rttov_copy_from_profiles_k( &
  ...
  INTEGER(jpim),                 INTENT(IN)    :: iprof1
  REAL(jprb),                    INTENT(INOUT) :: skin_k(:,:,:)        !(10/8,nchannels,nprofiles)
  ...
  DO i = 1, nprofiles
    ...
    DO j = 1, nchannels
      ...
      DO g = 1, ngases
        ...
        IF (rth%opts%rt_ir%addaerosl .AND. .NOT. rth%opts%rt_ir%user_aer_opt_param) THEN
          DO t = 1, SIZE(profiles_k(k)%aerosols, DIM=1)
            IF (SIZE(profiles_k(k)%aerosols, DIM=1) == naer_opac .AND. &
                gas_id(g) >= gas_id_aer_opac(1) .AND. gas_id(g) <= gas_id_aer_opac(naer_opac)) THEN
              IF (gas_id(g) == gas_id_aer_opac(t)) THEN
                gases_k(1:nlayers,j,iprof,g) = profiles_k(k)%aerosols(t,:)
              ENDIF
            ELSEIF (SIZE(profiles_k(k)%aerosols, DIM=1) == naer_cams .AND. &
                    gas_id(g) >= gas_id_aer_cams(1) .AND. gas_id(g) <= gas_id_aer_cams(naer_cams)) THEN
      ...
      IF (SIZE(skin_k,1) == 10) THEN
        ...
        skin_k(10,j,iprof) = profiles_k(k)%skin%specularity
      ELSE
...
SUBROUTINE rttov_copy_from_cld_profiles_k( &
  ...
  INTEGER(jpim) :: i, j, k, g, iprof, nchannels, nprofiles, ngases
!------------------------------------------------------------------------------
  ...
  DO i = 1, nprofiles
    ...
    DO j = 1, nchannels
      ...
      DO g = 1, ngases
        IF (gas_id(g) == gas_id_scatt_cc) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%cc(:)
        ELSEIF (gas_id(g) == gas_id_scatt_clw) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%clw(:)
        ELSEIF (gas_id(g) == gas_id_scatt_rain) THEN
          gases_k(:,j,iprof,g) = cld_profiles_k(k)%rain(:)
        ENDIF

        IF (use_totalice) THEN
          IF (gas_id(g) == gas_id_scatt_totalice) THEN
            gases_k(:,j,iprof,g) = cld_profiles_k(k)%totalice(:)
          ENDIF
        ELSE
          IF (gas_id(g) == gas_id_scatt_ciw) THEN
            gases_k(:,j,iprof,g) = cld_profiles_k(k)%ciw(:)
          ELSEIF (gas_id(g) == gas_id_scatt_sp) THEN
            gases_k(:,j,iprof,g) = cld_profiles_k(k)%sp(:)
          ENDIF
        ENDIF
      ENDDO
...
!! @param[in]     iprof1            starting profile number in array data
!! @param[in]     nprofiles         number of profiles to copy
!! @param[in,out] bt_k              input BT K perturbations
!! @param[in,out] rad_k             input radiance K perturbations, optional
...
!------------------------------------------------------------------------------
  ...
  DO i = 1, nprofiles
    ...
    ! For RTTOV-SCATT input perturbations are in BT only
