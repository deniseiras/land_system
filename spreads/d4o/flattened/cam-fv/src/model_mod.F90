...
real(r8) :: no_assim_above_scaleh    = -1.0_r8 
...
! amsu-a rescaling for vertical localization
real(r8)           ::  amsua_rescale = 0.5_r8

namelist /rad_localization_nml/  &
         amsua_rescale
...
! Read the DART namelist for amsu-a vert localization
call find_namelist_in_file('input.nml', 'rad_localization_nml', iunit)
read(iunit, nml = rad_localization_nml, iostat = io)
call check_namelist_read(iunit, io, 'rad_localization_nml')
!write(*,*) "AMSUA_RESCALE=",amsua_rescale
...
real(r8) :: lon_fract, lat_fract
real(r8) :: lon_lat_vert(3), quad_vals( ens_size, 4)
...
if (which_vert == VERTISHEIGHT) then
   if (vert_value > no_assim_above_height) my_status = 15
   return
endif

if (which_vert == VERTISSCALEHEIGHT) then
   if (vert_value > no_assim_above_scaleh) my_status = 16
   return
endif
if (which_vert == VERTISLEVEL) then
   ! level 1 is top; watch less than/greater than in tests
   if (vert_value < no_obs_assim_above_level) my_status = 17
   return
endif
...
if (no_normalization_of_scale_heights) then
   ...
   if (ptype == QTY_PRESSURE .and. is_vertical(location, "PRESSURE")) then
      pressure_array(:) = query_location(location, qloc_vloc)
      my_status = 0
   else
...
if (vertical_localization_type == VERTISHEIGHT) then 
   ...
   !  If no_normalization_of_scale_heights is true SH= log(surface_pressure/pressure),
   !  scale height will increase (go in the positive direction) with increasing height.
   !  If it is false, then SH= - log(pressure), which also goes in the positive direction
...
! for amsu-a dist
real(r8)     ::  v1, v2, vert_dist, horiz_dist

! if absolute distances aren't needed, or vertical localization isn't on,
! and obs in the top levels are not ignored,
...
if ((.not. present(dist) .or. .not. vertical_localization_on()) &
    .and. .not. discarding_high_obs ) then
...
! This returns a real value (converted from the original integer)
! which is assigned to an integer. 
vert_type  = nint(query_location(base_loc))

! if we are avoiding assimilating obs above a given vertical location, 
! test here and return 0 close obs.
if (discarding_high_obs) then
   vert_value = query_location(base_loc, qloc_vloc)
   call obs_too_high(vert_value, vert_type, status(1))
   if (status(1) /= 0) then
      num_close = 0
      return
   endif
...
!      base_type is passed in (with value 214!).  Was it extracted from base_loc?
!      It's not used here.
!      vert_type is extracted from base_loc
!      vertical_localization_type is set by namelist (vertical_localization_coord)
...
!print *, 'close_o, vval, vtype = ', i, query_location(locs(this), qloc_vloc), vert_type
   ...
   !write(*,*) "NAME QUANTITY= ", trim(get_name_for_quantity(loc_qtys(i)))
   dist(i) = get_dist(base_loc, locs(this))
   !write(*,*) "get_close_obs: dist = ", dist(i)
   ! The amsua_rescale will need to be applied only to variables that should be
   ! rescaled.
   ! This will involve a test on base_type.
   ! check if amsu-a obs l1b
   
   if ( (get_name_for_quantity(loc_qtys(i))=="QTY_BRIGHTNESS_TEMPERATURE").or. &
        (get_name_for_quantity( get_quantity_for_type_of_obs(base_type))=="QTY_BRIGHTNESS_TEMPERATURE") ) then
       ! if amsu-a obs the vertical localization must be activated
       ! otherwise you get unphysical results
       if (vertical_localization_on()) then 
         horiz_dist = get_dist(base_loc, locs(this), no_vert=.true.) 
         vert_dist = amsua_rescale* sqrt(dist(i)**2 - horiz_dist**2) !vertical rescaling
         dist(i) = horiz_dist**2 + vert_dist**2
         dist(i) = sqrt(dist(i))
         !write(*,*) "get_close_obs: horiz_dist = ", horiz_dist
         !write(*,*) "get_close_obs: vert_dist = ", vert_dist
         !write(*,*) "get_close_obs: final dist = ", dist(i)   
       else
         call error_handler(E_ERR, routine,  &
               'unexpected error: vert localisation needed for radiances', &
                source, revision, revdate)
       endif
   endif
   ...
   ! FIXME: Generally, damping is to prevent all obs from changing the state
   ! there.
   !        If anyone can think of circumstances requiring the use of
   !        VERTISUNDEF obs
   !        in CAM's damped region, we can leave this exclusion in for those
   !        cases.
   ! A better fix may be to set the vertical location of UNDEF obs = model top (layer?)
   ! just in this subroutine.  (Same in get_close_state?)
   !  if (.not. are_damping .or. vert_type == VERTISUNDEF) cycle
   if (.not. are_damping) cycle

   vert_value = query_location(locs(this), qloc_vloc)
...
! if we are avoiding assimilating obs above a given vertical location, 
! test here and return 0 close obs.
! FIXME?: Having the exclusion in get_close instead of model_interpolate
!         means that the excluded obs will not have a QC value saying 
!         that they were excluded by namelist.
if (discarding_high_obs) then
   vert_value = query_location(base_loc, qloc_vloc)
   call obs_too_high(vert_value, vert_type, status)
   if (status /= 0) then
      num_close = 0
      return
   endif
...
do i=1, num_close
   ...
   vert_type = nint(query_location(locs(this)))
   !print *, 'close_s, vval, vtype = ', i, query_location(locs(this), qloc_vloc), vert_type
   ...
   dist(i) = get_dist(base_loc, locs(this))
   !write(*,*) "get_close_state: dist = ", dist(i)
   ! The amsua_rescale will need to be applied only to variables that should be
   ! rescaled.
   ! This will involve a test on base_type.
   ! check if amsu-a obs l1b
   !write(*,*) "OBS BASE CONVERSION: ", get_name_for_quantity( get_quantity_for_type_of_obs(base_type) )

   if (get_name_for_quantity( get_quantity_for_type_of_obs(base_type) )=="QTY_BRIGHTNESS_TEMPERATURE") then
       ! if amsu-a obs the vertical localization must be activated
       ! otherwise you get unphysical results
       if (vertical_localization_on()) then 
         horiz_dist = get_dist(base_loc, locs(this), no_vert=.true.) 
         ...
         dist(i) = horiz_dist**2 + vert_dist**2
         dist(i) = sqrt(dist(i))
         !write(*,*) "get_close_state: horiz_dist = ", horiz_dist
         !write(*,*) "get_close_state: vert_dist = ", vert_dist
         !write(*,*) "get_close_state: final dist = ", dist(i)   
       else
         call error_handler(E_ERR, routine,  &
               'unexpected error: vert localisation needed for radiances', &
                source, revision, revdate)
