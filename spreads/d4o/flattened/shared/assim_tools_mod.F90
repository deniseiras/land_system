...
use obs_sequence_mod,     only : obs_sequence_type, obs_type, dobs_type, get_num_copies, get_num_qc, &
...
num_copies = min(1,get_num_copies(obs_seq))
num_qc = min(1,get_num_qc(obs_seq))
call init_obs(observation, num_copies, num_qc) !, spill=spill)
...
!*** THIS LOOP IS A VERY SERIOUS BOTTLENECK ***
SEQUENTIAL_OBS: do i = 1, obs_ens_handle%num_vars
   ...
   if (nth_obs == 0) then
      write(msgstring, '(2(A,I12))') 'Processing observation ', i, ' of ', obs_ens_handle%num_vars
   ...
   if (ens_handle%my_pe == owner) then
         ...
         SINGLE_SS_INFLATE: if (local_single_ss_inflate) then
            ...
            do group = 1, num_groups
               ...
               if(my_inflate > 0.0_r8 .and. my_inflate_sd > 0.0_r8) then
                  ...
                  ens_var_deflate = ens_obs_var / (1.0_r8 + gamma*(sqrt(ss_inflate_base) - 1.0_r8))**2
      ...
      if (is_doing_vertical_conversion) then
         vertvalue_obs_in_localization_coord = query_location(my_obs_loc(owners_index), qloc_vloc)
         whichvert_obs_in_localization_coord = query_location(my_obs_loc(owners_index)) ! qloc_which_vert (the default)
      else
   ...
   if (nint(obs_qc) /= 0) then
      if (timing(MLOOP)) then
         write(msgstring, '(A,1X,I0)') 'sequential obs cycl: obs', keys(i)
   ...
   if (.not. close_obs_caching) then
      ...
      if (timing(GC)) then
         write(msgstring, '(A32,3(1X,I0))') 'gc_ob_NC:nobs,tot,obs# ', num_close_obs, obs_ens_handle%my_num_vars, keys(i)
      ...
      if (base_obs_loc == last_base_obs_loc) then
         ...
         call get_close_obs(gc_obs, base_obs_loc, base_obs_type, &
                            ...
                            num_close_obs, close_obs_ind, close_obs_dist, ens_handle)
         if (timing(GC)) then
            write(msgstring, '(A32,3(1X,I0))') 'gc_ob_C: nobs,tot,obs# ', num_close_obs, obs_ens_handle%my_num_vars, keys(i)
            call read_timer(t_base(GC), msgstring, t_items(GC), t_limit(GC), do_sync=.false.)
         endif
   ...
   if (.not. close_obs_caching) then
      ...
      if (timing(GC)) then
         write(msgstring, '(A32,3(1X,I0))') 'gc_st_NC:nsts,tot,obs# ', num_close_states, ens_handle%my_num_vars, keys(i)
         call read_timer(t_base(GC), msgstring, t_items(GC), t_limit(GC), do_sync=.false.)
      endif
   else
      if (base_obs_loc == last_base_states_loc) then
         ...
         call get_close_state(gc_state, base_obs_loc, base_obs_type, &
                              ...
                              num_close_states, close_state_ind, close_state_dist, ens_handle)
         if (timing(GC)) then
            write(msgstring, '(A32,3(1X,I0))') 'gc_st_C: nsts,tot,obs# ', num_close_states, ens_handle%my_num_vars, keys(i)
            call read_timer(t_base(GC), msgstring, t_items(GC), t_limit(GC), do_sync=.false.)
         endif
   ...
   if (timing(LG_GRN)) then
      write(msgstring, '(A32,1X,I0)') 'before_state_update: obs', keys(i)
...
!PAR NEED TO TURN STUFF OFF MORE EFFICEINTLY
   ...
   if (timing(LG_GRN)) then
      write(msgstring, '(A32,1X,I0)') 'state_update: obs', keys(i)
      call read_timer(t_base(LG_GRN), msgstring)
   endif
   ...
   if (timing(LG_GRN)) then
      write(msgstring, '(A32,1X,I0)') 'obs_update: obs', keys(i)
      call read_timer(t_base(LG_GRN), msgstring)
   endif
   ...
   if (timing(MLOOP)) then
      write(msgstring, '(A32,1X,I0)') 'sequential obs loop: obs', keys(i)
      call read_timer(t_base(MLOOP), msgstring, elapsed = elapse_array(i))
   endif
...
! Free up the storage
call destroy_obs(observation) !,spill=spill)
...
character(len=129) :: file_meta !< output file name
...
character(len=20)  :: task_str !< string to hold the task number
character(len=129) :: file_copies !< output file name
integer :: i
...
character(len=20)  :: task_str !< string to hold the task number
character(len=20)  :: ob_str !< string to hold ob number
character(len=129) :: file_dist !< output file name
integer :: i
