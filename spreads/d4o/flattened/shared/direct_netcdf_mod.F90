...
if (state_ens_handle%distribution_type == 1) then
   ...
   call send_to(map_pe_to_task(state_ens_handle, recv_pe), buffer, request=request, maxreq=maxreq)
   if (maxreq > 0) call waitall(request(1:maxreq))
   deallocate(buffer)
...
if (state_ens_handle%distribution_type == 1) then
   ...
   ! MPI vector data type or packing should be used here.
   allocate(buffer(finish - start + 1))
   call receive_from(map_pe_to_task(state_ens_handle, recv_pe), buffer, request=request, maxreq=maxreq)
   if (maxreq > 0) call waitall(request(1:maxreq))
...
if (state_ens_handle%distribution_type == 1) then
   ...
   ! MPI vector data type or packing should be used here.
   allocate(buffer(elm_count))
   call receive_from(map_pe_to_task(state_ens_handle, sending_pe), buffer, request=request, maxreq=maxreq)
   if (maxreq > 0) call waitall(request(1:maxreq))
...
get_start_rank = mod(get_sum_variables_below(variable, domain), int(task_count(),8))
