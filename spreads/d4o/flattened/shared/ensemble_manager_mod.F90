...
use mpi_utilities_mod, only : task_count, my_task_id, send_to, receive_from, &
                              task_sync, broadcast_send, broadcast_recv, waitall, use_non_blocking_comms
...
integer :: requests(4)
integer :: numreq, maxreq
logical :: LLnb
LLnb = use_non_blocking_comms()
!LLnb = .FALSE.
...
if(ens_handle%my_pe == receiving_pe) then
   ...
   call receive_from(map_pe_to_task(ens_handle, owner), vars, mtime &
        , request = requests(numreq+1:numreq+2), maxreq = maxreq, non_blocking = LLnb)
...
if(ens_handle%my_pe == owner) then
   ...
   if(present(mtime)) then
      ...
      call send_to(map_pe_to_task(ens_handle, receiving_pe), ens_handle%vars(:, owners_index) &
           , request = requests(numreq+1:numreq+1), maxreq = maxreq, non_blocking = LLnb)
   endif
   numreq = numreq + maxreq
endif
!------ End of block ---------------------------------------------------------------------
...
real(r8),    allocatable :: transfer_temp2(:,:)
...
allocate(var_list(max_num_vars), copy_list(max_num_copies))

if ( use_var2copy_rec_loop ) then ! use updated version
     ...
     if (my_pe == recv_pe) then
        ...
        RECEIVE_FROM_EACH: do sending_pe = 0, num_pes - 1
           ...
           if (sending_pe /= recv_pe) then
              numreq = 0
              allocate(requests(num_copies_to_receive))
              allocate(transfer_temp2(my_num_vars,num_copies_to_receive))
           endif
           ...
           ALL_MY_COPIES: do k = 1, num_copies_to_receive
              ...
              if (sending_pe == recv_pe) then
                 ...
                 call receive_from(map_pe_to_task(ens_handle, sending_pe), transfer_temp2(1:my_num_vars,k),request=requests(numreq+1:numreq+1),maxreq=maxreq)
                 numreq = numreq + maxreq
              endif
           ...
           if (sending_pe /= recv_pe) then
              ...
              ALL_MY_COPIES_EPILOG: do k = 1, num_copies_to_receive
                 global_ens_index = copy_list(k)
                 ens_handle%copies(global_ens_index, :) = transfer_temp2(1:my_num_vars,k)
              enddo ALL_MY_COPIES_EPILOG
              
              deallocate(transfer_temp2)
              deallocate(requests)
           endif
        ...
        numreq = 0
        allocate(requests(my_num_copies))
        allocate(transfer_temp2(num_vars_to_send,my_num_copies))
        
        do k = 1, my_num_copies
           do sv = 1, num_vars_to_send
              ! Have to use temp because %var section is not contiguous storage
              transfer_temp2(sv,k) = ens_handle%vars(var_list(sv), k)
           enddo
           call send_to(map_pe_to_task(ens_handle, recv_pe), transfer_temp2(1:num_vars_to_send,k),request=requests(numreq+1:numreq+1),maxreq=maxreq)
           numreq = numreq + maxreq
        end do
  ...
  allocate(transfer_temp(max_num_vars))
...
if (use_copy2var_send_loop) then
...
SENDING_PE_LOOP: do sending_pe = 0, num_pes - 1
   ...
   if (my_pe /= sending_pe ) then
      ...
      if( num_vars_to_receive > 0 ) then
         numreq = 0
         allocate(requests(my_num_copies))
         allocate(transfer_temp2(num_vars_to_receive,my_num_copies))
         
         ! Loop to receive these vars for each copy stored on my_pe
         ALL_MY_COPIES_RECV_LOOP: do k = 1, my_num_copies
            call receive_from(map_pe_to_task(ens_handle, sending_pe), transfer_temp2(1:num_vars_to_receive,k),request=requests(numreq+1:numreq+1),maxreq=maxreq)
            numreq = numreq + maxreq
         enddo ALL_MY_COPIES_RECV_LOOP

         if (numreq > 0) call waitall(requests(1:numreq))
         
         ALL_MY_COPIES_RECV_LOOP_EPILOG: do k = 1, my_num_copies
            ! Copy the transfer array to my local storage
            do sv = 1, num_vars_to_receive
               ens_handle%vars(var_list(sv), k) = transfer_temp2(sv,k)
            enddo
         enddo ALL_MY_COPIES_RECV_LOOP_EPILOG
      ...
      do recv_pe = 0, num_pes - 1
         ...
         if (my_pe /= recv_pe ) then
            if (my_num_vars > 0) then
               numreq = 0
               allocate(requests(num_copies_to_send))
               allocate(transfer_temp2(my_num_vars,num_copies_to_send))
            endif
         endif
         ...
         SEND_COPIES: do copy = 1, num_copies_to_send
            if (my_pe /= recv_pe ) then
               ...
               if (my_num_vars > 0) then
                  transfer_temp2(1:my_num_vars,copy) = ens_handle%copies(copy_list(copy), :)
                  ! Have to  use temp because %copies section is not contiguous storage
                  call send_to(map_pe_to_task(ens_handle, recv_pe), transfer_temp2(1:my_num_vars,copy),request=requests(numreq+1:numreq+1),maxreq=maxreq)
                  numreq = numreq + maxreq
               endif
               
            else
               
               ! figure out what piece to receive from myself and receive it
               call get_var_list(ens_handle, num_vars, sending_pe, var_list, num_vars_to_receive)
               do k = 1,  my_num_copies
...
if (do_contents .and. allocated(ens_handle%vars)) then
   do j = 1, min(ens_handle%my_num_copies, limit_count)
      do i = 1, min(ens_handle%num_vars, int(limit_count,8)) ! Warning : kind(i) == 4 : kind(%num_vars) == 8 !!!
...
!   write(0,'(1x,a,i0,1x,i0)') 'assign_tasks_to_pes: nEns_members,layout_type(in) =',nEns_members,layout_type
!endif
...
!if (my_task_id()==0) then
!-   write(0,'(1x,a,i0,1x,i0)') 'assign_tasks_to_pes: nEns_members,layout_type(out)=',nEns_members,layout_type
!   write(0,'(1x,a,i0,a)') 'ens_handle%task_to_pe_list(1:',num_pes,')='
!   write(0,'((10i8))') ens_handle%task_to_pe_list(:)
!endif
