...
use obs_sequence_mod,      only : read_obs_seq, obs_type, obs_sequence_type,                  &
                                  ...
                                  get_obs_def, get_time_range_keys, set_obs_values,  &
...
use obs_d4o_mod,           only : d4o_format, d4o_catalog, d4o_debug, get_d4o_departures, get_d4o_inflation, &
                                  get_d4o_aux, put_d4o_aux

use fd4o_mod,              only : fd4o_debug, fd4o_mpirank, fd4o_mpisize
...
!GC: to use filter in two different working mode, skipping the departure
!computation or the assimilation. It must be yes or no or all
!character(len=4096)  :: d4o_departures=' '
integer, save :: i_d4o_departures = -9999 ! accepted values = -1, 0, 1
! yes =>  1
! no  =>  0
! all => -1
integer, save :: i_d4o_inflation = -9999 ! accepted values = -1, 0, 1
! yes =>  1
! no  =>  0
! final => -1
...
integer :: myrank, numranks, io_task

integer :: num_obs, ierr
integer(d4o_int1), allocatable  :: istatus_prior(:,:) ! can be num_obs x ens_size
real(r8), allocatable :: prior(:,:)         ! can be num_obs x ens_size
...
d4o_debug = (fd4o_debug() > 0)
if (d4o_format) then
   obs_sequence_in_name  = trim(d4o_catalog)
!  obs_sequence_out_name = trim(d4o_catalog) ! needless, since ALWAYS via catalog (after read_d4o_catalog kept in-memory)
endif
myrank = my_task_id()
numranks = task_count()

io_task = 0 ! "owns" ALL seq%obs(:)%values(:) for sure
...
!GC: get the current value of the environmet variable for the filter working mode
!call get_environment_variable('d4o_departures',d4o_departures)
!d4o_departures=trim(d4o_departures)
i_d4o_departures = get_d4o_departures() ! will abort if undefined or invalid value for d4o_departures; will also display the value on task#0
i_d4o_inflation = get_d4o_inflation() ! will abort if undefined or invalid value for d4o_inflation; will also display the value on task#0

!if (d4o_departures=='yes' .or. d4o_departures=='no' .or. d4o_departures=='all') then
!   call trace_message('d4o_departures = '//trim(d4o_departures))
!else
!  write(msgstring, '(A,I5)') 'd4o_departures must be "yes" or "no" or "all" '
...
num_obs = 0
if (associated(seq%obs)) num_obs = size(seq%obs(:)) ! global number of obs

if (i_d4o_departures == 1) then ! phase-1, d4o_departures=YES
   allocate(istatus_prior(num_obs,ens_size)) ! phase-1, d4o_departures=YES
   istatus_prior(:,:) = 0 ! pretty expensive !! can we avoid this initialization at all ?
endif

if (i_d4o_departures == 0) then ! phase-2, d4o_departures=NO : restore & replicate (istatus_prior,prior) values of each ens from the "seq" of the io_task
   call trace_message('Begin of AdvanceTime, restoring (istatus_prior,prior)','filter_main:',num_obs)
   call get_d4o_aux(io_task,seq,istatus_prior,prior,ierr)
   if (ierr /= num_obs) then
      call error_handler(E_ERR,'filter_main','unable to restore & replicate all (istatus_prior,prior) values in get_d4o_aux()',&
...
AdvanceTime : do
   ...
   if(my_task_id() == 0) then
      write(0,*) 'After  move_ahead: ens_size,key_bounds(1:2),num_obs_in_set=',&
                                     ens_size,key_bounds(1:2),num_obs_in_set
   endif
...
#if 0
   write(0,'(1x,a,3(i0,1x),/,(10i8))') &
        'get_time_range_keys: key_bounds(1:2),num_obs_in_set,keys(1:num_obs_in_set)=',&
                              key_bounds(1:2),num_obs_in_set,keys(1:num_obs_in_set)
#endif
...
!GC we inflate everything outside, we do not need to inflate again
   ...
   call get_obs_ens_distrib_state(state_ens_handle, obs_fwd_op_ens_handle, &
        ...
        opt_i_d4o_departures=i_d4o_departures,&
        opt_istatus_prior=istatus_prior,&
        opt_prior=prior)
   
   if (i_d4o_departures /= 0) then
      call     trace_message('d4o_departures=all or yes, call the forward operator and screening')
      ...
      call trace_message('d4o_departures=no, skip the forward operator and screening')
   end if !d4o_departures
   ...
   !GC if we compute the departures we are in phase 1 and no assimilation is
   !required. If d4o_departures=no, all then you can assimilate
   
   if (i_d4o_departures==1) then
      call     trace_message('d4o_departures=yes, skip assimilation')
   else
   ...
   if (compute_posterior .and. i_d4o_departures /= 1) then
...
!           opt_prior=prior)
   ...
   if (write_obs_every_cycle) then
      ...
      ! Only pe 0 outputs the observation space diagnostic file : not true for d4o
...
if (i_d4o_departures == 1) then ! phase-1, d4o_departures=YES : collect & save istatus_prior values of each ens to the "seq" of the io_task
   call trace_message('End of AdvanceTime, saving istatus_prior','filter_main:',num_obs)
   call put_d4o_aux(io_task,seq,istatus_prior,ierr)
   if (ierr /= num_obs) then
      call error_handler(E_ERR,'filter_main','unable to collect & save all istatus_prior values in put_d4o_aux()',&
           source, lineno=__LINE__)
   endif
endif

if (allocated(istatus_prior)) deallocate(istatus_prior)
...
call trace_message('Before obs memory cleanup') !LGG

! Free up the obs sequence
!LGG call destroy_obs_sequence(seq)
...
! Is this run using d4o-database ?
call get_environment_variable('d4o_catalog',d4o_catalog)
d4o_format = (d4o_catalog /= ' ')
...
subroutine filter_setup_obs_sequence(seq, in_obs_copy, obs_val_index, &
     input_qc_index, DART_qc_index, do_post, ens_size)
...
call read_obs_seq_header(obs_sequence_in_name, tnum_copies, tnum_qc, tnum_obs, tmax_num_obs, &
     obs_seq_file_id, obs_seq_read_format, pre_I_format, close_the_file = .true., ens_size=ens_size)
...
if (my_task == io_task) then
   ...
   if (do_post) then
      ...
      copies_num_inc = 4 + (2 * num_output_obs_members) ! NB: with d4o num_output_obs_members must be ens size
   else
      ! 2 is for prior mean and spread, plus
      ! prior values for all requested members
      copies_num_inc = 2 + (1 * num_output_obs_members) ! NB: with d4o num_output_obs_members must be ens size
   endif
else
   copies_num_inc = 0
endif
...
integer               :: ivalue, io_task, my_task, istatus
