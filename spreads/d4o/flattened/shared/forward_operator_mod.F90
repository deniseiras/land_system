...
subroutine get_obs_ens_distrib_state(ens_handle, obs_fwd_op_ens_handle, &
   ...
   opt_i_d4o_departures,opt_istatus_prior,opt_prior)
...
integer,          intent(in), optional :: opt_i_d4o_departures
integer(d4o_int1), intent(inout), optional :: opt_istatus_prior(:,:) ! num_obs x num_ens
real(r8),intent(in), optional :: opt_prior(:,:)               ! num_obs x num_ens
...
if (d4o_int1 == 1) then
   istatus = 123 ! we prefer values -128 .. +127 to stick within one-byte memory (rather than 4) per istatus -- so "999123" is a no-go
else
   istatus = 999123
endif
...
if(get_allow_transpose(ens_handle)) then ! giant if for transpose or distributed forward op
   ...
   thiskey(1) = keys(global_obs_num) ! *this* defines what key gets picked for this task# !!!
...
real(r8),                intent(out)   :: expected_obs(num_ens)  !! the array of computed forward operator values
integer, intent(in), optional    :: opt_i_d4o_departures
integer(d4o_int1), intent(inout), optional :: opt_istatus_prior(:,:) ! num_obs x num_ens
real(r8),intent(in), optional    :: opt_prior(:,:)            ! num_obs x num_ens
...
integer :: num_obs, i, member, ikey, i_d4o_departures

i_d4o_departures = -1
if (present(opt_i_d4o_departures)) i_d4o_departures = opt_i_d4o_departures
num_obs = size(keys)

!>@todo do you ever use this with more than one obs?
! WE ONLY CALL THIS with num_obs == 1
do i = 1, num_obs
   ! NEED to initialize istatus to okay value
   istatus = 0

   ! NEED to initialize expected_obs as missing data
   expected_obs = MISSING_R8
   ...
   ikey = keys(i)
   call get_obs_from_key(seq, ikey, obs)
   ...
   if (obs_kind_ind < 0) then
      ...
      if (isprior .and. i_d4o_departures == 0) then ! phase-2 [NO] -- priors & istatuses as retrieved from database
         istatus = opt_istatus_prior(ikey,:)
         expected_obs = opt_prior(ikey,:)
         assimilate_this_ob = assimilate_this_type_of_obs(obs_kind_ind)
         evaluate_this_ob = evaluate_this_type_of_obs(obs_kind_ind)
#if 0
         write(0,*) 'get_expected_obs_distrib_state(phase-2): ikey,istatus,expected_obs=',&
                                                              ikey,istatus,expected_obs
#endif
      else
         call get_expected_obs_from_def_distrib_state(state_ens_handle, num_ens, copy_indices, ikey, &
              obs_def, obs_kind_ind, state_time, isprior, &
              assimilate_this_ob, evaluate_this_ob, expected_obs, istatus)
         if (isprior .and. i_d4o_departures == 1) then ! phase-1 [YES] -- istatuses destined to database
            opt_istatus_prior(ikey,:) = istatus
#if 0
            write(0,*) 'get_expected_obs_distrib_state(phase-1): ikey,istatus,expected_obs=',&
                                                                 ikey,istatus,expected_obs
#endif
         endif
      endif
