...
integer, public, parameter :: qloc_lat = 1
integer, public, parameter :: qloc_lon = 2
integer, public, parameter :: qloc_vloc = 3
integer, public, parameter :: qloc_which_vert = 4
...
if (nlon < 1 .or. nlat < 1) then
   write(msgstring, '(A,I0,A,I0)') 'from the namelist, nlon is ', nlon, '; nlat is ', nlat
...
! One additional restriction; number of longitudes, nlon, for get_close -- lets not fail with this !
!if((nlon / 2) * 2 == nlon) then
if(mod(nlon,2) == 2) then
   write(msgstring, '(A,I0,A,I0)') 'nlon is ', nlon, '. Must be odd. Adding one to it => nlon=',nlon+1
   write(*,'(1x,a)') 'location_mod: '//trim(msgstring)
   nlon = nlon + 1
...
if (present(which_vert)) which_vert = loc%which_vert

end function get_location

!---------------------------------------------------------------------------

function set_location_single(lon, lat, vert_loc, which_vert, radians)
...
logical :: LLradians
real(r8) :: tmplon, tmplat

if ( .not. module_initialized ) call initialize_module()

LLradians = .FALSE. ! by default (lon,lat) are assumed to be in degrees
! In case they are already in radians, supply argument radians=.TRUE.
if (present(radians)) LLradians = radians

tmplon = lon
if (LLradians) tmplon = mod(lon*RAD2DEG + 360.0_r8,360.0_r8)
if(tmplon < 0.0_r8 .or. tmplon > 360.0_r8) then
   write(msgstring,'(1x,a,G25.16,a,L1)') &
        & 'longitude (',tmplon,') is not within range [0,360] : radians=',&
        & LLradians
   call error_handler(E_ERR, 'set_location', msgstring, source)
endif

tmplat = lat
if (LLradians) tmplat = lat*RAD2DEG
if(tmplat < -90.0_r8 .or. tmplat > 90.0_r8) then
   write(msgstring,'(1x,a,G25.16,a,L1)') &
        & 'latitude (',tmplat,') is not within range [-90,90] : radians=',&
        & LLradians
   call error_handler(E_ERR, 'set_location', msgstring, source)
endif

#if 1
set_location_single%lon = tmplon * DEG2RAD
set_location_single%lat = tmplat * DEG2RAD
#else
if (LLradians) then ! do not convert
   set_location_single%lon = lon
   set_location_single%lat = lat
else
...
subroutine get_boxdims(nlon_out, nlat_out)
  ! Return (nlon,nlat)
  integer, intent(out) :: nlon_out, nlat_out
  if ( .not. module_initialized ) call initialize_module()
  nlon_out = nlon
  nlat_out = nlat
end subroutine get_boxdims
...
type(location_type),        intent(in) :: loc
integer, optional, intent(in) :: attr
...
select case(attr)
   ...
   case (qloc_which_vert)
      ! already set
   case default
      call error_handler(E_ERR, 'query_location:', &
         'Only "qloc_lon","qloc_lat","qloc_vloc","qloc_which_vert" are legal attributes to request from location', &
          source)
end select
...
write(string1, '(A,2(F12.8,1X,"0x",Z16.16,1X),A)') 'Lon/Lat(deg): ', &
     loc%lon*RAD2DEG, loc%lon*RAD2DEG, &
     loc%lat*RAD2DEG, loc%lat*RAD2DEG, &
     ' Vert:'
...
select case  (loc%which_vert)
   ...
   case (VERTISSURFACE)
      write(charstring, '(A,F13.5,1X,"0x",Z16.16,A)') trim(string1), loc%vloc, loc%vloc, ' surface (m)'
   case (VERTISLEVEL)
      write(charstring, '(A,F13.6,1X,"0x",Z16.16,A)') trim(string1), loc%vloc,loc%vloc,  '  level'
   case (VERTISPRESSURE)
      write(charstring, '(A,F13.7,1X,"0x",Z16.16,A)') trim(string1), loc%vloc / 100.0_r8, loc%vloc / 100.0_r8, ' hPa'
   case (VERTISHEIGHT)
      write(charstring, '(A,F13.7,1X,"0x",Z16.16,A)') trim(string1), loc%vloc / 1000.0_r8, loc%vloc / 1000.0_r8, ' km'
   case (VERTISSCALEHEIGHT)
      write(charstring, '(A,F13.7,1X,"0x",Z16.16,A)') trim(string1), loc%vloc,loc%vloc,  ' scale ht'
   case default
...
if (howmuch /= 0 .and. iam0) then
   ...
   call error_handler(E_MSG, 'loc', msgstring)
   write(msgstring, "(A,3(F12.6))") ' latbox: bot, top, width = ', &
        & gc%gtt(whichtt)%bot_lat, gc%gtt(whichtt)%top_lat, gc%gtt(whichtt)%lat_width
   call error_handler(E_MSG, 'loc', msgstring)
   write(msgstring, "(A,3(F12.6))") ' lonbox: bot, top, width = ', &
        & gc%gtt(whichtt)%bot_lon, gc%gtt(whichtt)%top_lon, gc%gtt(whichtt)%lon_width
   call error_handler(E_MSG, 'loc', msgstring)

   write(msgstring, "(A,F12.6)") ' maxdist = ', RAD2DEG*gc%gtt(whichtt)%maxdist
   call error_handler(E_MSG, 'loc', msgstring)
   write(msgstring, "(A,3(F12.6))") ' latbox: bot, top, width = ', &
        & RAD2DEG*gc%gtt(whichtt)%bot_lat, RAD2DEG*gc%gtt(whichtt)%top_lat, RAD2DEG*gc%gtt(whichtt)%lat_width
   call error_handler(E_MSG, 'loc', msgstring)
   write(msgstring, "(A,3(F12.6))") ' lonbox: bot, top, width = ', &
        & RAD2DEG*gc%gtt(whichtt)%bot_lon, RAD2DEG*gc%gtt(whichtt)%top_lon, RAD2DEG*gc%gtt(whichtt)%lon_width
   call error_handler(E_MSG, 'loc', msgstring)
