...
interface waitall
   module procedure waitall_single
   module procedure waitall_array
...
public :: initialize_mpi_utilities, finalize_mpi_utilities,                  &
          task_count, my_task_id, task_comm, block_task, restart_task,       &
          ...
          waitall, use_non_blocking_comms, &
...
! Disable/enable non-blocking comms ?
logical :: non_blocking_comms = .TRUE. ! export non_blocking_comms=0 or f or F to disable
...
logical function use_non_blocking_comms()
  use_non_blocking_comms = non_blocking_comms
end function use_non_blocking_comms
...
if (errcode /= MPI_SUCCESS) then
   ...
   !call exit(-99)
   call fd4o_exit('MPI_Initialized returned error code ',errcode,1)
endif
if (.not.already) then
   ...
   if (errcode /= MPI_SUCCESS) then
      ...
      !call exit(-99)
      call fd4o_exit('MPI_Init returned error code ',errcode,1)
   endif
endif
...
if (errcode /= MPI_SUCCESS) then
   ...
   !call exit(-99)
   call fd4o_exit('MPI_Comm_rank returned error code ',errcode,1)
endif
...
! Always initialize d4o regardless of the truth value of the "d4o_format"
call static_d4o_init(my_local_comm)
...
call get_environment_variable('non_blocking_comms',clenv)
if (clenv /= ' ') then
   if (clenv == '1' .or. clenv == 't' .or. clenv == 'T') then
      non_blocking_comms = .TRUE.
   else if (clenv == '0' .or. clenv == 'f' .or. clenv == 'F') then
      non_blocking_comms = .FALSE.
   endif
endif

if (myrank == 0) then
   if (non_blocking_comms) then
      write(errstring, '(a)') "export non_blocking_comms=T # using non-blocking comms"
   else
      write(errstring, '(a)') "export non_blocking_comms=F # not using non-blocking comms"
   endif
...
use fd4o_mod, only : fd4o_finalize
...
call task_sync() ! this is MPI_Barrier()
...
if (dofinalize) then
   ...
   write(*,*) "PE", myrank, ": d4o_finalize() followed by MPI_Finalize() is being called now ..."
   call fd4o_finalize() ! any outstanding I/O due to d4o and/or SQLite completes here
   ! According to the MPI std:
   ! (1) All processes must call this routine before exiting
   ! (2) The number of processes running AFTER this routine is called is undefined; it is best not to perform much more than a return rc after calling MPI_Finalize.
...
function task_count(comm)
integer, intent(in), optional :: comm
integer :: task_count, ierr
...
if (present(comm)) then
   CALL MPI_Comm_size(comm,task_count,ierr)
else
...
function my_task_id(comm)
integer, intent(in), optional :: comm
integer :: my_task_id,ierr
...
if (present(comm)) then
   CALL MPI_Comm_rank(comm,my_task_id,ierr)
else
   my_task_id = myrank
endif
...
function task_comm(comm)
  integer, intent(in), optional :: comm
  integer :: task_comm
  if (present(comm)) then
     task_comm = comm
  else
     task_comm = my_local_comm
  endif
end function task_comm

subroutine waitall_single(req)
  integer, intent(inout) :: req
  integer STATUS(MPI_STATUS_SIZE)
  integer errcode
  call MPI_Wait(req, STATUS, errcode)
  if (errcode /= MPI_SUCCESS) then
     write(errstring, '(a,i8)') 'MPI_Wait returned error code ', errcode
     call error_handler(E_ERR,'waitall_single', errstring, source)
  endif
end subroutine waitall_single

subroutine waitall_array(req)
  integer, intent(inout) :: req(:)
  integer ARRAY_OF_STATUSES(MPI_STATUS_SIZE,size(req))
  integer numreq, errcode
  numreq = size(req)
  if (numreq == 1) then
     call waitall(req(1))
  else if (numreq > 1) then
     call MPI_Waitall(numreq, req, ARRAY_OF_STATUSES, errcode)
     if (errcode /= MPI_SUCCESS) then
        write(errstring, '(a,i0,a,i8)') 'MPI_Waitall(req(1:',numreq,')) returned error code ', errcode
        call error_handler(E_ERR,'waitall_array', errstring, source)
     endif
  endif
end subroutine waitall_array
...
subroutine send_to(dest_id, srcarray, time, label, request, maxreq, non_blocking)
 integer, intent(in) :: dest_id
 real(r8), intent(in) :: srcarray(:)
 type(time_type), intent(in), optional :: time
 character(len=*), intent(in), optional :: label
 integer, INTENT(OUT), optional :: request(:)
 integer, intent(out), optional :: maxreq
...
if (present(maxreq)) maxreq = 0
LLnb = non_blocking_comms
if (present(non_blocking)) LLnb = non_blocking
LLrequest = (LLnb .and. itemcount <= SNDRCV_MAXSIZE .and. .not. make_copy_before_sendrecv .and. present(maxreq) .and. present(request))
if (LLrequest) then
   if (present(time) .and. size(request) >= 2) then
      LLrequest = .TRUE.
   else if (.not. present(time) .and. size(request) >= 1) then
      LLrequest = .TRUE.
   else
      LLrequest = .FALSE.
   endif
...
if (itemcount <= SNDRCV_MAXSIZE) then
   ...
   if (.not. make_copy_before_sendrecv) then
      if (LLrequest) then
         maxreq = maxreq + 1
         call MPI_Isend(srcarray, itemcount, datasize, dest_id, tag, &
              my_local_comm, request(maxreq), errcode)
      else
...
if (errcode /= MPI_SUCCESS) then
   if (LLrequest) then
      write(errstring, '(a,i8)') 'MPI_Isend returned error code ', errcode
      call error_handler(E_ERR,'send_to', errstring, source)
   else
      write(errstring, '(a,i8)') 'MPI_Ssend returned error code ', errcode
      call error_handler(E_ERR,'send_to', errstring, source)
...
if (present(time)) then
   ...
   if (LLrequest) then
      maxreq = maxreq + 1
      call MPI_Isend(itime, 2, MPI_INTEGER, dest_id, tag*2, my_local_comm, request(maxreq), errcode)
      if (errcode /= MPI_SUCCESS) then
         write(errstring, '(a,i8)') 'MPI_Isend returned error code ', errcode
         call error_handler(E_ERR,'send_to', errstring, source)
      endif
...
subroutine receive_from(src_id, destarray, time, label, request, maxreq, non_blocking)
...
if (itemcount <= SNDRCV_MAXSIZE) then
   if (.not. make_copy_before_sendrecv) then
      if (LLrequest) then
         maxreq = maxreq + 1
         call MPI_Irecv(destarray, itemcount, datasize, src_id, tag, &
              my_local_comm, request(maxreq), errcode)
      else
         call MPI_Recv(destarray, itemcount, datasize, src_id, tag, &
...
if (errcode /= MPI_SUCCESS) then
   if (LLrequest) then
      write(errstring, '(a,i8)') 'MPI_Irecv returned error code ', errcode
      call error_handler(E_ERR,'receive_from', errstring, source)
   else
      write(errstring, '(a,i8)') 'MPI_Recv returned error code ', errcode
      call error_handler(E_ERR,'receive_from', errstring, source)
...
if (present(time)) then
   ...
   if (LLrequest) then
      maxreq = maxreq + 1
      call MPI_Irecv(itime, 2, MPI_INTEGER, src_id, tag*2, &
           my_local_comm, request(maxreq), errcode)
      if (errcode /= MPI_SUCCESS) then
         write(errstring, '(a,i8)') 'MPI_Irecv returned error code ', errcode
         call error_handler(E_ERR,'receive_from', errstring, source)
      endif
      call waitall(request(maxreq:maxreq))
      maxreq = maxreq - 1
   else
...
subroutine array_broadcast(array, root, icount, request, maxreq)
...
logical :: LLrequest, LLnb
LLnb = non_blocking_comms
LLnb = .FALSE. ! *must* use blocking MPI_Bcast or results incorrect for some reason
...
if (present(maxreq)) maxreq = 0
LLrequest = (LLnb .and. itemcount <= BCAST_MAXSIZE .and. .not. make_copy_before_broadcast .and. present(maxreq) .and. present(request))
...
if (itemcount <= BCAST_MAXSIZE) then
   if (.not. make_copy_before_broadcast) then
      if (LLrequest) then
         maxreq = 1
         call MPI_Ibcast(array, itemcount, datasize, root, my_local_comm, request, errcode)
      else
...
if (errcode /= MPI_SUCCESS) then
   if (LLrequest) then
      write(errstring, '(a,i8)') 'MPI_Ibcast returned error code ', errcode
      call error_handler(E_ERR,'array_broadcast', errstring, source)
   else
      write(errstring, '(a,i8)') 'MPI_Bcast returned error code ', errcode
      call error_handler(E_ERR,'array_broadcast', errstring, source)
...
integer  :: itemcount
integer :: requests(6), numreq, maxreq
...
if (itemcount <= PACKLIMIT .and. morethanone) then
   ...
   call array_broadcast(packbuf, from, itemcount, request=requests(numreq+1), maxreq=maxreq)
   numreq = numreq + maxreq

else

   call array_broadcast(array1, from, request=requests(numreq+1), maxreq=maxreq)
   numreq = numreq + maxreq

   if (morethanone) then
      if (present(array2)) then
         call array_broadcast(array2, from, request=requests(numreq+1), maxreq=maxreq)
         numreq = numreq + maxreq
      endif
      if (present(array3)) then
         call array_broadcast(array3, from, request=requests(numreq+1), maxreq=maxreq)
         numreq = numreq + maxreq
      endif
      if (present(array4)) then
         call array_broadcast(array4, from, request=requests(numreq+1), maxreq=maxreq)
         numreq = numreq + maxreq
      endif
      if (present(array5)) then
         call array_broadcast(array5, from, request=requests(numreq+1), maxreq=maxreq)
         numreq = numreq + maxreq
      endif
      
      if (doscalar) then
         call packscalar(local, scalar1, scalar2, scalar3, scalar4, scalar5)
         call array_broadcast(local, from, request=requests(numreq+1), maxreq=maxreq)
         numreq = numreq + maxreq
      endif
...
if (numreq > 0) call waitall(requests(1:numreq))
...
if (itemcount <= PACKLIMIT .and. morethanone) then
   ...
   if (morethanone) then
      ...
      if (doscalar) then
         call unpackscalar(local, scalar1, scalar2, scalar3, scalar4, scalar5)
      endif
   endif
...
if (non_blocking_comms) then
   call mpi_iallreduce(MPI_IN_PLACE, min_var, num_elements, datasize, MPI_MIN, get_dart_mpi_comm(), requests(1), errcode)
   call mpi_iallreduce(MPI_IN_PLACE, max_var, num_elements, datasize, MPI_MAX, get_dart_mpi_comm(), requests(2), errcode)
...
if (non_blocking_comms) then
   call MPI_Ibcast(flag, 1, MPI_LOGICAL, root, my_local_comm, req, errcode)
   call waitall(req)
else
...
call execute_command_line(trim(execute),exitstat=rc)
...
use fd4o_mod, only : fd4o_traceback
...
call fd4o_traceback("EXIT_ALL",exit_code) ! Does not EXIT -- just prints the traceback
call MPI_Abort(MPI_COMM_WORLD, exit_code, ierror) ! must use MPI_COMM_WORLD to avoid hangs on some tasks
