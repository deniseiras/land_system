...
call static_d4o_init() ! Important NOT to pass in any communicator (not even a dummy one) here
...
integer, intent(in), optional :: comm ! not used, just for compatibility
integer :: task_count
...
subroutine send_to(dest_id, srcarray, time, label, request, maxreq, non_blocking )
 ...
 integer, intent(out), optional :: request(:),maxreq
...
! !!NAG_BLOCK_EDIT END COMMENTED_OUT
  ...
  !   call exit(exit_code)
  call fd4o_exit("EXIT_ALL",exit_code,1)
