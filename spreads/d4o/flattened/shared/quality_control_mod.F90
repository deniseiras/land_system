...
if(isprior) then
   ...
   if((.not. assimilate_this_ob) .and. (.not. evaluate_this_ob)) then
      ...
      write(*,*) 'get_dart_qc: assimilate_this_ob,evaluate_this_ob,failed_fop,evaluate_this_ob,inconsistent,isprior,dart_qc,ens_size,istatus(:)=',&
           &                   assimilate_this_ob,evaluate_this_ob,failed_fop,evaluate_this_ob,inconsistent,isprior,dart_qc,ens_size,istatus(:)
...
this_obs_name = get_name_for_type_of_obs(this_obs_type)
...
!LGG
   case ('EOS_2_AMSUA_TB', 'NOAA_15_AMSUA_TB', &
         'NOAA_16_AMSUA_TB', 'NOAA_17_AMSUA_TB', &
         'NOAA_18_AMSUA_TB', 'NOAA_19_AMSUA_TB', &
         'METOP_1_AMSUA_TB', 'METOP_2_AMSUA_TB')
      if (ratio > 2.0) then
         failed_outlier = .true.
      else
         failed_outlier = .false.
