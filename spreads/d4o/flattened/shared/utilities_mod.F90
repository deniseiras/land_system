...
subroutine error_handler(level, routine, text, src, rev, rdate, aut, text2, text3, lineno )
...
character(len=:), allocatable :: src_colon_lineno
integer :: ilen
   ...
   case (E_DBG, E_WARN, E_ERR)
      ...
      if (present(src)) then
         if (present(lineno)) then
            ilen = len_trim(src) + 20
            allocate(character(len=ilen)::src_colon_lineno)
            write(src_colon_lineno,'(a,":",i0)') trim(src),lineno
            call log_it(' source : '//trim(src_colon_lineno))
            deallocate(src_colon_lineno)
         else
            call log_it(' source : '//trim(src))
         endif
      endif
