...
#define  ALLOC(x,size)			   \
  { ll_t bytes = (ll_t)sizeof(*x) * (size);	\
    ...
    x = malloc(bytes);							\
    if (!x) { char msg[4096];						\
      snprintf(msg,sizeof(msg),"malloc() of %s (%lld bytes) failed in file=%s, line=%d\n", \
	       #x, bytes, __FILE__, __LINE__);				\
      d4o_ErrExit(__FUNCTION__,msg,-1); } }
	      ...
	      int *retc)
{
  ...
  { /* Little/big-endian selection */
    int i_am_little = d4o_is_little_endian();
...
#pragma cdir nodep
    ...
    fd4o_rsort32_(&Method, &aN, &aInc, &aStart_addr, data, index, &aIndex_adj, &rc);
...
#pragma cdir nodep
	...
	if (k1 + sum != n || k2 != n) {
	  ...
	  snprintf(msg,sizeof(msg),
		  "***Programming error in %s(): k1 + sum != n || k2 != n; k1=%d,k2=%d,sum=%d,n=%d\n",
		   __FUNCTION__,k1,k2,sum,n);
	  d4o_ErrExit(__FUNCTION__,msg,-1);
	}
      }
	              ...
	              int *retc)
{
  ...
  if (method == SORT_INT && index_adj == 1) { /* 32-bit ints ; Fortran-arrays */
    ...
    *retc = *N;
    ...
    fd4o_rsort32_(Mode, N, Inc, Start_addr, Data, index, Index_adj, retc);
  }
#else
