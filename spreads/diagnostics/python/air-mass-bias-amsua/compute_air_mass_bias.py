        ...
        for idc, channel in enumerate(chs):
            ...
            with tqdm(total=total_rows) as pbar:
              for idx in range(0, total_rows, batch_size):
                  ...
                  batch_obs_val = obs_values[idx:idx + batch_size]
                  ...
                  for obs_val, p_val in zip(batch_obs_val, closest_z):
                      pairs.append({'obsvalue': obs_val, 'pvalue': p_val})
            ...
            obs_values = pairs_df['obsvalue']
            p_values = pairs_df['pvalue']

            # Perform linear regression
            slope, intercept, r_value, p_value, std_err = linregress(obs_values, p_values)
            ...
            plt.scatter(obs_values, p_values, color='blue', alpha=0.5, label='Data')

            # Perform linear regression
            slope, intercept, r_value, p_value, std_err = linregress(obs_values, p_values)
            regression_line = slope * obs_values + intercept

            # Plot regression line
            plt.plot(obs_values, regression_line, color='red', label='Linear Regression')
