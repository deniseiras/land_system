...
# Where to save the output, plot and data. THIS MUST BE THE SAME AS THE ONE USED FOR THE FSOI_JO!!!!
path2sv = path2db + '/diag-' + adate
...
if rank == manager:
   ...
   aname = path2an + '/' + ename + '.dart.i.cam_output_mean.' + adate + '.nc'
   ...
   # OI mask to see where we really have corrections. If nan we did not corrected that location and it must be discarded by the mean
   oi_mask = np.full_like(oi, np.nan)
...
sub_oi_mask    = np.zeros([len(obs2proc), len(lev), local_NGLAT, local_NGLON], dtype=int)
sub_ocount     = np.zeros(len(obs2proc))

# global obs used counter, per type
ocount = np.zeros(len(obs2proc))

# Distribute longitide slises among tasks
# Main computation.
#------------------------------------------
...
#
#if DEBUG >= 1:
#    print("\n Task: ", rank, ' obs.  sub-domain: lat in [',latu,'    ',latd,'] and lon in [',lonl,'    ',lonr,']',  flush=True)
if lonl < 0:
    lonl1 = 0
    lonl2 = 360 + lonl
if lonr > 360:
...
#    con.close()
    ...
    con=sqlite3.connect(db)
    if lonl<0:
         df_temp = pd.read_sql("select entryno,id,obsvalue, obs_error, prior_mean, prior_spread, kind, dart_qc, member, deglat, deglon,\
                                levelht, (select distinct description from toc where kind=body.kind) \
                                as description from hdr join body on id=body.hdr_id join ens on id=ens.hdr_id and \
                                entryno=body_entryno where deglat>"+ str(latd) + " and deglat<" + str(latu) + " and deglon>="\
                                + str(lonl1) + " and deglon<" + str(lonr) +" and deglon>" + str(lonl2)+ " and deglon<=360 and dart_qc=0 and member=1", con)
    elif lonr>360:
         df_temp = pd.read_sql("select entryno,id,obsvalue, obs_error, prior_mean, prior_spread, kind, dart_qc, member, deglat, deglon,\
                                levelht, (select distinct description from toc where kind=body.kind) \
                                as description from hdr join body on id=body.hdr_id join ens on id=ens.hdr_id and \
                                entryno=body_entryno where deglat>"+ str(latd) + " and deglat<" + str(latu) + " and deglon>"\
                                + str(lonl) + " and deglon<=" + str(lonr1) +" and deglon>=0  and deglon<" + str(lonr2)  +" and dart_qc=0 and member=1", con)
    else:
         df_temp = pd.read_sql("select entryno,id,obsvalue, obs_error, prior_mean, prior_spread, kind, dart_qc, member, deglat, deglon,\
                                levelht, (select distinct description from toc where kind=body.kind) \
                                as description from hdr join body on id=body.hdr_id join ens on id=ens.hdr_id and \
                                entryno=body_entryno where deglat>"+ str(latd) + " and deglat<" + str(latu) + " and deglon>"\
                                + str(lonl) + " and deglon<" + str(lonr) +" and dart_qc=0 and member=1", con)
    con.close()
    ...
    if len(df_temp['obsvalue'].tolist())<=0:
       print("\n Task: ", rank, " No obs in this cell from this db", flush=True)
       continue
        ...
        for kk in range( len(lev) ):
              ...
              for o2p in obs2proc:
                  ...
                  for iss in range( len(m1) ):
                       ...
                       # compute the oi
                       sub_oi[idx_o2p, kk, ii, jj] = sub_oi[idx_o2p, kk, ii, jj]  + loc * prior_spread[iss]**2 /(prior_spread[iss]**2 + sigma_o[iss]**2)
                       sub_oi_sigma_o[idx_o2p, kk, ii, jj] = sub_oi_sigma_o[idx_o2p, kk, ii, jj]  + loc * sigma_o[iss]

                       sub_ocount[idx_o2p] = sub_ocount[idx_o2p]+1
                       sub_oi_mask[idx_o2p, kk, ii, jj] = 1 


                  # Normalize. PAY ATTENTION, we load a box of observations and some of that could be outside the elipsoid of the GC loc.
                  if np.sum(gch)==0:
                     norm_loc=1
                  sub_oi[idx_o2p, kk, ii, jj] = sub_oi[idx_o2p, kk, ii, jj] / norm_loc
                  sub_oi_sigma_o[idx_o2p, kk, ii, jj] = sub_oi_sigma_o[idx_o2p, kk, ii, jj] / norm_loc
...
if rank==manager:
   ...
   print('\n Task: ', rank, " building the global oi matrix", flush=True)
   # Set the first part of the matrix for rank 0
   oi[:,:,task_id['sub_row_start']:task_id['sub_row_end']+1,task_id['sub_col_start']:task_id['sub_col_end']+1] = sub_oi
   oi_mask[:,:,task_id['sub_row_start']:task_id['sub_row_end']+1,task_id['sub_col_start']:task_id['sub_col_end']+1] = sub_oi_mask
   oi_sigma_o[:,:,task_id['sub_row_start']:task_id['sub_row_end']+1,task_id['sub_col_start']:task_id['sub_col_end']+1] = sub_oi_sigma_o
   ...
   for task in range(1, size):
       ...
       recvbuf_sigma_o= np.empty([len(obs2proc),len(lev),sub_nr, sub_nc], dtype=float)
       request_sigma_o = comm.Irecv(recvbuf_sigma_o, source=task, tag=40000+task)
       recvbufs_sigma_o.append(recvbuf_sigma_o)
       requests_sigma_o.append(request_sigma_o)
   ...
   MPI.Request.Waitall(requests_sigma_o)
   ...
   for recvbuf, recvbuf_mask in zip(recvbufs, recvbufs_mask):
       ...
       oi[:,:,rs:re,cs:ce] = recvbuf
       oi_mask[:,:,rs:re,cs:ce] = recvbuf_mask
       task = task + 1
   
   task = 1
   for recvbuf_sigma_o in recvbufs_sigma_o:
       rs = task_id_list[task]['sub_row_start']
       re = task_id_list[task]['sub_row_end'] + 1
       cs = task_id_list[task]['sub_col_start']
       ce = task_id_list[task]['sub_col_end'] + 1
       oi_sigma_o[:,:,rs:re,cs:ce] = recvbuf_sigma_o
       task = task + 1
   ...
   comm.Isend(sub_oi, dest=manager, tag=rank)   
   print("\n Task: ", rank, " sending sub domain with shape: [",sub_oi.shape[0],", ",sub_oi.shape[1],", ",sub_oi.shape[2],", ",sub_oi.shape[3],"]", flush=True)
   comm.Isend(sub_oi_mask, dest=manager, tag=10000+rank)   
   comm.Isend(sub_oi_sigma_o, dest=manager, tag=40000+rank)   
...
if rank==manager:
   ...
   mask = np.isnan(oi_mask)
   masked_oi = np.ma.masked_array(oi, mask)
   oim= np.zeros(len(obs2proc))
   oisd= np.zeros(len(obs2proc))
   for io in range(len(obs2proc)):
       flattened  = masked_oi[io,:,:,:].flatten()
...
if rank==manager:
   if plot_img=='TRUE':
      ...
      fsoi_plot_bar(oim,oisd, obs2proc, ocount, title='Global (single obs types)'+'\n Date: '+adate , xlabel='$OI$ %', ylabel='', save_path=path2sv+'/oi_bar_global.png', fs=16, dpi=100)
      fsoi_plot_bar(NH_oim, NH_oisd, obs2proc, ocount, title='Northern Hemisphere'+'\n Date: '+adate, xlabel='$OI$ %', ylabel='', save_path=path2sv+'/oi_bar_nh.png', fs=16, dpi=100)
      fsoi_plot_bar(SH_oim, SH_oisd, obs2proc, ocount, title='Southern Hemisphere'+'\n Date: '+adate, xlabel='$OI$ %', ylabel='', save_path=path2sv+'/oi_bar_sh.png', fs=16, dpi=100)
      fsoi_plot_bar(TR_oim, TR_oisd, obs2proc, ocount,title='Tropics'+'\n Date: '+adate, xlabel='', ylabel='$OI$ %', save_path=path2sv+'/oi_bar_tr.png', fs=16, dpi=100)
   
      fsoi_plot_bar_gr(oim, oisd, obs2proc, ocount, title='Global'+'\n Date: '+adate , xlabel='$FSOI-J_o$ %', ylabel='', save_path=path2sv+'/oi_bar_global_gr.png', fs=16, dpi=100)   
      ...
      for obs in obs2proc:
          ...
          oigr[il]=oigr[il]+np.abs(oim[index])
      ...
      totalf = sum(oigr)
      oigr = [fgr * 100 / totalf if fgr != 0 else 0 for fgr in oigr] 
       
      
      print('labels=',labels)
      print('sizes=',sizes)
      print('oigr=',oigr)
      create_pie_chart(labels, sizes,  title='SPREADS: Obs used for the OI'+'\n Date: '+adate,colormap='tab20', dpi=100, save_path=path2sv+'/pie_global_nobs_used.png')
      create_pie_chart(labels, oigr,  title='SPREADS: Global OI Summary'+'\n Date: '+adate, colormap='tab20', dpi=100, save_path=path2sv+'/pie_global_oi.png')
      ...
      #fsoi_plot_map(fsoi_map, co_lon, co_lat, title='FSOI-Jo, All Obs., Vertical Integration.'+'\n Date: '+adate, clabel='$FSOI-J_{o}$', xlabel='', ylabel='', fs=16, dpi=100, save_path=path2sv+'/all_obs_vint_map.png')
      idlev = find_closest_index(lev, 150)
      oi_map = oi[:,idlev,:,:].mean(axis=0)
      
      #DBG
      np.savez(path2sv+'/oi.npz',oi=oi,oi_map=oi_map, lat=co_lat, lon=co_lon, labels=labels, oigr=oigr, sizes=sizes, oim=oim, oisd=oisd, obs2proc=obs2proc, ocount=ocount)
      
      fsoi_plot_map(oi_map, co_lon, co_lat, title='OI, All Obs., Model Level: '+ str( round(lev[idlev],2) )  +'\n Date: '+adate, clabel='$OI$', xlabel='', ylabel='', fs=16, dpi=100, save_path=path2sv+'/oi_150_map.png') 
...
if rank == manager:
   ...
   with Dataset(path2sv+'/mpi_oi_data.nc', 'w', format='NETCDF4_CLASSIC') as ds:
       ...
       ois_var = ds.createVariable('oi_sigma_o', 'f8', ('num_obs_types', 'model_lev', 'c_lat', 'c_lon'))
       ois_var.units = ''
       ois_var.long_name = 'sigma_o used in the OI computation. First dimension is the list of obs type: ' + str(obs2proc)
       ois_var[:] = oi_sigma_o
    
       ds.setncattr('title', 'Simplified OI for ensemble filtering: anlysis date: ' + adate)
