...
#BSUB  -P R000
#BSUB  -W 2:00
#BSUB  -u giovanni.conti@cmcc.it
#BSUB  -N  
#BSUB  -o Test1.bld1
#BSUB  -e Test1.bld1

# 36(proc_per_nodes)*1(nodes_per_instance)*20(members)=720
...
#    4) CESM's short term archiv.er (st_archive) will use a fourth directory for
...
setenv case            twaccm 
#setenv compset         HIST_CAM60_CLM50%SP_CICE%PRES_DOCN%DOM_MOSART_CISM2%NOEVOLVE_SWAV
...
setenv cesmroot     /users_home/cmcc/${USER}/${CESMDIR}
setenv sourcemods   ~/${cesmtag}/SourceMods   
setenv caseroot     /users_home/cmcc/${USER}/${CESMEXP}/${case}
setenv dartroot     /users_home/cmcc/${USER}/DART

setenv cime_output  /work/cmcc/${USER}/CESM2
setenv archdir      ${cime_output}/archive

setenv baseobsdir   /work/cmcc/${USER}/observations/201701_6H_CESM
...
setenv refcase     f.e21.FHIST_BGC.f09_025.CAM6assim.004
setenv refyear     2017
setenv refmon      01
setenv refday      15
setenv reftod      00000
...
#setenv stagedir ${cime_output}/dart_startup_2017/run #${archdir}/${refcase}/rest/${reftimestamp}
setenv stagedir /work/cmcc/gc02720/ic/Kevin_L70
#setenv stagedir /work/cmcc/gc02720/CESM2/dart_hybrid_2017/run
...
# "2 degree":
setenv sst_dataset ${cesmdata}/atm/cam/sst/sst_HadOIBl_bc_0.9x1.25_1850_2017_c180507.nc 
...
setenv sst_year_start 2017
setenv sst_year_end   2017
...
#setenv save_every_Mth_day_restart 4
...
while ($inst <= $num_instances)
   ...
   echo "ncdata = 'cam_initial_0001.nc'" >> ${fname}  
 
   echo " empty_htapes  = .true. "                        >> ${fname}
   #echo " fincl1        = 'PHIS:I','T:I','U:I','V:I','Q:I','PS:I','PSL:I','Z500:I','Z3:I'"                      >> ${fname}
   echo " fincl1        = 'PHIS:I', 'LANDFRAC:I','TREFHT:I', 'TS:I', 'ICEFRAC:I', 'OCNFRAC:I'"   >> ${fname}
   echo " nhtfrq        = -6 "                      >> ${fname}
   echo " mfilt         = 1 "                             >> ${fname}
   #echo " mfilt         = -$stop_n "                             >> ${fname}
  ...
  # if ($inst == 1) then
      if ($start_year > 2015) then
         ...
         set cesm_data_dir = "${cesmdata}/atm"
         set cesm_chem_dir = "${cesmdata}/atm/cam/chem/emis/CMIP6_emissions_1750_2015"
         set chem_root     = "${cesm_chem_dir}/emissions-cmip6"
         set chem_dates    = "1750-2015_0.9x1.25_c20170616"
         set chem_yr       = "1750-2015_0.9x1.25_c"
         ...
         echo "  'H2O -> ${cesm_data_dir}/cam/chem/emis/elev/H2OemissionCH4oxidationx2_3D_L70_1849-2015_CMIP6ensAvg_c180927.nc'"  >> ${fname}
         echo "  'num_a1 -> ${chem_root}_num_so4_a1_anthro-ene_vertical_${chem_dates}.nc'"        >> ${fname}
         echo "  'num_a2 -> ${chem_root}_num_a2_so4_contvolcano_vertical_850-5000_0.9x1.25_c20170724.nc'"        >> ${fname}
         echo "  'SO2 -> ${chem_root}_SO2_contvolcano_vertical_850-5000_0.9x1.25_c20170724.nc'"        >> ${fname}
         echo "  'so4_a1 -> ${chem_root}_so4_a1_anthro-ene_vertical_${chem_dates}.nc'"            >> ${fname}
         echo "  'so4_a1 -> ${chem_root}_so4_a1_contvolcano_vertical_850-5000_0.9x1.25_c20170724.nc'"            >> ${fname}


echo " srf_emis_specifier ="                                                                 >> ${fname}
         echo "  'bc_a4 ->  ${chem_root}_bc_a4_anthro_surface_${chem_yr}20170608.nc'"                  >> ${fname}
         echo "  'bc_a4 ->  ${chem_root}_bc_a4_bb_surface_${chem_yr}20170322.nc'"                      >> ${fname}
         echo "  'DMS ->    ${chem_root}_DMS_bb_surface_${chem_yr}20170322.nc'"                        >> ${fname}
         echo "  'DMS ->    ${chem_root}_DMS_other_surface_${chem_yr}20170322.nc'" >> ${fname}
         echo "  'num_a1 -> ${chem_root}_num_so4_a1_bb_surface_${chem_yr}20170322.nc'"                 >> ${fname}
         echo "  'num_a1 -> ${chem_root}_num_so4_a1_anthro-ag-ship_surface_${chem_yr}20170616.nc'"     >> ${fname}
         echo "  'num_a2 -> ${chem_root}_num_so4_a2_anthro-res_surface_${chem_yr}20170616.nc'"         >> ${fname}
         echo "  'num_a4 -> ${chem_root}_num_bc_a4_bb_surface_${chem_yr}20170322.nc'"                  >> ${fname}
         echo "  'num_a4 -> ${chem_root}_num_bc_a4_anthro_surface_${chem_yr}20170608.nc'"              >> ${fname}
         echo "  'num_a4 -> ${chem_root}_num_pom_a4_anthro_surface_${chem_yr}20170608.nc'"             >> ${fname}
         echo "  'num_a4 -> ${chem_root}_num_pom_a4_bb_surface_${chem_yr}20170509.nc'"                 >> ${fname}
         echo "  'pom_a4 -> ${chem_root}_pom_a4_anthro_surface_${chem_yr}20170608.nc'"                 >> ${fname}
         echo "  'pom_a4 -> ${chem_root}_pom_a4_bb_surface_${chem_yr}20170322.nc'"                     >> ${fname}
         echo "  'SO2 ->    ${chem_root}_SO2_anthro-ag-ship-res_surface_${chem_dates}.nc'"        >> ${fname}
         echo "  'SO2 ->    ${chem_root}_SO2_anthro-ene_surface_${chem_dates}.nc'"                >> ${fname}
         echo "  'SO2 ->    ${chem_root}_SO2_bb_surface_${chem_yr}20170322.nc'"                        >> ${fname}
         echo "  'so4_a1 -> ${chem_root}_so4_a1_anthro-ag-ship_surface_${chem_dates}.nc'"         >> ${fname}
         echo "  'so4_a1 -> ${chem_root}_so4_a1_bb_surface_${chem_yr}20170322.nc'"         >> ${fname}
         echo "  'so4_a2 -> ${chem_root}_so4_a2_anthro-res_surface_${chem_dates}.nc'"             >> ${fname}
         echo "  'SOAG ->   ${chem_root}_SOAGx1.5_anthro_surface_${chem_yr}20170608.nc'"               >> ${fname}
         echo "  'SOAG ->   ${chem_root}_SOAGx1.5_bb_surface_${chem_yr}20170322.nc'"                   >> ${fname}
         echo "  'SOAG ->   ${chem_root}_SOAGx1.5_biogenic_surface_${chem_yr}20170322.nc'"             >> ${fname}
         ...
         # Defaults:
         echo " tracer_cnst_datapath    = '${cesm_data_dir}/cam/tracer_cnst'" >> ${fname}
         ...
         echo " tracer_cnst_file = 'tracer_cnst_halons_3D_L70_1849-2015_CMIP6ensAvg_c180927.nc'"  >> ${fname}
         echo " tracer_cnst_type          = 'CYCLICAL'"      >> ${fname}
         echo " tracer_cnst_cycle_yr      = 2010"            >> ${fname}

        ##echo " flbc_file  = '/work/cmcc/cmip03/Forcing/SetLBCdate/LBC_1750-2020_CMIP6_GlobAnnAvg_c180926.nc' " >> ${fname}
         echo " flbc_file  = '/data/inputs/CESM/inputdata/atm/waccm/lb/LBC_17500116-20150116_CMIP6_0p5degLat_c180905.nc' " >> ${fname}
         echo " flbc_type      = 'FIXED'"   >> ${fname}
         echo " flbc_fixed_ymd = 20100702" >> ${fname}


         echo " waccm_forcing_datapath         = '/data/inputs/CESM/inputdata/atm/waccm/waccm_forcing'" >> ${fname}
...
#  echo "building_temp_method = 0 "                  >> ${fname}
   ...
   #finidat = ${stagedir}/${refcase}.clm2${inst_string}.r.${reftimestamp}.nc
   echo " finidat='f.e21.FHIST_BGC.f09_025.CAM6assim.004.clm2_0001.r.2017-01-15-00000.nc'" >> ${fname} 
   ...
   if ($status == 0) then
      ...
      echo " ice_ic = 'f.e21.FHIST_BGC.f09_025.CAM6assim.004.cice_0001.r.2017-01-15-00000.nc'" >> ${fname} 
   endif
...
echo "finidat_rtm='f.e21.FHIST_BGC.f09_025.CAM6assim.004.mosart_0001.r.2017-01-15-00000.nc'" >> ${fname}
...
# USING ONLY WITH CONTINUE_RUN=FALSE THE RESTAGING OF THE RESTART MUST BE MODIFIED!
./stage_cesm_files
