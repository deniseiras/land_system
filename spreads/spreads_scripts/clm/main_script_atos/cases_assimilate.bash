...
source ${HPCPERM}/d4o-bs/install/INTEL/source.me
# source ${HPCPERM}/d4o/install/INTEL/source.me
...
DARTROOT=$(grep "export dartroot=" "$SCRIPTDIR/cases_create_blq.sh" | sed -e "s/=/ /; s/USER/$USER/" | awk '{gsub(/[{}]/, "", $3); print $3}')
export DARTROOT=$(echo "$DARTROOT" | sed -e 's/"//g; s/\$//g')
echo " DARTROOT=  $DARTROOT"



case_name=$(grep "case_name=" "$SCRIPTDIR/cases_create_blq.sh" | sed -e "s/=/ /g" | awk '{gsub(/[{}]/, "", $2); print $2}')
export case_name=$(echo "$case_name" | sed -e 's/"//g')
echo " case_name= $case_name"

# CLONESROOT=$(grep "clonesroot=" "$SCRIPTDIR/cases_create_blq.sh" | sed -e "s/=/ /; s/USER/$USER/; s/CESMEXP/$CESMEXP/" | awk '{gsub(/[{}]/, "", $3); print $3}')
# export CLONESROOT=$(echo "$CLONESROOT" | sed -e 's/"//g; s/\$//g')
# echo " CLONESROOT=  $CLONESROOT"
CLONESROOT=${HOME}/cesm-exp/${case_name}
echo " CLONESROOT=  $CLONESROOT/${case_name}"

export NTSLOTS=$(grep "nts=" "$SCRIPTDIR/cases_create_blq.sh" | sed -e "s/=/ /" | awk '{print $2}')
...
export nens=$(grep "nens=" "$SCRIPTDIR/cases_create_blq.sh" | sed -e "s/=/ /" | awk '{print $2}')
export TOTALPES=$(echo "$TOTALPES*$nens" | bc)
echo " TOTALPES= $TOTALPES"

# TMPROOT=$(grep "tmpdir=" "$SCRIPTDIR/cases_create_blq.sh" | sed -e "s/=/ /; s/USER/$USER/" | awk '{gsub(/[{}]/, "", $2); print $2}')
# export TMPROOT=$(echo "$TMPROOT" | sed -e 's/\$//g; s/"//g')
export TMPROOT=${SCRATCH}/CMCC-CM/TMP${case_name^^}
...
cp "${SCRIPTDIR}/rep_input2.py" "${TMPROOT}/."
cd "${RUNDIR}"
echo "rundir"
pwd
...
export BASEOBSDIR="/ec/res4/scratch/ita0829/obs/b2d4o_AMSUA_V2"
...
set +e
rm -rf rep_input_list.txt
touch rep_input_list.txt
...
while [ $inst -le $nens ];do
   ...
   while [ $its -le $NTSLOTS ];do
      ...
      if [ $? -eq 0 ]; then
         ...
         # nohup /ec/res4/scratch/ita0829/spreads-atm/.conda/envs/netcdf4/bin/python ${TMPROOT}/rep_input.py ${ftoass} >/dev/null 2>&1 &
         echo ${ftoass} >> rep_input_list.txt
      fi
...
/ec/res4/hpcperm/ita0829/conda/envs/cesm/bin/python3 ${TMPROOT}/rep_input2.py $(cat rep_input_list.txt) 30
set -e
...
#-----------------------------------------------------------------------
    ...
    ic=0
    while [ "$its" -le "$NTSLOTS" ]; do
      if grep -q "0" "${TMPROOT}/TS${its}/filter.flag"; then
...
if grep -q "2" "${TMPROOT}/allTS/filter.flag"; then # CLOSE THE IF AT THE END
   ...
   echo "ERROR:CHECK ${TMPROOT}/allTS"
