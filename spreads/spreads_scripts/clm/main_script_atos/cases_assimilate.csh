...
set DARTROOT=`grep "export dartroot=" $SCRIPTDIR/cases_create_blq.sh | sed -e "s/=/ /; s/USER/$USER/"`
...
set CLONESROOT=`grep "clonesroot=" $SCRIPTDIR/cases_create_blq.sh | sed -e "s/=/ /; s/USER/$USER/; s/CESMEXP/$CESMEXP/"`
...
set case_name=`grep "case_name=" $SCRIPTDIR/cases_create_blq.sh | sed -e "s/=/ /g"`
set case_name=`echo $case_name | sed -e 's/"//g'`
set case_name=$case_name[2]
echo " case_name= $case_name"


set NTSLOTS=`grep "nts=" $SCRIPTDIR/cases_create_blq.sh | sed -e "s/=/ /"`
...
set nens=`grep "nens=" $SCRIPTDIR/cases_create_blq.sh | sed -e "s/=/ /"`
set nens=$nens[2]
set TOTALPES=`echo $TOTALPES*$nens | bc`
echo " TOTALPES= $TOTALPES"


set TMPROOT=`grep "tmpdir=" $SCRIPTDIR/cases_create_blq.sh | sed -e "s/=/ /; s/USER/$USER/"`
...
set BASEOBSDIR = "/work/cmcc/gc02720/obs/b2d4o_gpsrobs_wdp_snd"
...
while ( $its <= $NTSLOTS )
   ...
   set ldbo=`ls ${TMPROOT}/TS$its/*.db | wc -l`
   if ( $ldbo > 0 ) then
...
while ( $inst <= $nens )
   ...
   while ( $its <= $NTSLOTS )
	...
	echo -e "Member: ${inst}, TS${its}: python preproc. ${ftoass}"
        echo ${ftoass} >> replist.txt
	#python ${TMPROOT}/rep_input.py ${ftoass}
...
date
python rep_input.py replist.txt
echo -e "END PYTHON PROCESS "
