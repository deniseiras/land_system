...
CASEROOT=$1
ASSIMILATION_CYCLE=$2

source ${CASEROOT}/DART_params.sh || exit 1
...
ENSEMBLE_SIZE=$(./xmlquery NINST_LND --value)
EXEROOT=$(./xmlquery EXEROOT --value)
RUNDIR=$(./xmlquery RUNDIR --value)
ARCHIVE=$(./xmlquery DOUT_S_ROOT --value)
TOTALPES=$(./xmlquery TOTALPES --value)
STOP_N=$(./xmlquery STOP_N --value)
DATA_ASSIMILATION_CYCLES=$(./xmlquery DATA_ASSIMILATION_CYCLES --value)
TASKS_PER_NODE=$(./xmlquery MAX_TASKS_PER_NODE --value)
...
FILE=$(head -n 1 rpointer.lnd_0001)
FILE=${FILE%:*}
LND_DATE_EXT=${FILE##*.}
LND_DATE=$(echo $LND_DATE_EXT | sed -e "s#-# #g")
LND_YEAR=$(echo $LND_DATE | awk '{print $1}')
LND_MONTH=$(echo $LND_DATE | awk '{print $2}')
LND_DAY=$(echo $LND_DATE | awk '{print $3}')
LND_SECONDS=$(echo $LND_DATE | awk '{print $4}')
LND_HOUR=$(echo "$LND_SECONDS / 3600" | bc)
