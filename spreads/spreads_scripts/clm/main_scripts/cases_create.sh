...
#export MODEL_PATH="/work/cmcc/${USER}"
...
# Experiment name
case_name="land_exp60"

# Where to copy the clones (Use a clear name for the experiment. The dir name must 
# not appears in the case.template.original)
export clonesroot="/users_home/cmcc/${USER}/${CESMEXP}/land_exp60"

# Number of clones
nens=60

# DART directory
export dartroot="/work/cmcc/spreads-lnd/spreads"

# Convert case_name to uppercase for tmpdir
case_name_upper=$(echo "$case_name" | tr '[:lower:]' '[:upper:]')

# Use case_name as lowercase initially
echo "Original case_name (lowercase): $case_name"

# Compose tmpdir using the uppercase version of case_name
# DA working dir

tmpdir="/work/cmcc/${USER}/CMCC-CM/TMPLAND_EXP60"
echo "tmpdir: ${tmpdir}"
...
# Set the obs dir
#baseobsdir="/work/cmcc/mg20022/obs/b2d4o_d4o_benchmark_v1"
baseobsdir="/work/cmcc/lg07622/land/datain/observations/ESA_CCI_SM/obs/all"
...
#script_name_original="case.template-juno.original-SP-1deg"
script_name_original="case.template-juno-land"

# Chose the input namelist for DART
dartnamelist="input.nml.original.land-all"

# Ref date (YYYY-MM-DD-SSSSS)
RDATE=2005-12-01-00000

# Start date
SDATE=2000-03-01-00000

# IC dir
#ICDIR="/work/cmcc/gc02720/ic/ic_spread_ugento_1deg_oldgrid"
ICDIR="/work/cmcc/lg07622/land/work/d4o/control60/run"
...
use_SourceMods="TRUE"
SourceModDir="/work/cmcc/lg07622/land/datain/SourceMods_release-cesm2.3_lgg/SourceMods"
...
if [ ${nts} -eq 13 ]
  then
     ...
     echo " Single time slot for land for now... " 
...
if [ -d ${clonesroot} ]; then
   ${REMOVE} -r ${clonesroot}
   ${REMOVE} -r /work/cmcc/${USER}/CMCC-CM/${case_name}_*
fi
...
if [ -d ${dirff} ]; then
   ${REMOVE} -r ${dirff}
   ${REMOVE} -r /work/cmcc/${USER}/CMCC-CM/${case_name}_f_*
fi
...
if [ "${use_SourceMods}" == "TRUE" ]; then
   if [ ! -d "${caseroot}/SourceMods" ]; then mkdir ${caseroot}/SourceMods; fi
...
do
   ...
   inst_string2=`printf %04d $inst`
   
   new_case="$clonesroot/$case_name$inst_string"
   # Create the clone 
   echo "${cesmroot}/cime/scripts/create_clone  --case $new_case  --clone  $caseroot"
   ${cesmroot}/cime/scripts/create_clone  --case $new_case  --clone  $caseroot
  ...
  sed -i "s/_0001/$inst_string/g" user_nl_clm
  #${MOVE} user_nl_clm_0001 user_nl_clm
  repl="."$inst_string2"."
  sed -i "s/.0001./$repl/g" user_nl_datm_streams
  sed -i "s/_0001/$inst_string/g" user_nl_mosart
  sed -i "s/_0001/$inst_string/g" user_nl_rtm
  sed -i "s/_0001/$inst_string/g" user_nl_hydros
   ...
   echo "finidat_hydros='${refcase}.hydros${inst_string}.r.${init_time}.nc'">> user_nl_hydros
   ...
   if [ "${script_name_original}" != "case.template-juno.original-SP" ] && [ "${script_name_original}" != "case.template-juno.original-SP-foretest" ] && [ "${script_name_original}" != "case.template-juno.original-SP-1deg" ]; then
	 echo "SP activated for CLM"    
         echo "stream_meshfile_ch4finundated ='/work/cmcc/gc02720/inputdata/lnd/clm2/paramdata/finundated_inversiondata_0.9x1_ESMFmesh_cdf5_130621.nc'" >> user_nl_clm
         echo "stream_meshfile_popdens ='/work/cmcc/gc02720/inputdata/lnd/clm2/firedata/clmforc.Li_2017_HYDEv3.2_CMIP6_hdm_0.5x0_ESMFmesh_cdf5_100621.nc'" >> user_nl_clm
         echo "stream_fldfilename_lightng ='/work/cmcc/gc02720/inputdata/atm/datm7/NASA_LIS/clmforc.Li_2016_climo1995-2013.360x720.lnfm_Total_c160825.nc'" >> user_nl_clm
         echo "stream_meshfile_lightng = '/work/cmcc/gc02720/inputdata/atm/datm7/NASA_LIS/clmforc.Li_2016_climo1995-2013.360x720_ESMFmesh_cdf5_150621.nc'" >> user_nl_clm
   fi
...
#   ./preview_namelists || exit 75
   ...
   ${LINK} -f ${stagedir}/${refcase}.hydros${inst_string}.r.${init_time}.nc .
...
${COPY} ${dartroot}/d4o/flattened/clm/filter.dir/filter ${tmpdir}/.
${COPY} ${dartroot}/d4o/flattened/clm/fill_inflation_restart.dir/fill_inflation_restart  ${tmpdir}/.
...
#   fi 
   ...
   ${COPY} $dartroot/d4o/flattened/clm/filter.dir/filter $tmpdir/$tsname/.
