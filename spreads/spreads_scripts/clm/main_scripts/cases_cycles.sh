      ...
      if [ $status -eq 0 ]
        then
         ...
         do
            ...
            ./cases_assimilate_landB.bash ${FGAT}
            grep "1" check_assi.flag
            statA=$?
            if [ $statA -eq 0 ]
              then
...
#       fi
	   ...
	   then
             ...
             datevar=$(grep finidat user_nl_clm | tail -1)
       else
             #datevar=$(cat /work/cmcc/${USER}/CMCC-CM/${case_name}_0001/run/rpointer.ice | cut -d'/' -f2)
             datevar=$(cut -d'/' -f2 </work/cmcc/${USER}/CMCC-CM/${case_name}_0001/run/rpointer.lnd )
       fi
