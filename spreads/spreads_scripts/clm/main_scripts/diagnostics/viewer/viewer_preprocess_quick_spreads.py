#!/work/cmcc/spreads-atm/.conda/envs/tools/bin/python
import glob
import pandas as pd
import sqlite3
import time
...
DATAIN = f"/work/cmcc//CMCC-CM/archive/{case_name}"
DATAOUT = f"/work/cmcc//CMCC-CM/basic_diag/{case_name}"
            ...
            if 'catalog' not in f:
                ...
                try:
                    ...
                    dtemp = pd.read_sql("select id, reportype, entryno, kind, (select distinct description from toc where kind=body.kind) as description,\
                                    obsvalue, height, obs_error, prior_mean, prior_spread, posterior_mean, posterior_spread, levelht, deglat, deglon, dart_qc, member, body.status as bstatus, hdr.status as hdrstatus from hdr join body on id=body.hdr_id join ens on id=ens.hdr_id and entryno=body_entryno \
                                    where member=1 ", conn)
                    
                    dtemp['bias-prior'] = dtemp['obsvalue'] - dtemp['prior_mean']
                    dtemp['bias-poste'] = dtemp['obsvalue'] - dtemp['posterior_mean']
                    dtemp['total_spread'] = np.sqrt(dtemp['prior_spread']**2 + dtemp['obs_error']**2)
                    dtemp['obs_influence'] = dtemp['prior_spread']**2 / (dtemp['prior_spread']**2 + dtemp['obs_error']**2)
