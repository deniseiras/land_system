...
do
   ...
   ATM_DATE=`head -n 1 bk_rpointer.atm | sed -e "s#\.# #g" | awk '{ print $4}'`
   echo "$ATM_DATE"  
   cp bk_${case_name}${inst_string}.cam.i.nc       ${case_name}${inst_string}.cam.i.${ATM_DATE}.nc 
   cp bk_${case_name}${inst_string}.clm2.r.nc      ${case_name}${inst_string}.clm2.r.${ATM_DATE}.nc 
   cp bk_${case_name}${inst_string}.mosart.r.nc    ${case_name}${inst_string}.mosart.r.${ATM_DATE}.nc 
   cp bk_${case_name}${inst_string}.cice.r.nc      ${case_name}${inst_string}.cice.r.${ATM_DATE}.nc 
   cp bk_${case_name}${inst_string}.cpl.r.nc       ${case_name}${inst_string}.cpl.r.${ATM_DATE}.nc 
   cp bk_${case_name}${inst_string}.docn.rs1.bin   ${case_name}${inst_string}.docn.rs1.${ATM_DATE}.bin 
   cp bk_rpointer.atm rpointer.atm
   cp bk_rpointer.lnd rpointer.lnd
   cp bk_rpointer.rof rpointer.rof
   cp bk_rpointer.ice rpointer.ice
   cp bk_rpointer.ocn rpointer.ocn
   cp bk_rpointer.drv rpointer.drv
