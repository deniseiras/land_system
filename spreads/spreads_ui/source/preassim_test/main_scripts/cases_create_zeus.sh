...
export clonesroot="/users_home/cmcc/${USER}/${CESMEXP}/junov1"
...
# DA working dir
tmpdir="/work/cmcc/${USER}/CESM2/TMPJUNOV1"
...
baseobsdir="/work/cmcc/sm09722/ALL_D4O/b2d4o_EXP1"
...
script_name_original="case.template-zeus.original"
...
if [ -d ${clonesroot} ]; then
   ${REMOVE} -r ${clonesroot}
   ${REMOVE} -r /work/cmcc/${USER}/CESM2/${case_name}_*
fi
...
if [ -d ${dirff} ]; then
   ${REMOVE} -r ${dirff}
   ${REMOVE} -r /work/cmcc/${USER}/CESM2/${case_name}_f_*
fi
...
#   ./preview_namelists || exit 75
   ...
   cp /work/cmcc/${USER}/CESM2/case_template/bld/cesm.exe  ${RUNDIR}/../bld/cesm.exe
