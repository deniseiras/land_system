...
echo "inside ./run_dart_to_clm_snow.bash ${CASE} ${CASE} ${LND_DATE_EXT} ${CASEROOT} ${ENSAMBLE_SIZE}"
string_id=""
...
for ii in $(seq 1 ${ENSAMBLE_SIZE}); do
#  ni=$(printf "%04d" ${ii})
  RESTART=$CASEROOT/${CASE}/run/${CASE}.clm2.r.${LND_DATE_EXT}.nc
  #POSTERIOR=clm2.r.${LND_DATE_EXT}.nc
...
#for RESTART in "${CASE}.clm2_"*.r."${LND_DATE_EXT}".nc; do
    ...
    #CLM_VECTOR="${CASE}.clm2_00$(printf "%02d" $enscount).h2.${LND_DATE_EXT}.nc"
    CLM_VECTOR="$CASEROOT/${CASE}/run/${CASE}.clm2.h2.${LND_DATE_EXT}.nc"
