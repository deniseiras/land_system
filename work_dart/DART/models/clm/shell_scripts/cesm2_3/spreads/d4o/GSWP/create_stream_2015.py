...
def generate_dynamic_datafiles(stream: str, member: int, init_date: int, end_date: int) -> str:
    ...
    over the simulation period. The very first file is appended inline to the datafiles=
    keyword, and all subsequent file paths appear on new, indented lines with a trailing
    comma and a backslash (line continuation) on every line except the last.
    ...
    The base directory is:
        /work/cmcc/spreads-lnd/work/forcing/EDA/
    
    For all ensemble members the folder is "EDA_n" followed by the member number (no leading zero),
    and the file-code is "EDA" followed by the member number in two-digit zero-padded format.
    
    Returns:
      A string where the first file path appears inline, and each subsequent file path is on a new indented line,
      with all lines (except the last) ending with a comma and a backslash.
    """
    base_forcing = "/work/cmcc/spreads-lnd/work/forcing/EDA/"
...
def generate_file_content(member: int, init_date: int, end_date: int) -> str:
    ...
    The content comprises several stream blocks. For each block, the year entries
    (year_first, year_last, year_align) are updated to reflect the simulation period.
    For the three streams with dynamic file lists (Solar, Precip, TPQW), the datafiles entry
    is constructed based on the simulation period and the ensemble member conventions.
    """
    # Fixed stream: presaero.hist (static datafiles, only update year entries)
    presaero_hist = (
        "presaero.hist:datafiles=/data/inputs/CESM/inputdata/atm/cam/chem/trop_mozart_aero/aero/"
    ...
    # Streams with dynamic file lists:
    clmgswp3v1_solar = (
        ...
        "CLMGSWP3v1.Solar:meshfile=/data/inputs/CESM/inputdata/atm/datm7/atm_forcing.datm7.GSWP3.0.5d.v1.c170516/"
    ...
    clmgswp3v1_precip = (
        ...
        "CLMGSWP3v1.Precip:meshfile=/data/inputs/CESM/inputdata/atm/datm7/atm_forcing.datm7.GSWP3.0.5d.v1.c170516/"
        "clmforc.GSWP3.c2011.0.5x0.5.TPQWL.SCRIP.210520_ESMFmesh.nc\n"
    ...
    clmgswp3v1_tpqw = (
        ...
        "CLMGSWP3v1.TPQW:meshfile=/data/inputs/CESM/inputdata/atm/datm7/atm_forcing.datm7.GSWP3.0.5d.v1.c170516/"
        "clmforc.GSWP3.c2011.0.5x0.5.TPQWL.SCRIP.210520_ESMFmesh.nc\n"
    ...
    presndep_hist = (
        "presndep.hist:datafiles=/data/inputs/CESM/inputdata/lnd/clm2/ndepdata/"
        "fndep_clm_f09_g17.CMIP6-SSP5-8.5-WACCM_1849-2101_monthly_c191007.nc\n"
        ...
        "presndep.hist:meshfile=/data/inputs/CESM/inputdata/share/meshes/"
    ...
    preso3_hist = (
        "preso3.hist:datafiles=/data/inputs/CESM/inputdata/cdeps/datm/ozone/"
        "O3_surface.f09_g17.CMIP6-SSP5-8.5-WACCM.001.monthly.201501-210012.nc\n"
        ...
        f"preso3.hist:year_first={max(init_date, 2015)}\n"
        f"preso3.hist:year_last={end_date}\n"
        f"preso3.hist:year_align={max(init_date, 2015)}\n"
        "preso3.hist:vectors=null\n"
        "preso3.hist:lev_dimname=null\n"
        "preso3.hist:meshfile=/data/inputs/CESM/inputdata/share/meshes/"
        "fv0.9x1.25_141008_polemod_ESMFmesh.nc\n"
    ...
    co2tseries = (
        "co2tseries.20tr:datafiles=/data/inputs/CESM/inputdata/atm/datm7/CO2/"
        "fco2_datm_globalSSP5-8.5__simyr_2014-2501_CMIP6_c190506.nc\n"
...
def create_datm_streams_files(members: int, init_date: int, end_date: int, output_dir: str = ".") -> None:
    ...
    For each member, the file is named using a zero-padded four-digit index (e.g. user_nl_datm_streams_0001)
    and the content is built with:
      - Updated year entries (year_first, year_last, year_align)
      - Dynamically generated datafiles lists for the Solar, Precip, and TPQW streams, with the first file
        inline with the "datafiles=" keyword and subsequent files on new, indented lines.
      - All ensemble members follow the same naming convention: folder "EDA_n" followed by the member number (no leading zero)
        and file-code "EDA" followed by a two-digit zero-padded member number.
    """
